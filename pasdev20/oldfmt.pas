...
(* Program to format PASCAL source programs for ease of reading and editing.
  ...
  files written to 'LPT:' are put in the same print request.
   ...
   and the defaults will be chosen from:
      ...
      /LIST,UPPERCASE,NODECAP,COLUMN=49,LENGTH=45,NOXREF,NOAPPEND,
	QUEUE:"/noheader/loc:110"
   ...
   compilable form.  Unless overridden by the <output file> specification,
   ...
   literal strings, or directive lines.  Nested comments are handles, and
   so are conditional comments; i.e., anything within (*$X...*) or (*$Y...*)
   ...
   The <queue> option allows specification of options to be past to the QUEUE
   ...
   The command interpreter permits previous line editing. *)
$PAGE includes
$INCLUDE rlb:cmdutl.typ
$INCLUDE rnd:qstr.typ
$INCLUDE rnd:editre.inc
$INCLUDE rlb:lookup.typ
$INCLUDE rlb:filutl.inc

external procedure passcc;			(* magic entry to runtime to pass all characters *)

type runfile = packed array[1..132] of char;

external procedure run (fid: runfile; from_file: boolean);
$PAGE declarations
label 100,					(* EOF abort point *)
      150,					(* next command *)
      200;					(* exit *)

const cr=chr(15b);				(* "character" returned at EOLN by READCH *)
      tab=chr(11b);
      maxtitle_length = 70;

type						(* processing modes *)
  modes = (intext, incomment, inliteral, indirective);
  title_type =  packed array[1..maxtitle_length] of char;
...
var
  ...
  lineno: 0..99999;				(* line number reset every page *)
...
var
  ...
  lindex, idx: cmdlineidx;			(* parsing cursor *)
  filename: file_id;				(* file to process *)
  fid_qualifier: file_id;			(* default extension, ppn, etc. *)
  default_fid_qualifier: file_id;		(* above default applied to all *)
  defaultoptions: options;			(* default option set *)
  curoptions: options;				(* options in effect for processing of a single file *)
  file_queued: boolean;				(* indicates output to 'LPT:' *)
  queue_file: file_id;				(* name of queue file *)
  queue_options: string;			(* special options for queue *)
...
procedure list_header;
 ...
 procedure title_line;
  begin
   writeln ( output, 'File ', filename,
		     '  MDSI, Company Confidential  ',
		     'Date ', date,
		     '  FORMAT Version 1.6  ', compdate );
   writeln (output)
  end;

 begin
  filename := pathname (input);			(* get actual path of file *)
  fname := substr (filename, index (filename, ':') + 1);    (* strip dev and ppn *)
  fname := substr (fname, 1, index (fname, '[') - 1);

  for i := 1 to 4 do title_line;
...
$PAGE flush
  ...
  begin
	...
	if curoptions.pagelength <> 0 then
	  if (lineno mod curoptions.pagelength) = 1 then do_phys_page(title_heading);
      write (lineno:5, tab)
    end;
    writeln (buffer:bufidx)
  end;
$PAGE readch
  ...
  procedure readch;
   begin
     if eoln then ch := cr			(* use CR to signify end of line *)
     else begin
       read (ch);
	...
	(commentlvl>0) (* implying curmode=incomment *) ) then
	  ...
	  else if eoln then ch:= cr		(* implicitly eating the at-sign *)
	  else begin
	    read(ch);
	    ch:= uppercase(ch)
	  end;
...
$PAGE nextch
  ...
  begin
	...
	then begin
      ...
      flush; readln; startline;
      if eof then begin				(* clean up *)
	if list = curoptions.outputopt then page (output);
	goto 100				(* exit mainline while not EOF loop *)
      end;
    end;

    readch					(* get the next character *)
  end (*NEXTCH*);

$PAGE list mainline
begin

  passcc;					(* pass all wierd characters through runtime *)
  open(tty); rewrite(tty);
  writeln (tty, 'FORMAT, Version 1.6');
  line := '';					(* init control vars *)
  title_heading := '                             ';
  ...
  default_fid_qualifier := '';
  file_queued := false;
  queue_options := '/noheader/loc:97';

  loop						(* through files *)
    write(tty,'*'); break;
    line := editread (line);

  exit if line = '';				(* until a blank line entered *)

    lindex := index (line, '/', length (line) + 1); (* get filename *)
    filename := substr (line, 1, lindex - 1);

    idx := index (filename, '=');		(* see if there is output info *)
    if idx = 0
      then fid_qualifier := default_fid_qualifier
      else begin
	fid_qualifier := substr (filename, 1, idx - 1);
	...
	if not lookup_cmdnames (line, lindex, cmdnamelist, maximum (commands), cmd)
	  then begin
	    ...
	    queueopt:	begin
			  ...
			  if not checkpunct ('"') then begin
			    writeln (tty, 'Option string expected.');
			    goto 150;
			  end;
			  idx := index (substr (line, lindex), '"');
			  if idx = 0 then begin
			    writeln (tty, 'String not closed.');
			    goto 150;
			  end;
    ...
    if uppercase (fid_qualifier) = 'LPT:' then begin	(* queue result to printer *)
      fid_qualifier := 'DSK:###FMT.LST';
      curoptions.appendflag := true;
      if not file_queued then begin		(* create a null queue file *)
	rewrite (output, '###fmt.lst');
	queue_file := pathname (output);
	close (output)
      end;
      file_queued := true;
    end;

    if filename = '' then begin			(* default option setting *)
      defaultoptions := curoptions;
      default_fid_qualifier := fid_qualifier;
      goto 150
    end;
    ...
    case curoptions.outputopt of		(* get suffix for output file *)
      list:  fid_qualifier := '.lst[,] ' || fid_qualifier;
      edit:  ;					(* will go in original *)
      newop:   fid_qualifier := '.tmp[,] ' || fid_qualifier
    end;

    reset(input,'.PAS '||filename);
    if eof (input) then begin			(* non existent file *)
      writeln (tty, 'File is null or nonexistent.');
      goto 150
    end;
    rewrite(output, '.PAS ' ||filename|| ' ' || fid_qualifier, curoptions.appendflag);
    ...
    indirective:
      begin
	if ch = cr then curmode := intext;	(* assume directive not in middle of comment *)
	nextch
      end
...
100 (* EOF abort *) :
    ...
    close(output);				(* close up this file *)
