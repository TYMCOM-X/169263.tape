...
type
  with_desc = ^with_descriptor;	(* gives addr for a with construct *)

  with_descriptor = packed record	(* one for each with *)
    with_addr: addr_desc;	(* base of record *)
    prev_with: with_desc;
    reg_allocated: boolean			(* true if reg allocated for this with *)
  end;

public var
  regdesc: reg_descriptor;	(* for all registers *)
  last_with: with_desc;		(* recentmost with statement *)
...
public function fetch (exp: expr; targ_addr: addr_desc ): addr_desc; forward;
...
public procedure decr_reg_usages (reg: registers);
 ...
 begin
  if (reg > 1) andif (reg <= max_reg) andif not regdesc[reg].with_reg then
  with regdesc[reg] do begin
...
begin
  if ( reg > 1 ) andif ( reg <= max_reg ) andif not regdesc[reg].with_reg then begin
    with regdesc[reg] do begin
...
public procedure with_start (withtpl: tuple);
var
  w: with_desc;
  maddr: addr_desc;
  reg: registers;
...
begin
  maddr := fetch (withtpl, no_preference);	(* get addressibility for record *)
  new (w);
  w^.prev_with := last_with;
  last_with := w;
  with maddr do begin
    if indirect orif (index <> noreg) orif (register <> noreg) andif
     (register < fp) andif not regdesc[register].with_reg then begin

    (* load address into an index register *)

      w^.reg_allocated := true;
      (* Attempt to reuse base register *)

      if (register <> noreg) andif (register < fp) andif not regdesc[register].with_reg then
	regdesc[register].uses_remaining := 0;
      if index <> noreg then regdesc[index].uses_remaining := 0;
    ...
    else w^.reg_allocated := false;
  end;
  w^.with_addr := maddr;
  withtpl^.result := ptr (ord (w));	(* for future reference *)
end;  
$PAGE with_end
  (* WITH END terminates a with construct *)

public procedure with_end;
var
  w: with_desc;

begin
  w := last_with;
  last_with := w^.prev_with;	
  if w^.reg_allocated then with w^.with_addr do begin
    regdesc[register].with_reg := false;
    regdesc[register].uses_remaining := 0;
  end;
  dispose (w);
end;  
...
type param_ary = array [ 1..32 ] of addr_desc;
...
begin
  with exp^ do begin
    arg_count := nopers;	(* count of argument longwords, *)
...
begin
  with exp^ do begin
    ...
    if result_vtype = operand_addr.byte_size
      ...
      assert ( cvt_opcode[result_vtype] <> 0 );	(* QUAD WORDS ??? *)
      opc := typ_opc ( cvt_opcode [ result_vtype ] , operand_addr.byte_size );
...
begin
  with exp^ do begin
    ...
    if nopers = 1 then begin	(* real to integer round *)
...
begin
  ...
  must_load := not is_register ( addr ) orif	(* must load in value not in reg, or, *)
	       ...
	       regdesc[primary_reg].with_reg orif	(* if with register, or, *)
...
begin
  ...
  indirect := passed_by_address ( id_sym );
  if (id_sym = id_sym^.block^.return_sym) and	(* Fcn values which are stored locally *)
     (not indirect)	(* must be addressed relative to FP *)
    then param_addr.reloc.kind := local_sc;	(* we force this by changing the *)
					(* relocation kind to local_sc *)

  if indirect then param_addr.indirect := true;
...
begin
  with exp^ do begin
    assert ( id_sym^.kind in [consts,vars,values] );	(* must hold to access storage class field *)
...
function binary_addr ( opcode: opc_range; op1_addr: addr_desc; 
	op2_addr: addr_desc; attributes: op_attr_set; targ_addr:addr_desc): addr_desc;
...
begin
  with exp^.cst_val do begin
    ...
    if kind in [scalar_cst,ptr_cst] then begin	(* pointer and scalar constants *)
      ...
      else if (kind = set_cst) andif (valp^.set_len = 0)
...
public function offset_addr ( addr, offset: addr_desc ): addr_desc;
...
begin
  with field_ref_exp^ do begin
    ...
    if base_rec^.result <> nil then begin	(* Field accessed under a WITH *)
      with_desc := ptr ( ord ( base_rec^.result ) );
      base_addr := with_desc^.with_addr;
    end
    else begin					(* Record specified explicitly *)
...
$PAGE do_size_op
(* DO_SIZE_OP generates code for a SIZE_OP tuple (parameter EXP).
   If a flexible type is involved code is generated to compute
   the size at run time.  Otherwise an address descriptor is 
   returned for the constant size.  *)

function do_size_op ( exp: expr ): addr_desc;

var
  fixed_size: unit_range;
  align: align_range;
  flex_type: typ;
  fld_sym: sym;
  elem_size: bit_range;
...
begin
  with exp^ do begin
    ...
    (* First get the constant component of the size.  If the type
       is not a flex type then we're done.  Otherwise code will be
       generated to compute the size at run time.  If the type is
       a record whose last field is flexible, then the constant part
       includes the fixed size portion of the record and the
       upper bound word.  Note that the size field
       in variant type nodes is the maximum fixed size of any record
       containing that variant.  *)

    alc_data ( type_ref, fixed_size, align );
    fixed_size := fixed_size div byte_size;

    (* If the TYPE_PARM field of EXP is not NIL, then we have
       a flexible type and must compute the size at run time.  *)

    if type_parm <> nil then begin
      flex_type := type_ref;

      (* if the type is a record or variant then the last
	 field must be a flex array or string.  Set a pointer to
	 the type node for the array or string. *)

      if flex_type^.kind in [records,variants] then begin
	fld_sym := flex_type^.field_list;
	while (fld_sym^.next <> nil) andif
	      (fld_sym^.next^.fld_variant = fld_sym^.fld_variant) do
	  fld_sym := fld_sym^.next;
	flex_type := fld_sym^.type_desc;
      end;

      (* Calculate the number of string or array elements per word.  *)

      assert ( flex_type^.flexible );
	...
	lower_bound := flex_type^.index_type^.minval;
      end;
      
      (* Calculate (upb - lwb + 1) * bytes_per_elem + fixed_size  *)

      bytes_per_elem := elem_size div bits_per_byte;
      const_part := 1 - lower_bound;

      if (fixed_size mod bytes_per_elem) = 0	(* incorporate fixed portion size into *)
	then const_part := const_part + fixed_size div bytes_per_elem;	(* array size calc, if possible *)

      upb_addr := fetch ( type_parm , no_preference );	(* fetch address of upperbound *)
      vtype := upb_addr.byte_size;	(* vax_type of bound determines vax_type of result *)
      temp_addr := add_constant ( upb_addr, const_part, no_preference );	(* add constant part to bound *)

      if bytes_per_elem <> 1 then begin	(* multiply dimension by bytes per elem, if necessary *)
	free ( temp_addr );
	reg := get_vax_reg ( vtype );
    	elem_size_addr := typ_int_value ( bytes_per_elem, vtype );
	result_addr := typ_reg_addr ( reg, vtype );
	opc := typ_opc ( mull2, vtype );
	if is_register ( temp_addr ) andif (reg = temp_addr.register)
	  then gen2 ( opc, elem_size_addr, result_addr )
	  else gen3 ( opc + 1, elem_size_addr, temp_addr, result_addr );
      end
      else result_addr := temp_addr;

      if (fixed_size mod bytes_per_elem) <> 0	(* add in fixed size, if we didn't earlier *)
	then result_addr := add_constant ( result_addr, fixed_size, no_preference );

      do_size_op := result_addr;

    end

    (* Not a flexible type.  Return an address descriptor for the
       constant size.  *)

    else begin
      do_size_op := int_value ( fixed_size );
    end;
...
begin
  with exp^ do begin
	...
	then upb_addr := cvt_long ( duplicate_addr ( base_desc.len_addr ),
...
begin
  with exp^ do begin
	...
	begin	(* case conversion op - chars only!! *)
	  assert ( desc.kind = chars );
	  do_fetch := fetch ( operand[ 1 ] , targ_addr );
	  if opcode = lwc_op
	...
	do_fetch := str_search_verify ( exp );
      ...
      ALC_TEMP_OP: begin
	assert ( ( exp^.desc.base^.size mod byte_size ) = 0 );
	DO_FETCH := GET_TEMP ( INT_VALUE(EXP^.DESC.BASE^.SIZE Div BYTE_SIZE ) , VAX_LONG );
      end;
