...
procedure move_str_date ( dest: strop; src: expr );
 const one_arg: val = (scalar_cst, -#O1000000); (* [-1,,0] *)
 var r: registers;
 begin
  gen_rr (push, sb, sp);
...
var
    set_size, ix: integer;     (* length in words *)
    maddr: addr_desc;
    reg: registers;
  ...
  if lhs^.desc.set_cst_lwb and lhs^.desc.set_cst_len then begin
    ...
    if set_size = 3 then begin  (* clear with SETZM *)
      ...
      gen_rm (dmovem, 0, maddr);
        ...
        gen (move, reg, 0, 0, gen_blt (#O1000000 - set_size, none, 0, maddr.reloc));
      end
      else (* must generate address at runtime *) begin
        reg := get_reg (anyreg, 36);
        gen_rm (move, reg, maddr);      (* load set address *)
        gen_ri (hrli, reg, #O1000000 - set_size);
      end;
      gen_rx (setzm, 0, reg);
...
begin
  ...
  if cst_addr (laddr) then begin
    gen (move, reg, 0, 0, gen_blt (#O1000000 - set_size, none, laddr.offset, laddr.reloc));
  end
  else begin
