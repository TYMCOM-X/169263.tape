...
$PAGE Bound Command
  ...
  begin
    next := false;
    idx := buffer.curlineno + buffer.lbound - 1;    (* save curlinenterms of unbounded linenos *)
...
$PAGE Indent Command
  ...
  begin
    ...
    for ln := cmdrange.lbound to cmdrange.hbound do begin
      ...
      source := qgetline (buffer, ln, err); 
      chkerr;
      pos := 1;  
      col := 1;  (* derive first non-white column *)
      while (pos <= length (source)) andif (source[pos] <= ' ') do  begin
	if source [pos] = ' '
	then col := col + 1
	else if source[pos] = tab then 
	  repeat 
	    col := col + 1 
	  until (col mod 8) = 1;
	pos := pos + 1
...
$PAGE Command Dispatcher and Action Routines
  ...
  case cmd of
	...
	if discard_changes then begin
	  ...
	  execrange.hbound := 0;
	  qinit (buffer);
	  qfileload(buffer,fid,wmod,0,cnt,err);
	  execrange.hbound := execrange.hbound + cnt;
	  if lno < ble then ble := ble + cnt;
	  buffer.curlineno := buffer.nbr_in_entire_file;
	  chkerr
	end
      end;
	...
	chkerr;
	ck_extra_txt;
	if (optcmd = insert) and (tmprange.lbound > 0) then
...
$END
    ...
    print:
      begin
	ck_extra_txt;
	qlistlines (buffer, cmdrange.lbound, cmdrange.hbound, ttyoutput, true, false,
	  qdata.tabprint, err);
	chkerr;
	buffer.curlineno := cmdrange.hbound
      end;                                      (* print *)
	...
	if cmd_lookup (line, lindex, token_chars, setparams, match) then begin
	  ...
	  case setopt of
	    ...
	    mark_param:
	      begin
		if not spredparse (line, lindex, predi, qdata.wildswitch, err) then
		  seterr(qnoparamval);
		ck_extra_txt;
	...
	if (cmdrange.lbound <> (cmdrange.hbound - total)) or (total = 0) or
	  (all_splitop in splitop_set)
	then print_times (total)
      end;                                      (* qsplit *)
	...
	qlistlines (buffer, buffer.curlineno, buffer.curlineno, ttyoutput, true, false,
	  qdata.tabprint, err);
	chkerr
      end;                                      (* uparrow *)
