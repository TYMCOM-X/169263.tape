...
$SYSTEM versio.inc[31024,320156]
$SYSTEM ptmcon.inc
$SYSTEM vaxexp.inc
$IF OPTSTATS
...
$ENDIF
  ...
  rt_refs: packed array [rt_symbol] of boolean; (* table of runtime syms. ref'd. *)
...
begin
  ...
  cr := sdi_chain;
  for i := 1 to sdi_count do  (* fill in the sdi table *)
    with sdi_list^[i] do begin
      assert (cr^.operands[cr^.noperands].reloc.reldef^.defined);
      minimum_span := cr^.operands[cr^.noperands].reloc.reldef^.addr
        - op_length(cr) - 1 - cr^.operands[cr^.noperands].offset;   (* minimum span of sdi i *)
      if abs (minimum_span) <= 50 then
        ...
        if (span > 127) or (span < -128) then begin
          ...
          for j := 1 to sdi_count do   (* look for branches whose span includes sdi i *)
            if sdi_list^[j].span <> 0 then begin
              ...
              min_distance := ic - op_length(sdi_list^[j].sdi_ptr) - 1
                - sdi_list^[j].sdi_ptr^.operands[br_operand].offset;
              if min_distance > 0 then begin  (* j before i *)
                if sdi_list^[j].minimum_span > min_distance then (* forward jump beyond i? *)
                  sdi_list^[j].span := sdi_list^[j].span + long
              end
              else if min_distance < 0 then (* j after i *)
                if sdi_list^[j].minimum_span < min_distance then (* backward jump to before i? *)
                  sdi_list^[j].span := sdi_list^[j].span - long
            end;
...
procedure rel_longword (int_value: integer);
...
(* REL INIT opens the relocatable binary output file and emits those records
   ...
   pring of the intermediate form.  *)
...
$ENDIF
    ...
    for rt := minimum (rt) to maximum (rt) do
...
procedure stoim_b_int (int_value: integer);  (* emit integer in one byte *)
  begin
    int_bytes.int := int_value;
    stoim (int_bytes.bytes.b1)
  end;
  
procedure stoim_w_int (int_value: integer);  (* emit integer in two bytes *)
  begin
    int_bytes.int := int_value;
    stoim (int_bytes.bytes.b1);
...
procedure stoim_l_int (int_value: integer);  (* emit integer in four bytes *)
  begin
    int_bytes.int := int_value;
    with int_bytes.bytes do begin
...
procedure stoim_disp ( int_value: integer; disp_size: elem_sizes );
...
procedure make_room_for_non_stoim (bytes_required: integer);
...
begin
      ...
      if public_dcl then begin (* only concerned with publics here *)
        ...
        if kind in [vars, conditions] then begin
          ...
          if init_value.kind = subr_cst then begin      (* global defns for public routines *)
            ...
            rel_longword (get_def (subr_def, init_value.blkp^.number)^.addr);
            int_bytes.int := 0;
  ...
  if ((startdef = nil) and (byte_in_word > 0))
			or
     ((startdef <> nil) and (byte_in_word = 0)) then begin
