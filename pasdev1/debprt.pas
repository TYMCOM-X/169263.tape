  ...
  real_template: packed record
                   case boolean of
                     ...
                     false: (negative: boolean;
                             ...
                             rest_of_frac: 0..#O377777777;
                             ignored: boolean;
                             frac_extension: 0..#O377777777777);
                 end;
...
var
  ...
  idx:          minimum (set_range)..maximum (set_range) + 1;
...
begin
  ...
  if desc.dtype.set_element_type <> ord (nil) then begin
    ...
    (* The following loop examines the bits of the set, printing those set
       ...
       is set in a row, range notation "x..y" is used.  Since the automaton doesn't recognize
       the end of a range until seeing the 'off' bit following it, we scan one bit
       beyond the actual end of the set (relying on ext$setelem to return false for it).  *)

    prefix := char (0);
    state := previous_bit_not_set; (* initialize our little automaton *)

    for idx := desc.set_min to desc.set_max + 1 do begin
