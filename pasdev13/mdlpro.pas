...
begin					(* procedure post_process *)
  resptr := #o400012;			(* for symbols in MAIN *)
...
$PAGE post_process the symbol definitions
  if newmdl^.multiseg then begin	(* sharable, define LTV for res *)
    ...
    resptr := #o400012;			(* LTV of sharable, id and stcheck *)
    repeat
  ...
  while a <> nil do
    with a^ do begin			(* if any, indeed *)
      loseg := ((loseg + #o777) div #o1000) * #o1000;	(* page bdy *)
      aorig := loseg;
      loseg := loseg + asize;
      a := next
      end;

  loseg := ((loseg + #o777) div #o1000) * #o1000;	(* round up *)
  ...
  while m <> nil do begin		(* also check locals not on MTV *)
    if m^.marea = nil then loseg := resptr	(* where to start *)
      else loseg := m^.marea^.aorig + #o12;	(* + 1 for mod ID word *)
