...
procedure do_movem (maddr: addr_desc; reg: registers; p: bit_range; alignment: data_alignment);
 begin
  case maddr.mode of
    ...
    byte:   begin
	      if alignment = left_alignment then begin
		clr_reg (reg);
...
ved_sb := get_temp (1);			(* get space for a pointer *)
...
$PAGE alc_temps - main routine
 ...
 begin
  global_stk_begin := cur_block^.stack_end;
  init_code;
  while code_ptr <> nil do begin
    block_stk_begin := cur_block^.stack_end;
    while not end_block do begin
      stmt_stk_begin := cur_block^.stack_end;
	...
	with code_ptr^ do begin
	  if (kind = instruction) andif (inst.opcode = pushj) andif
	    ...
	    if called_block^.owner = cur_block^.owner then
	      stmt_stk_begin := max (stmt_stk_begin, called_block^.stack_end);
	  end;
	end;
	next_code;
	...
	with code_ptr^.tempname^ do
	  if allocate and not free_at_stk and not global then begin
  ...
  cur_block^.stack_end := global_stk_begin;
  if stack_frame <> nil then
    def_value (stack_frame, cur_block^.stack_end, false);
...
procedure compile_body ( stack_frame: def );
 ...
 begin
  ...
  reg_reset (t_chain^.upward_thread^.block_order_no - 1);
  ...
  while tpl <> nil do begin
	...
	goto_op:
	  ...
	  begin
        ...
        sub_range_chk:
	  range_check (tpl, rt_sub_chk);

	str_range_chk:
	  string_range_check (tpl);

	val_range_chk:
	 range_check (tpl, rt_val_chk);

	ptr_chk:
	  pointer_check (tpl, rt_ptr_chk);

	file_chk:
	  pointer_chk (tpl, rt_fil_chk);
...
procedure compile_subr;
 ...
 begin
  ...
  if not quick then begin
    ...
    if cur_block^.level > 2 then begin	(* nonquick, not level 1 need parent stack ptr *)
      for lev := cur_block^.max_call_level downto cur_block^.apparent_level do
	gen (hrr, sp, sp, 1, none);	(* emit display chasing instructions *)
    end;
  end;
...
begin
  ...
  if prog_options.overlay_opt then begin
    loc_cst := lowseg_break; (* emit consts in lowseg for overlay compilation *)
    cst_save := cst_area; (* put origin word at start of cst area *)
    cst_area.first := nil;
    cst_area.last := nil;
    gen_origin (cst_area, loc_cst);
    cst_area.first^.next := cst_save.first;
    cst_area.last := cst_save.last;
  end
  else
  ...
  if prog_options.overlay_opt and prog_options.debug_opt then
    rel_end (startaddr, loc_cst, loc_fp)
  else if prog_options.overlay_opt and not prog_options.debug_opt then
    rel_end (startaddr, loc_cst, loc_code)
  else if not prog_options.overlay_opt and prog_options.debug_opt then
