...
public var
  ...
  high_base: def; (* relocatable #O400000 *)
...
(* In order to release stack space allocated to a dynamic temporary, a special
   ...
   end pointer (reg=sb=#O17) is created in the prologue.  It is marked as un-
...
procedure agg_assignment (tpl: tuple);
 ...
 type mask_array = packed array [0..17] of #O000000 .. #O777777;

 const mask: mask_array =
     (  #O000000, #O400000, #O600000,  #O700000, #O740000, #O760000,
        #O770000, #O774000, #O776000,  #O777000, #O777400, #O777600,
        #O777700, #O777740, #O777760,  #O777770, #O777774, #O777776  );
    ...
    if rhs^.opcode = func_call_op then begin
      ...
      if lhs^.opcode <> desc_ref then begin
        ...
        if agg_width > 0 then begin
          ...
          lock (r);
          maddr := locate (lhs);
          unlock (r         do_movem (maddr, r, agg_width, left_aligned)
        end;
...
(* PROLOGUE establishes the register state at block entry.  Specifically, it scans
   ...
   (1) tags the display_op as residing in reqister #O16 (sp), (2) tags all parameters
  ...
  while code_ptr <> nil do begin
    with code_ptr^ do begin
      if (kind = instruction) andif (reloc.kind = temp_sc) then begin
        inst.index := sp;
        if inst.offset <= #O3777777
          then inst.offset := inst.offset + reloc.relval^.loc.offset
          else inst.offset := inst.offset - #O1000000 + reloc.relval^.loc.offset;
        ...
        if bptr.bpoffset <= #O377777
          ...
          else bptr.bpoffset := bptr.bpoffset - #O1000000 + bpreloc.relval^.loc.offset;
...
procedure cleanup;
 ...
 function used (rel: reloc_value): boolean;    (* determines if location referenced is allocated *)
  begin
   case rel.kind of
...
procedure compile_body ( stack_frame: def );
 ...
 begin
  ...
  while tpl <> nil do begin
        ...
        assign_op:
          begin
            case lhs^.desc.kind of
              bools, ints, chars, scalars, pointers, files, procs, funcs:
	...
	retjump_op: ; (* just information for optimizer *)
        ...
        first_expr..last_expr:
          if opcode = alc_temp_op then begin
...
begin
  ...
  if assembly_opt in all_opts then
    mac_header ('DEC 10 Optimizing compiler');
