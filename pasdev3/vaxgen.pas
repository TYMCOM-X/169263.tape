...
static var
  ...
  label_numbering: int_type;	(* for sequential numbering of labels across blocks *)
  ...
  FPSP_DIFF : INT_TYPE;	(* Contains the displacement difference between the
			   SP and FP when using dynamic temps *)
...
begin
  ...
  If ACONSTP ( DESC , LEN )
    Then Begin
      ...
      temp_base := ngm (cur_block^.stack_end + temp_base + LEN ,
		   vax_size[vax_alignment]) - cur_block^.stack_end;
      temp_max := max (temp_max, temp_base);
      get_temp.offset := - (cur_block^.stack_end + temp_base);
    End
  Else Begin
...
procedure bb_start ( label_node_tuple: tuple );
...
procedure bb_end ( jump_tuple: tuple );
...
procedure init_static;
 ...
 var
   ...
   need_origin: boolean;
   id: byte;
   static_size, actual_size, tsize: bit_range;
   talign: align_range;

 begin

  if lowseg_break > 0 then	(* init psect *)
    gen_origin (static_area, static_psect);
  need_origin := root_block^.children^.kind = data_blk;
  static_size := 0;	(* Track size of emitted constants *)
  block := root_block;
  while block <> nil do begin
	...
	with symbols^ do begin
	  if (kind = vars) andif (dcl_class = static_sc) andif
	    ...
	    skip_bytes (static_area, static_size, ngm (static_size, talign));
	    ...
	    if (init_value.kind = string_cst) andif (init_value.valp^.str_varying_ref) then begin
	      ...
	      actual_size := str_lw_width + init_value.valp^.str_len * char_size;
	      skip_bytes (static_area, actual_size, type_desc^.size);
	    end;
	    static_size := static_size + tsize;
	  end
	    ...
	    init_value.defp := valdef (reld.reldef);
	  end;
	  symbols := next;
	end;
  ...
  if (lowseg_break > loc_static) and (assembly_opt in prog_options.semantic_options) then
    mac_pad (lowseg_break - loc_static);  (* put .BLKB for unitialized static area into macro listing *)
 end;
$PAGE store
...
(* BLOCK_MOVE moves a specified number of bytes of uninterpreted
   ...
   SOURCE_ADDR is the base addresthe source and DEST_ADDR
...
procedure agg_assignemt (tpl: tuple);
...
begin
  with tpl^ do begin
	...
	free (upb);
      ...
      else block_move ( typ_int_value (ngm (rhs^.desc.base^.size, bits_per_byte)
	...
	if dynamic_flex (rhs) orif (rhs^.opcode = ident_ref) andif
	 (rhs^.desc.base^.flexible) then begin	(* on heap or parameter *)
	  ...
	  block_move ( typ_int_value (rhs^.desc.base^.size div bits_per_byte, vax_word), raddr, laddr);
	end;
	free (upb);
      end (* target flexible *)
	...
	block_move ( typ_int_value (lhs^.desc.base^.size div bits_per_byte, vax_word), raddr, laddr);
      end;
...
begin (* expand_agg_constructor *)
  ...
  with alc_temp_tpl^ do begin
    ...
    result := nil;
    ref_fre := 0;
    usage_count := agg_val_tpl^.nopers + 2 (* n opers + agg_val + below *)
  end;
  ...
  if agg_val_tpl^.desc.kind = arrays then begin
    ...
    for i := 1 to agg_val_tpl^.nopers do begin
...
begin
  with cur_block^ do begin
    ...
    (* Set the difference value, FPSP_DIFF, for the stack to frame ptr *)
    FPSP_DIFF := 8;
...
begin
  with value_chk_op^ do begin
    ...
    low_const := aconstp ( low_addr, low_value );	(* TRUE if *scalar* const *)
    high_const := aconstp ( high_addr, high_value );
    low_is_minint := low_const andif (low_value = int_std_min);
    high_is_maxint := high_const andif (high_value = int_std_max);

    if not (low_is_minint and high_is_maxint) then begin

      cmp_opcode := typ_opc ( cmpl, vtype );
      if low_const and (low_value = 0)
        then gen1 ( tstl, value_addr )
	else gen2 ( cmp_opcode, value_addr, low_addr );	(* cmp value to low limit *)
      high_cmp_label := make_def ( local_def );
      gen_branch ( bgeq, high_cmp_label );	(* if value >= low limit, branch *)
						  (* to high limit check *)
      ...
      mark_def ( code_area, high_cmp_label );
      if not high_is_maxint then begin
	gen2 ( cmp_opcode, value_addr, high_addr );	(* cmp value to high limit *)
	gen_branch ( bgtr, err_call_label );	(* if value > high limit, branch *)
						(* to call to error routine *)
      end;
    end;
  ...
  with sub_check_op^ do begin
    ...
    if iconstp ( operand[ 1 ], index_value ) then begin
	 ...
	 get its value also.  *)
      ...
      (* If we detect a range error at compile time, then
	 simply generate a call to the error routine.  *)

      index_error := ( index_value < lwb_value ) or	(* below lower bound, or *)
		     ( ( upb_is_constant ) andif 	(* below upper limit if *)
		       ( index_value > upb_value ) ) or	(* known, or *)
		     ( ( upb_vtype = vax_word ) and	(* we have a string and *)
		       ( ( index_value < 1 ) or	(* index is outside hardware *)
			 ( index_value > max_str_length ) ) );	(* imposed limits *)
...
procedure compile_body ( var begin_addr: def );
 ...
 begin
  ...
  make_basic_blocks (label_numbering);	(* explode booleans and supply labels *)
  ...
  i := ngm (cur_block^.stack_end + temp_max - cur_block^.stack_begin, bytes_per_unit);
  if i > 0
...
procedure compile_subr;
 ...
 begin
  with cur_block^.subr_sym^.name^ do		(* emit title for assembly listing *)
    gen_cmt (code_area, 'Begin subroutine ' || substr (text, 1, len));
...
begin
  with cur_block^.id^ do		(* emit title for listing *)
    gen_cmt (code_area, 'Begin program ' || substr (text, 1, len));
...
begin
  ...
  label_numbering := 0;	(* start label numbers sequential from zero *)
  ...
  if switch (cur_block^.dump_switches, 'STORAGE') then
    dmpstorage;


  (* ASsign psect id's. *)

  if lowseg_break <> 0 then begin
    psect_id[static_psect] := 0;
    psect_id[data_psect] := 0;
  ...
  else if rel_file <> '' then
    del_file (rel_file || '.DEB[,]');
