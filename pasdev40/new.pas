...
type
  ...
  bucket_record = record
    module_name : string[8];
    page_number : four_bytes;
    start_PC : four_bytes;
    end_PC : four_bytes;
    count : four_bytes;
  end;
...
var
  ...
  program_end_PC : four_bytes;
  number_buckets : four_bytes;
  runtime_count : four_bytes;
  unknown_count : four_bytes;
  trace_flag : boolean;
  ...
  bucket_array : ^array [1..*] of bucket_record;
  bucket_file : text;
  result_file : text;

public var
  flag : boolean;
  tr_nd : boolean; (* trap not done flag *)
$PAGE error
...
var
  local_index : four_bytes;
  local_buffer : string[20];
  local_module_name : string[8];

begin
  new (bucket_array, number_buckets);
  for local_index := 1 to number_buckets do begin
    readln (bucket_file, local_buffer);
    if substr (local_buffer, 1, 7) = 'Module' then begin
      local_module_name := substr (local_buffer, 8);
      readln (bucket_file, local_buffer);
    end (* if *);
    with bucket_array^[local_index] do begin
      module_name := local_module_name;
      getstring (substr (local_buffer, 1, 8), start_PC:8:h);
      getstring (substr (local_buffer, 10, 3), page_number:3);
      count := 0;
    end (* with bucket_array^[local_index] *);
  end (* for *);
  for local_index := 1 to number_buckets - 1 do
    bucket_array^[local_index].end_PC :=
      bucket_array^[local_index + 1].start_PC - 1;
  bucket_array^[number_buckets].end_PC := program_end_PC;
end (* set_up_buckets *);
$PAGE trace
...
begin
  with parameter_area do begin
    ...
    session := 0;
    CRTCB_options := #h8000;  (* NEW TASK'S MONITOR IS SPECIFIED BY FIELDS THAT FOLLOW *)
    ...
    LOAD_options := #h400C; (* SEE IO & FILE MGMT ADDENDUM PG 3 *)
...
begin
  with parameter_area do begin
    taskname := sub_task_taskname;
    session := 0;
    EXMON_mask := #h10000000; (* ENABLE SINGLE STEP TRAP *)
  end (* with parameter_area *);
  trace ('EXMMSK', true);
...
begin
  ...
  with buffer do begin
    ...
    if single_step_flag then
      execution_options := #h1000; (* single step *)
  end (* with buffer *);
  trace ('REXMON', true);
...
begin
  ...
  readln (bucket_file, program_end_PC:8:h);
  readln (bucket_file, number_buckets);
  ...
  EXMMSK;
  START;
  TR_ND := TRUE; (* SET NOT DONE FLAG BEFORE CALL TO REXMON *)
  REXMON;
end (* initialize *);
$PAGE RSTATE
...
$PAGE update_array
procedure update_array (input_PC : four_bytes);

var
  low_bound : four_bytes;
  high_bound : four_bytes;
  curr_index : four_bytes;
  found_flag : boolean;

begin
  low_bound := 1;
  high_bound := number_buckets;
  found_flag := false;
  while (not found_flag) and
        (low_bound <= high_bound) do begin
    curr_index := (low_bound + high_bound) div 2;
    if input_PC < bucket_array^[curr_index].start_PC then
      high_bound := curr_index
    else
      if input_PC > bucket_array^[curr_index].end_PC then
	low_bound := curr_index + 1
      else
        found_flag := true;
  end (* while *);
  bucket_array^[curr_index].count := bucket_array^[curr_index].count + 1;
end (* update_array *);
$PAGE read_PC
procedure read_PC;
...
begin
  ...
  if local_PC < program_start_PC then
    unknown_count := unknown_count + 1
  else
    update_array (local_PC);
end (* read_PC *);
$PAGE print_results
...
begin
  writeln (result_file, 'Task = ', sub_task_filename);
  if single_step_flag then
    writeln (result_file, 'Single step increment = ', step_amount)
  else
    writeln (result_file, 'Delay increment = ', DELAY_amount:0,
             ' millisecond(s)');
  if trace_flag then
    writeln (result_file, 'Trace on')
  else
    writeln (result_file, 'Trace off');
  writeln (result_file, 'Starting PC = ', program_start_PC:8:h);
  writeln (result_file, 'Ending PC = ', program_end_PC:8:h);
  writeln (result_file, 'Number of buckets = ', number_buckets);
  writeln (result_file, 'Number of runtime calls = ', runtime_count);
  writeln (result_file, 'Number of unknown calls = ', unknown_count);
  writeln (result_file);
  for local_index := 1 to number_buckets do begin
    with bucket_array^[local_index] do begin
      writeln (result_file, local_index:8, module_name:10, ' ',
	       start_PC:8:h, ' ', end_PC:8:h, ' ', count:8);
    end (* with bucket_array^[local_index] *);
  end (* for *);
end (* print_results *);
$PAGE main
begin
  ...
  while flag do begin
    read_PC;
    if single_step_flag then be      for step_count := 1 to step_amount do begin
        while tr_nd and flag do ;
        tr_nd := true;
        if flag then REXMON;
        end;
    end
