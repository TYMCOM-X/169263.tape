...
var
  ...
  target_length, cleared_length, source_length: integer;
  defn1, defn2: def;
...
begin
  ...
  if aconstp (to_desc.arg[3], target_length) then begin
    ...
    if target_length <= from_desc.arg[3].offset then
      ...
      gen_c5 (movc5, from_desc.arg[3], from_desc.arg[1],
                     int_value (target_length), target_addr);
    free (target_addr)
  end
...
function gen_cst_set (set_byte: set_byte_array;
                      const_base, byte_count, first_byte: integer): set_desc;
...
begin
  ...
  for j := first_byte to byte_count do be    new (cr,setbyte);
    for bit := 0 to 7 do
...
public procedure prep_bitreference (element: addr_desc; var bit_position: addr_desc;
                                    min_val, max_val: integer; target_desc: set_desc;
                                    var defn: def);
...
public procedure emit_insv (bit_value: integer; source_desc: set_desc;
...
type
  mask_table = array [1..7] of integer;
const
...
begin
  with exp^ do begin
    ...
    if (second_op.in_temp and not first_op.in_temp) orif
       (in_context (second_op) and
          not in_context (first_op)) then begin
...
procedure perform_or (var second_op: set_desc; last_op: boolean);
  ...
  begin
    ...
    (* Second operand is not singleton. *)
      ...
      if not in_context (do_union) andif not second_op.in_temp and
         not (last_op and adr_equal (desired_loc, second_op.arg[1])) then
        make_temp_set (do_union)
      else if not do_union.in_temp andif not in_context (second_op) and
              not (last_op and adr_equal (desired_loc, do_union.arg[1])) then
        make_temp_set (second_op);

      if (second_op.in_temp and not do_union.in_temp) orif
         (in_context (second_op) andif
              not in_context (do_union)) then
        ...
        if upperbound (operand) = 0 (* null set *) then begin
          ...
          if (ctxt_lenbits = 0) orif (desc.set_length = 0) orif 
             ...
             (* and, in case the integer expression generation stuff detects
                a constant expression genset_op_bounds didn't ... *)
