...
public procedure pf_access ( parmkind: sym_kind; routine: tuple;
			     var reg: registers; allocated: boolean );
...
begin
  ...
  if (routine^.opcode <> cst_ref) and ( (routine^.opcode <> ident_ref) orif
                                              (routine^.id_sym^.kind <> consts) ) then begin
    if parmkind = values then
...
type
    dynamic_allocation = (bounded, flexible);
...
begin
  if parm.parm_type^.kind = strings then begin
  ...
  else if parm.parm_type^.kind = arrays then begin      (* might be flex *)
    ...
    case dest_allocation of
      ...
      flexible: begin
        case source_allocation of
          ...
          flexible: begin      (* pass upb as well as address *)
    ...
    if parm.parm_type^.kind = sets then begin
      ...
      pf_access (parm.parm_kind, exp, reg, short_form);
      rsize := 1;
    end
...
(* SCL FUNCTION CALL compiles code for a func_call_op "node" whose result   of size two words or less, e.g. scalars and reals. *)
