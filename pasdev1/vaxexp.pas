...
begin
  loc_tnode := tnode; (* local copy we can change *)
  while loc_tnode^.kind = indirect_type do
    loc_tnode := loc_tnode^.actual_type;
  case loc_tnode^.kind of
    ...
    scalars:
      unpacked_vax_type := packed_vax_type (loc_tnode);
    ...
    reals:
      if loc_tnode^.precision <= srealprec
  ...
  with exp^ do begin
    ...
    if not adr_equal(targ_addr,no_preference) and (targ_addr.byte_size=result_vtype)
      then begin
        ...
        store ( temp_addr ,_addr , alignment ( exp ) );
...
begin
  with exp^ do begin
	...
	if id_sym^.init_value.kind = subr_cst then
	  (* Only "address (procedure or function)" can cause us to be here - in all other contexts
	     subroutines are references by cst_ref's, not ident_ref's.  *)
	  ident_addr := def_addr (get_def (subr_def, id_sym^.init_value.blkp^.number))
	else begin
...
begin
  with field_ref_exp^ do begin
    ...
    if base_rec^.desc.base^.packable or field_sym^.type_desc^.packable
...
$PAGE calc_width
(* CALC WIDTH computes the size of a variable of a specified type with, 
   optionally, a specified upper bound parameter.  *)

public function calc_width (agg_type: typ; upb: addr_desc): addr_desc;
...
begin
  ...
  if a <> 0 then begin
    assert (not adr_equal (upb, no_preference));
    if a = 1 then
      calc_width := upb
    else begin
      gen3 (mull3, int_value (a), upb, r1_addr);
      free (upb);
      calc_width := r1_addr
    end;
    if b <> 0 then
      calc_width := add_constant (calc_width, b, r1_addr)
  end
  else begin
    assert (adr_equal (upb, no_preference));
    calc_width := int_value (b)
  end
end (* calc_width *);
$PAGE fetch_temporary
(* FETCH TEMPORARY returns a dscriptor for a static or dynamic temporary.
   The temporary is for a value of the type described by the type
   descriptor of the ALC_TEMP typle.  If there is also an upper bound 
...
begin
  if exp^.operand[1] = nil then
    upb := no_preference
  else
    upb := fetch_fullword (exp^.operand[1]);
  width := calc_width (exp^.desc.base, upb);
  fetch_temporary := get_temp (width, vax_long)
end (* fetch_temporary *);
$PAGE case_op
...
begin
  with exp^ do begin
    ...
    if (opcode = array_ref) andif
	 (base_array^.desc.base^.packable or base_array^.desc.base^.element_type^.packable)
