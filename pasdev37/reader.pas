...
external var
    ...
    nodecap, (* on if currently within at-sign string
		and $decap is on *)
    revis_flag, (* on if current line of text is an update to document *)
...
const
  tab = chr(#o11); (* this is the only non-
		     printing character passed through
		     by the reader *)
  eolchar = chr(#o15); (* character returned at end of line
...
$PAGE do_char_cmds -- processes ^ and | character commands
  ...
  (* This procedure processes character commands preceded by ^ or |.
    ...
    const
      valid_attrs: setofchar := ['U', 'A', 'B', 'O', 'D', 'C'];
      attr_chars: attr_vector := ('U','A', 'B', 'O', 'D', 'C');
...
(*    getlinenr(lineno); (* update line information *) *)
    ...
    cmd (outlen>0) andif (outline[1].value = '$')
  end (* not eof *)
