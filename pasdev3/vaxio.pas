...
begin
  with node^ do begin
	...
	with rw_item^.desc.base^ do begin
	    ...
	    check_input := (minval <> int_std_min) or (maxval <> int_std_max)
...
begin
  with node^.file_arg^ do begin
    ...
    (* For PUTSTRINGs we allocate a temp for the destination string
      ...
      if str_addr.type_desc.str_kind = nonvarying
        then temp_length := str_addr.max_len
      else temp_length := str_addr.max_len + (str_lw_width div byte_size);
      get_put_str_desc := str_temp( duplicate_addr(str_addr.len_addr) , temp_length );
	...
	store ( get_put_str_addr.len_addr , getput_addr_desc , unsigned_value );
	getput_addr_desc.offset := getput_addr_desc.offset - 4;
...
var
  ...
  (* FREE_BOTH is passed as a parameter to MOVE_STRING and is used
...
begin
  ...
  if getputstring and
    ...
    move_string ( dest_desc, get_put_str_desc, false, free_both, false, dummy );
  end
...
begin
  ...
  (* phe address of the file parameter. Dynamic temps used within
