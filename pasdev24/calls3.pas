...
CONST
    ...
    CORR_PER_RECD := 4064; (*TIME COLLECTION OVERHEAD IN EBOX UNITS*)
...
VAR
    ...
    CUR_CORR: 0..MAXIMUM(INTEGER); (* CURRENT STAT RECORD'S TIMER OVERHEAD
                                      CORRECTION AMOUNT *)
...
BEGIN
  REWRITE (TTY);
  WRITELN (TTY);
  WRITELN (TTY,'CALLS3 Version ',VERSION_NUMBER);
  BREAK;
  ...
  IF NOT EOF (STATFILE) THEN BEGIN
    ...
    REPEAT
      WITH STATFILE^ DO BEGIN
        IF TIMER > CUR_CORR THEN
          TIMER := TIMER - CUR_CORR
        ELSE TIMER := 0;
        CUR_CORR := CUR_CORR + CORR_PER_RECD;
