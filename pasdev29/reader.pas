...
static var 
  ...
  linecnt: array[1..8] of integer; (* saves current line count
...
begin
  ...
  current_state := [];
  for i := 1 to inf_depth do
    linecnt[i] := 0;	(* initialize the line counter variables *)
...
begin
  ...
  else lc:= linecnt[inf_depth] (* otherwise, just return count *)
end (*getlnr*);
...
(*    getlinenr(lineno); (* update line information *) *)
    linecnt[inf_depth]:= linecnt[inf_depth]+1;
    ...
    cmdflg:= (o>0) andif (outline[1].value = '$')
  end (* not eof *)
