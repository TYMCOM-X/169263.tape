...
public function get_extern (name: sym): def;
 var d: def;
 begin
...
begin
  if precision <= srealprec
    then new ( cr, realword )
    else new ( cr, doubleword );
  cr^.rvalue := rvalue;
  gen_emit ( area_list, cr );
end  (* proc gen_real *) ;
$PAGE gen_opcode
...
public procedure gen_string (var area_list: code_list; str: string);
  
  var cr: code;
  
  begin
    new (cr, stringword: length (str));
    cr^.stringlen := length (str);
    cr^.strvalue [1:length(str)] := str;
    gen_emit (area_list, cr)
  end;
  
  
  
public procedure gen_pstring (var area_list: code_list; str: string);
...
procedure gen_component (value: val_ptr; base_offset: bit_range;
 ...
 var
   ...
   cr_string: code;		(* Ptr for string code records *)
  ...
  with value^ do begin
    case kind of
      ...
      real_cst:
	    ...
	    new (cr, doubleword);
            cur_offset := cur_offset + 2*bits_per_real
          end
          else begin
            new (cr, realword);
	    cur_offset := cur_offset + bits_per_real
	  end;
	  cr^.rvalue := real_val;
	  gen_emit (area_list, cr)
        end;
	...
	if str_varying_ref then begin
	  new ( cr_string , stringword: str_len );
	  cr_string^.stringlen := str_len;
	  cr_string^.strvalue [ 1:str_len ] := substr ( str_val , 1 , str_len );
	  gen_emit ( area_list , cr_string );
	  cur_offset := cur_offset + str_lw_width + str_len * char_size
	end
	else begin
	  new ( cr_string, pstringword: str_len );
	  cr_string^.stringlen := str_len;
	  cr_string^.strvalue [ 1: str_len ] := Substr ( str_val,1,str_len );
	  gen_emit ( area_list, cr_string );
	  cur_offset := cur_offset + str_len * char_size
	end;

      set_cst:
        begin
          for i := 1 to (set_len + set_origin mod bits_per_byte + bits_per_byte - 1) div bits_per_byte do begin
            new (cr, setbyte);
            max_bit := min (bits_per_byte - 1, 
              set_len + set_origin mod bits_per_byte - ( (i - 1) * bits_per_byte) - 1);
  ...
  begin
	...
	scalar_cst:
	  if vals_type^.kind in [bools, chars] then
	    ...
	    if vals_type^.base_type^.size <= bits_per_byte then
	      bits_required := bits_per_byte
	    else if vals_type^.base_type^.size <= bits_per_word then
	      bits_required := bits_per_word
	    else
	      bits_required := bits_per_unit
	  end
	  else if (vals_type^.kind = ints) and vals_type^.packable then begin
	    if vals_type^.size <= bits_per_byte
	      then bits_required := bits_per_byte
	    else if vals_type^.size <= bits_per_word
	...
	real_cst:
	  if real_prec > srealprec 
	    ...
	    else bits_required := bits_per_real;

	string_cst:
	  begin
	    bits_required := str_len * char_size;
	    if str_varying_ref then bits_required := bits_required + str_lw_width
	  end;

	set_cst:
	  begin
	    bits_required := set_len + set_origin mod bits_per_byte;
	  end;
...
public function gen_cval (value: val; vals_type: typ): rel_syllable;
  ...
  begin
	...
	gen_cval := reldef (defval (valp^.def_addr))
      else gen_cval := reldef ( defval ( defp ) );
    end (* with *)
  end;
$PAGE gen_cst
