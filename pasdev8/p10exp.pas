...
type
  reg_set_vector = array [index_range] of set_of_registers;
...
type rs_list_type = array [index_range] of reg_state_ptr;	(* short allocated *)
...
procedure prepare_mem_ref (treg: reg_selector; exp: expr); forward;
...
function precision (exp: expr): bit_range;
 ...
 begin
  case exp^.desc.kind of
	...
	if flexible then
	  goto 100
	else if size <= 36 then
	  answer := 36
	else if size <= 72 then
	  answer := 72
	else
	  goto 100;
      end;
...
public function usage (e: expr): usage_cnt;
 var vdesc: val_desc;
 begin
  vdesc := valdesc (e^.result);
...
$PAGE reg_is_free
(* REG IS FREE returns a flag which indicates if a register ("reg") of suitable
   size ("regsize") is currently free.  This assumes that a register pair
   (regsize = 72) is never requested with reg = 17. *)

function reg_is_free (reg: registers; regsize: bit_range): boolean;
 begin
  with regdesc[reg]^ do begin
    if free and not locked then begin
      if regsize <= 36 then begin
	reg_is_free := true;
	return;				(* <---- successful return *)
      end;
      with regdesc[reg+1]^ do begin
	if free and not locked then begin
	  reg_is_free := true;
	  return;			(* <---- successful return *)
        end;
  ...
  reg_is_free := false;			(* <---- failing return *)
 end;
$PAGE lock, unlock
...
public procedure lock (reg: registers);
 begin
  ...
  if regdesc[reg]^.group_size = 2 then
...
public function get_temp (temp_size: unit_range): val_desc;
 begin
  ...
  with get_temp^ do begin
    next := vdesc_list;			(* chain to list of  descriptors *)
    prev := nil;
    if vdesc_list <> nil
...
public function get_value_desc (exp: expr): val_desc;
 begin
  if exp^.result <> nil then begin	(* alreadly has one *)
    get_value_desc := valdesc (exp^.result);
    return;
  end

  else begin				(* must create one *)
    if exp^.ref_fre <> 0 then begin	(* expression already processed once *)
      trace;
      stop;
    end;
    new (get_value_desc);
    ...
    exp^.result := exprptr (get_value_desc);
    return;
  end
 end;
$PAGE free
...
$PAGE detach
(* DETACH disassociates an index or base expression (the "loc_index") from the
...
(* ATTACH associates the value descriptor of an index or base expression ("aux")
   ...
   until the last reference, or until the association is broken by a call to "detach". *)
...
public procedure tag_loc (mloc: addr_desc; exp: expr; aux_index: expr);
 var vdesc: val_desc;
 begin
  vdesc := valdesc (exp^.result);		(* get its descriptor *)
  with vdesc^ do begin
    loc := mloc;				(* record the address *)
    if aux_index <> nil	(* remember index or base expression *)
      then attach (vdesc, valdesc (aux_index^.result));
  end (* with *) ;
 end;
$PAGE tag_reg
...
public procedure tag_rv (exp: expr; func: blk);
 const
    rv_reference: addr_desc := (fw, false, false, sb, 5, 0, 0, (absolute_sc));
    qrv_reference: addr_desc := (fw, false, false, sp, 0, 0, 0, (local_sc, nil));
 var vdesc: val_desc;
 begin
  vdesc := valdesc (exp^.result);
...
procedure rs_load ( rs: reg_state );
...
public procedure bb_end;
 ...
 begin
  ...
  scan := cur_bb^.downward_thread^.prev;
  ...
  jmp := cur_bb^.downward_thread^.prev;		(* scan list of successors *)
...
public function locate (exp: expr): addr_desc;
 var vdesc: val_desc;
 begin
  vdesc := valdesc (exp^.result);
  if vdesc = nil
...
type rmask_table = array[0..36] of integer;
 ...
 begin
  case mem.mode of
    ...
    byte:   begin
	      ...
	      if alignment in [left_aligned, left_unpadded]
	        then left_shift (reg, 36, 36 - mem.slice_size);
	    end;

    slice:  if alignment in [left_aligned, left_unpadded]
	      then begin (* load, shift, and mask *)
		gen_rm (move, reg, mem);
		left_shift (reg, 36 - mem.slice_offset, mem.slice_offset);
		if alignment = left_aligned then
		  left_mask (reg, 36, mem.slice_size);
	      end
	      else gen (ldb, reg, 0, 0, gen_bptr (mem))
  end;
 end;
$PAGE move_reg
...
public function do_inst
 ...
 var
   ...
   use1, use2: usage_cnt;		(* number of remaining references to the operands *)
 ...
 begin
  ...
  if (op1 <> op2) and (commutative in inst_options) then begin
    use1 := usage (op1);			(* get counts of remaining uses *)
    use2 := usage (op2);
    reverse := false;				(* default is to select op1 *)
	...
	else if (not is_register (loc1)) and is_register (loc2)
	  then reverse := true;
      end;
  ...
  if reg_is_free (r, regsize) then begin	(* put result in place of op1 *)
  ...
  else if (mem_form in inst_options) and ((r2 <> noreg) andif reg_is_free (r2, opsize)) then begin
...
public function prep_operand
 ...
 begin
  ...
  else if reg_is_free (reg, dstsize) then begin		(* checks for size, remaining uses *)
...
procedure rt_call ( treg: reg_selector; exp: expr; rts: rt_symbol );
 ...
 begin
  ...
  with exp^ do begin
    for i := 1 to nopers do	(* compile the subtrees *)
      prepare (anyreg, operand[i]);
    clr_rv;
    for i := 1 to nopers do begin	(* fetch the operands *)
      mem[i] := fetch_direct (operand[i], max (precision (operand[i]), 36));
      mem_lock (mem[i]);
    end;

    for i := 1 to nopers do begin	(* free the operands, before alloc of result reg *)
      mem_unlock (mem[i]);
      free (operand[i]);
    end;
    reg := get_reg (treg, p);
...
begin
  with exp^ do begin
	...
	then do_move_op (fltr, treg, exp, 36, false) (* integer -> single real *)
      ...
      reg := prep_operand (operand[1], treg, p1, pr);	(* move to reg of requisite size *)
      if p1 < pr			(* if single -> double *)
...
procedure do_setcvt_op (treg: reg_selector; exp: expr);
 ...
 var
   reg: registers;
   tregsize, opregsize, regsize: set_element_range;
   tlen, tlwb, oplen, oplwb: set_element_range;
 ...
 begin
  with exp^ do begin
    if operand[1]^.opcode = desc_ref		(* must convert long set *)
      ...
      if opregsize > tregsize then begin	(* 72 -> 36 *)
	if ((tlwb + tlen - 1) <= (oplwb + 36 - 1))	(* if extracting only from first word *)
      ...
      if opregsize < regsize		(* 36 -> 72, must extend *)
	then gen_ri (setz, reg+1, 0);
...
function set_check ( opreg: registers; op: expr; lwb, upb: set_element_range ): boolean;
...
function set_ce
	...
	cstlwb: set_element_range;	(* limits of 1 bits to turn on *)
	exp: expr;
	cstoffset: int_type
...
begin
  constant_set := false;
  with cexp^ do begin
    if (opcode = gen_set_op) andif ((nopers = 0) orif (iconstp (operand[1], lwb)
      andif ((nopers = 1) orif iconstp (operand[2], upb)))) then begin
	constant_set := true;
	if nopers > 0 then begin
	  if nopers = 1 then upb := lwb;	(* single element *)
	...
	begin
	  ...
	  if exp^.copy_tuple then begin
	    ...
	    with vdesc^ do begin
	      ...
	      tag_reg (vreg, exp);
	      if loc_index <> nil then detach (vdesc);	(* mark the value as unstored *)
	      loc := null_location;
	    end;
	  end;
	end;
  ...
  with exp^ do begin
	...
	begin
	  if nlevels = 0 then begin	(* referencing current stack frame *)
	    ...
	    mem.index := sp;
	    tag_loc (mem, exp, nil);	(* there is no need to reload sp, thus nil *)
	  end

	  else if nlevels = 1 then begin	(* referencing first parent *)
	    mem := absolute_reference;	(* 1(16) rhw *)
	    mem.mode := rhw;
	    mem.offset := 1;
	    mem.index := sp;
	    tag_loc (mem, exp, nil);
	    ...
	    reg := get_reg (treg, 36);
	    gen (hrrz, reg, sp, 1, none);
	    for off := 2 to nlevels do gen (hrrz, reg, reg, 1, none);
	    tag_reg (reg, exp);
	  end;
	end;
	...
	begin
      ...
      gen_set_op:
	begin
	  if nopers = 0 then begin	(* [], clear the register *)
	    reg := get_reg (treg, desc.set_length);
	    clear (reg);
	    tag_reg (reg, exp);
	  end

	  else if nopers = 1	(* [e], add one element to the set *)
	    then gen_singleton_set (treg, exp)

	  else (* if nopers = 2 then *)
	...
	begin
	  prepare (anyreg, operand[1]);
	  if nopers = 2 then prepare (anyreg, operand[2]);
	  reg := dload (treg, operand[1], 36);
	  if nopers = 2 then begin		(* move display to lhw *)
	    lock (reg);
	    mem := fetch (operand[2], 18);
	    free (operand[2]);
	    unlock (reg);
	    gen_rm (hrl, reg, mem);
	  end;
	...
	begin
	  reg := get_reg (treg, 36);
	  gen (calli, reg, 0, 23b, none); (* MSTIME UUO *)
	  tag_reg (reg, exp);
	end;
	...
	begin
	  reg := get_reg (treg, 36);
	  gen (calli, reg, 0, 30b, none); (* PJOB UUO *)
	  gen (calli, reg, 0, 27b, none); (* RUNTIM UUO *)
	  tag_reg (reg, exp);
	end;
 ...
 begin
  ...
  if (exp^.opcode = mem_ref) andif (exp^.item.index <> nil) then begin
	...
	if vdesc^.loc_index <> nil then
	  detach (vdesc); (* dissociate fromm index register *)
...
public function prep_set_addr (treg: registers; exp: expr): addr_desc;
...
public function load (* treg: reg_selector; exp: expr; p: bit_range): registers *) ;
 ...
 begin
  ...
  maddr := locate (exp);
  if maddr = null_location then begin
    trace;
    stop;
  end;
