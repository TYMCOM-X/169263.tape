...
function const_comparison (c1: integer; cmp: cmp_codes; c2: integer): boolean;
  ...
  case test^.opcode of
    ...
    in_op: begin
      ...
      if (maxval - set_lwb) >= set_size_limit then
        set_len := set_size_limit (* if max_val = #O377777777777 and lwb = 0 adding 1 goes neg. *)
      else
  ...
  (* If this is not a value, then perform the indicated comparision. *)
    ...
    case test^.opcode of
      ...
      in_op: begin
        ...
        set_lwb := max (0, minval);
        if (maxval - set_lwb) >= set_size_limit then
          set_len := set_size_limit (* if maxval = #O377777777777 & lwb = 0 adding 1 goes neg. *)
        else
