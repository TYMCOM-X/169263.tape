...
begin
  ...
  while m <> nil do begin
    if m^.modid > 0 then begin		(* for all non-sharable overlays *)
      ...
      putstring (str, '$.A', (m^.marea^.aorig div #o1000):3:O);
  ...
  while a <> nil do begin		(* with names, of course *)
    ...
    putstring (str, '$.A', (a^.aorig div #o1000):3:O);
    do_label (str);
  ...
  do_label ('$.F000');
  hwords (#o400000,0);			(* database, no buffer *)
...
begin
  ...
  if not eof (f) then
	...
	repeat
	  ...
	  alloc ( ((a^.asize + #o777) div #o1000) * #o1000 );
	  a := a^.next
	  until a = nil
	end;
...
begin
  ...
  if not eof (f) then 
    ...
    else writeln (f, name^.text, '/SAVE/G');	(* non-shar for others *)
    ...
    if not setupcode (name^.text,assmfile) then	(* can we do it? *)
      ...
      if marea <> nil then begin	(* if non-sharable then *)
	extern ( 'MOE.TV' );		(*    MOE for errors *)
      end
      else begin			(* if sharable then *)
