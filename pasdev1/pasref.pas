...
(*   +--------------------------------------------------------------+
     ...
     CHANGES:
        ...
        7/28/78  RNF    modified   most   internal   subroutines   in
                        accordance with CIN-#2.
...
begin
  with subscripted_ref^ do begin
    ...
    if check_lwb or check_upb then begin
      ...
      if check_lwb then
	lwb_expr := arr_bound (lwb_op, base_array)
      else
	lwb_expr := nil;
 
      if check_upb then
	upb_expr := arr_bound (upb_op, base_array)
      else
	upb_expr := nil;
...
var
    ...
    index: integer; (* Used for constant subscript checking. *)
  ...
  with op^ do
    case opcode of
	  ...
	  if operand[1]^.desc.kind = strings then
	    len1 := operand[1]^.desc.str_length
	  else
	    len1 := 1; (* must be char *)
	  if operand[2]^.desc.kind = strings then
	    len2 := operand[2]^.desc.str_length
	  else
	    len2 := 1;
          str_length := min ( maximum (char_range), len1 + len2);
	  if ( (operand[1]^.desc.kind = strings) andif
	       ((operand[1]^.desc.str_kind = varying) or operand[1]^.desc.str_flex) ) or
	     ( (operand[2]^.desc.kind = strings) andif
	       ((operand[2]^.desc.str_kind = varying) or operand[2]^.desc.str_flex) ) then
        ...
        with operand[1]^.desc do
          case kind of
            ...
            ints:  begin
              ...
              desc.int_prec := operand[1]^.desc.int_prec + 1;
              opcode := ineg_op;
            end;
            reals:  begin
