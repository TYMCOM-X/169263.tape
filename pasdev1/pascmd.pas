...
type error_codes =
	 ...
	 bad_length, bad_ext_unique_length, bad_alloc, opt_expected, paren_expected, val_expected,
...
begin
  ...
  case error of
    ...
    bad_ext_unique_length:
      writeln (tty, 'Illegal length for external symbol uniqueness.');
    bad_alloc:
...
begin
  with prog_options do begin
	...
	cmd_error (opt_expected);
      ...
      if ( not positive and not (option_ix in opdnoo_no_options) )	 ( automatic and not (option_ix in opdauo_auto_options) )
	...
	opt_ext_unique_length:
	  if not positive then
	    external_unique_length := 0
	  else begin
	    val := numeric_value (-1);
	    if (val < 1) or (val > 255) then cmd_error (bad_ext_unique_length);
	    external_unique_length := val
	  end;
