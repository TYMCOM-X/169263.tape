...
type
  ...
  stack_level = 0..#O177777;                     (* stack frame numbers *)
  ovl_mod_no = 0..#O377777;                      (* overlay module number *)
  int_type_ptr = ^machine_word;     (* for coercing addresses to be pointers to
                                   machine words *)
  radix_type = (octal_radix, decimal_radix, hex_radix);
                                                (* supported radices *)

const
  radix_label: array [radix_type] of string[8] :=
		('octal', 'decimal', 'hex');
...
type
  ...
  descriptor = packed record
    ...
    case dkind: desc_kinds of
      ...
      slice_dt,
      array_dt:
        (  lower_bnd: machine_word;                 (* lower bound of array ( not available from
                                                   index type node if flexible or generic) *)
           upper_bnd: machine_word)                 (* upper bound of array or upper element
                                                   of slice range *)
  end;
...
const
  nil_ovl_modno := #O200000;                     (* indicates overlay module number in brkpt
                                                   table is unknown *)
...
type
  ...
  brkpt_entry = packed record                   (* breakpoint table entry *)
    in_core: boolean;                           (* true if always resident or overlay module
                                                   which is in core *)
    mod_no: ovl_mod_no;                         (* overlay module number, #O377777 if unknown
                                                   or not an overlay *)
...
type
  stack_frame = packed record
    ...
    caller_top: ^stack_frame;                   (* #O17(RH) on entry to proc *)
...
type
  reason_code = ( (* describes reason debug called *)
    ...
    trap,  (* Debugger entered (via DDT) after trap, e.g., ill
              mem ref. Continuation of execution impossible.    rterr, (* Debugger entered (via DDT) after error detected
  ...
  rt_static_record = record
    ...
    step_count: machine_word;                               (* STEP and SSTEP cmds stmt count *)
    proc_skip: boolean;                         (* true => skip procs when stepping *)
    brk_skip: machine_word;                         (* breakpoint proceed count *)
    ...
    regsave: array [1..#O16] of machine_word;            (* reg save area when debugger entered *)
