...
const
  SECKEYWDS: SYMSET :=
    [BEGINSY, TYPESY, CONSTSY, VARSY, LABELSY, CLASSY, EXTERNALSY,
     PROCSY, FUNCTIONSY];

  NONSTMT: SYMSET :=			(* simple statememt terminators *)
    [BEGINSY, ITERSY, SEMISY, FORSY, W_DOSY, ENDSY,
     DOSY, IFSY, THENSY, ELSESY, CASESY, EXITSY, UNTILSY];
...
(* FOR_COM is the bottom of the chain of routines used in INDFOR to
   ...
   FOR_COM will not return to its caller until THIS_SYM is not a
...
$PAGE FOR_COM 	jams comments through format
  begin					(* code for FOR_COM *)
	...
	LIN_INDENT := OUT_IND;		(* save indent for this line *)
      ...
      if not (THIS_SYM in [SA_COM,DIRECTIVE]) then
	...
	DIRECTIVE: begin
	  ...
	  resume (WRITER);
	  if not (THIS_SYM in [SA_COM,DIRECTIVE]) then return
	  end (* DIRECTIVE *);
	...
	SA_COM: begin
	  ...
	  until THIS_SYM <> SA_COM;
	if THIS_SYM <> DIRECTIVE then return
	end (* SA_COM *);
...
procedure GO_PAST( SKIP_TOKEN: SYMBOLS;
		   FORMAL: COROUTINE);
...
(* GO_PAST calls the formal parameter routine (either FOR_NO_REL
   ...
   and then performs one more formal call. THIS_SYM is thus set
   to the token following the SKIP_TOKEN. *)

  begin
  while THIS_SYM <> SKIP_TOKEN do
    FORMAL;
  FORMAL
...
(* FOR_TYPE performs indentation for a type statement, i.e. what one
  ...
  ORIGL := CUR_IND;			(* save current level on stack *)
	...
	LPARENSY:
	  ...
	  GO_PAST(RPARENSY,FOR_SAV_REL)
	  end (* LPARENSY *);

	ENDSY,
	ASSIGNSY,			(* may be var or typed const *)
	RPARENSY,
	SEMISY: return;		(* let caller deal with final token *)

	RECORDSY:
	  begin
	  if 3 in C_OPTIONS then		(* get offset from current *)
	...
	PROCSY,
	FUNCTIONSY: FOR_PRO_DECL;		(* routine type definition *)
...
(* FOR_REC_BODY formats the body of a record declaration, i.e. everything
  ...
  while not (THIS_SYM in SECKEYWDS + [ENDSY,RPARENSY,CASESY]) do
  ...
  if THIS_SYM = CASESY then
	...
	GO_PAST(LPARENSY, FOR_NO_REL); (* skip junk if any *)
	...
	GO_PAST(RPARENSY,FOR_NO_REL); (* match the parens *)

	if THIS_SYM = SEMISY then
	  FOR_NO_REL;			(* eat sems if any *)
	CUR_IND := LASTVAR		(* back to level of last variant *)
      until THIS_SYM in SECKEYWDS + [ENDSY,RPARENSY];
...
(* FOR_TYPE DECL processes a section beginning with the keyword
  ...
  while not (THIS_SYM in SECKEYWDS) do
...
(* FOR_VAR_DECL deals with a var declaration section in much the same
  ...
  while not (THIS_SYM in SECKEYWDS) do
    ...
    while THIS_SYM <> COLONSY do	(* id list goes on min ind. *)
    ...
    while not (THIS_SYM in SECKEYWDS + [SEMISY]) do
...
(* FOR_CONST is a lot like FOR_VAR_DECL, except that const decls. don't
  ...
  while not (THIS_SYM in SECKEYWDS) do
    ...
    while not (THIS_SYM in SECKEYWDS +
	       [COLONSY,ASSIGNSY,EQUALSY,SEMISY]) do
      FOR_NO_REL;			(* get to type or value(if there) *)

    if THIS_SYM = COLONSY then
      begin
...
$PAGE FOR_OPTIONS formats block option list
procedure FOR_OPTIONS;

(* FOR_OPTIONS formats the OPTIONS list following a block header.
   Just indent a quantum and preserve relative until the end.  But
   instead of hunting a semicolon, just kick out if something unacc-
   eptable shows up. *)

  begin
  CUR_IND := CUR_IND + QUANTUM;
  FOR_NO_REL;				(* option keyword *)
  while THIS_SYM in
    [ETC, COMMASY, LPARENSY, NUMERIC, RPARENSY] do
    FOR_SAV_REL;
  CUR_IND := CUR_IND - QUANTUM
  end (* procedure FOR_OPTIONS *);
...
(* FOR_PRO_DECL formats procedure and/or function headings both as actual
  ...
  if THIS_SYM = LPARENSY then
    ...
    while not (THIS_SYM in SECKEYWDS + [RPARENSY, OPTIONSY]
		- [VARSY,PROCSY,FUNCTIONSY]) do
      begin				(* process rest of par list *)
      if THIS_SYM in [ETC, VARSY] then (* the normal things in proc heads *)
  ...
  CUR_IND := ORIGL;			(* restore *)
  if THIS_SYM = OPTIONSY then
    FOR_OPTIONS			(* do option list if any *)
...
(* FOR_BODY processes a body of code from 'begin' to 'end', including
   ...
   until an appropriate place mars found.  *)
...
label 2,3;
  ...
  while not (THIS_SYM in SECKEYWDS - [BEGINSY]) do
    begin				(* for safety, kick out if new sect. *)
    if STMT and (THIS_SYM in NONSTMT) then
	...
	if (not NOBEGIN) or 
	   (FIRST_ON_LINE and (5 in C_OPTIONS)) then
	  begin					(* begin is CSH *)
	...
	FOR_NO_REL;
	if 2 in C_OPTIONS then		(* tick a q now, put on stack *)
	  CUR_IND := CUR_IND + QUANTUM; (* to line terminator correctly *)
	MARK (MANY_FLAG);		(* record ind. for terminator *)

	if not (2 in C_OPTIONS) then	(* indent(2) means we already ticked *)
	...
	P_LEVEL := 0
      ...
      ONSY, FORSY, W_DOSY: begin		(* all take a 'do' *)
	NOBEGIN := true;
	if not FIRST_ON_LINE then
	  NEW_LEVEL (ONE_FLAG)
	...
	if FIRST_ON_LINE and not
	  (2 in C_OPTIONS) then		(* deindent if first, and not IND(2)*)
	  CUR_IND := CUR_IND - QUANTUM;
	...
	while not (THIS_SYM in [DOSY,SEMISY,ENDSY,UNTILSY]) do
	  FOR_SAV_REL;			(* get to terminator *)
	if THIS_SYM = DOSY then
	...
	while THIS_SYM <> THENSY do	(* get that then *)
	  FOR_SAV_REL;
	...
	if FIRST_ON_LINE then		(* only matters if first *)
	  ...
	  else if 2 in C_OPTIONS then	(* make it look like indented...*)
	...
	if NEXT_SYM = IFSY then
	  goto 3;			(* level all set, go to it. *)

	MARK (ONE_FLAG);		(* record indent of 'if' for semi *)
	NOBEGIN := true;
	if 1 in C_OPTIONS then		(* we should back off to the 'then' *)
	  CUR_IND := KWLEVEL;		(* which we saved above *)

	FOR_NO_REL;
	if (THIS_SYM <> BEGINSY)
	or (2 in C_OPTIONS) then		(* decide whether to indent next *)
	  CUR_IND := CUR_IND + QUANTUM
      end (* ELSE *);
	...
	while not (THIS_SYM in [ENDSY,ELSESY,SEMISY,UNTILSY]) do
...
var
  ...
  detach;				(* end initializations *)
	...
	COMPILSY: begin
	  FOR_NO_REL;			(* Program, Module, or Datamodule *)
	  while not (THIS_SYM in [OPTIONSY,SEMISY]) do
	    FOR_SAV_REL;
	  if THIS_SYM = OPTIONSY then
	    FOR_OPTIONS			(* options not required *)
	  end (* COMPILSY *);
