...
public var
  ...
  THIS_SYM,				(* current token type *)
  NEXT_SYM: SYMBOLS;			(* lookahead--may be comment *)
  ...
  C_OPTIONS: OPTIONSET;			(* current options in effect *)
...
external function SYM_LOOKUP (COM_LINE): TOKENTYPE;
...
$PAGE DO_INT simple routine to return integer value of input text
  function DO_INT: LINE_IDX;
    ...
    begin
    GETSTRING (substr (USRLINE, LPTR), DO_INT);
...
$PAGE DO_RES_SYM process a list of BEFORE/AFTER options
  ...
  var
	...
	else 				(* must be special symbol *)
	  if length(TMPSTR) = 1 then	(* lookup in single char array *)
	    TMPTOK := SYM_TABLE[ TMPSTR[1] ]
	  else TMPTOK := SYM_LOOKUP (TMPSTR);

	if TMPTOK = NO_GOOD then	(* all failed, no good *)
	  ERROR ('Unrecognized reserved word or symbol -- "'||TMPSTR||'"');

	TOKS := TOKS + [TMPTOK]		(* slap it in *)
	end (* got something *);
...
$PAGE DO_IND_OPT to process INDENT(1,2,3...) option list
  ...
  (* DO_IND_OPT processes numbers  in a list following the keyword INDENT,
     and, if acceptable, places them into the set C_OPTIONS. *)

  var
    TMP: LINE_IDX;

    begin
    C_OPTIONS := [];			(* in case we find nothing *)
    while (LPTR <= length(USRLINE)) andif
      (USRLINE[LPTR] <> ')' ) do
      begin
      TMP := DO_INT;			(* try to get a number *)
      if (TMP > 0) and (TMP <= MAX_OPT) then
	C_OPTIONS := C_OPTIONS + [TMP]
  ...
  var
	...
	case WHAT of
	  ...
	  BEF_TOK:			(* list of 'BEFORE' spacings *)
	    DO_RES_SYM (TOK_BEFORES);

	  AFT_TOK:			(* and of 'AFTER' spacings *)
	    DO_RES_SYM (TOK_AFTERS);
	...
	if length(USRLINE) <> LPTR then
	  ER'Extraneous characters follow option list');
