...
begin
  ...
  if not data_type^.flexible then begin
    ...
    a := 0;
    b := (data_type^.size + byte_size - 1) div byte_size;
    c := 1;
  end
	...
	if alloc_size
      ...
      variants:
	begin
	  const_part := ngm (data_type^.size,
			     allocation_tables[records,no_packing]^.width_factor);
	  if const_part mod byte_size <> 0 then begin
	    trace;
	    stop;
	  end;
    ...
    if (elem_size >= efw) or (efw mod elem_size = 0) or (efa < efw) then begin
	  ...
	  then Size = (X * ElemSize) + BaseSize.  If an element occupies an
	  integral number of bytes, then the bits-to-bytes conversions can
	  be incorporated directly into the A and B constants.  Otherwise,
	  a division by ByteSize will be necessary.  *)

      if elem_size mod byte_size = 0 then begin
	a := elem_size div byte_size;
	b := (const_part - elem_size * adjustment) div byte_size;
	c := 1;
      end
      else begin
	a := elem_size;
	...
	b := ((const_part - elem_size * adjustment) div byte_size) * c + c - 1;
      end
      else begin
...
procedure map_array
      ( elem_type: typ;                 (* type of elements of array *)
        packing: packing_context;       (* either minimal or field packing *)
...
procedure map_record ( rec_type: typ; packing: packing_context );
...
(*  MAP REC VARIANT is the working routine for MapRecord.  It performs the
    ...
    which allocation for fief this particular variant should begin (for
...
begin
  tag^.size := ro;                                (* tag size is minimum offset so far *)
  ...
  while variant <> nil do begin
    ...
    max_width := max (max_width, variant^.size);        (* get max size of any variant *)
...
var trial_context: packing_context;
...
var
  field, last_field, next_field: sym;
...
begin
  ...
  (*  Set the size of the entire record or variant, and return the offset of
...
begin
  ...
  with subr_type^ do begin
    ...
    if return_type <> nil then begin
      if p_b_address (return_type) then begin
	rv_loc := rv_addr_loc;
    ...
    if rv_loc = rv_at_start then
      ploffset := ngm (ploffset, rv_align) + rv_size;
...
static var
    packing_choice: array [boolean] of packing_context :=
  ...
  with type_desc^ do begin
    ...
    case kind of
	  ...
	  if minval >= 0 then
	    ...
	    size := max (int_bits (minval), 1 + int_bits (maxval));
        end;

      reals:
	size := real_base_size [precision];
	...
	if (element_type <> nil) and (index_type <> nil) then begin
	  ...
	  if flexible
	    then size := 0 (* don't count the hidden size word *)
	    else size := twidth; (* computed array size *)
	end;
