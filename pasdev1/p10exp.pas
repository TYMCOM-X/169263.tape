...
function select_single_reg_for_free (treg: reg_selector): registers;
 ...
 begin
  ...
  r := 2;
  while r <= #O14 do begin               (* examine each group of registers *)
    with regdesc[r]^ do begin
...
function select_double_reg_for_free (treg: reg_selector): registers;
 ...
 begin
  ...
  r := 2;
  while r <= #O14 do begin               (* scan the register groups *)
    with regdesc[r]^ do begin
...
public function get_reg (treg: reg_selector; p: bit_range): registers;
 ...
 begin
  ...
  if p <= 36
    then begin
      reg := select_single_reg_for (treg);
      clr_reg (reg);
    end
    else begin
...
public procedure bb_start;
 ...
 begin
  ...
  if (cur_bb^.inward_jumps <> nil) andif
    ...
    then kill_regs (regs_allocated * [0..#O14]); (* don't kill the sp *)
...
procedure left_shift (reg: registers; p: bit_range; n: integer);
 begin
  if n <> 0 then begin
    if p <= 36
...
procedure get_bounds (exp: expr; var minval, maxval: integer);
 var base: typ;
 begin
...
procedure half_set (lwb, upb: integer; reg: registers);
...
var
    ...
    i, top: integer;   (* top will be number of non-constant operands *)
...
begin
  with cset do if value <> 0 then begin
    ...
    else if lh = #O777777 then gen_ri (orcmi, reg, - (value + 1))
...
begin
  with cset do begin
    ...
    else if lh = #O777777 then gen_ri (hrroi, reg, rh)
    else if rh = 0 then gen_ri (hrlzi, reg, lh)
    else if rh = #O777777 then gen_ri (hrloi, reg, lh)
        ...
        begin
          if item.base <> nil then begin                (* must offset byte pointer *)
            ...
            if item.index <> nil then begin     (* create byte pointer at run time *)
              ...
              gen_ri (hrli, bpreg, (p*#O10000) + (item.size*#O100)); (* move in p and s *)
              tag_reg (bpreg, exp);
            end
