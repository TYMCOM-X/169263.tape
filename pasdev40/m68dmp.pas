...
$PAGE page_skip
(* PAGE_SKIP - does a page skip on the output file. *)
...
$PAGE read_one_byte
(* READ_ONE_BYTE - gets the next byte from input_record, as indicated by
                   byte_ptr.  If necessary, it reads a new num_bytes long
                   input_record and resets byte_ptr to 1. *)
...
begin
  ...
  local_char_number := 256;
  repeat
    if local_char_number = 256 then begin
    ...
    local_char := '.';
    if (local_one_byte >= ord (' ')) and
       (local_one_byte <= ord ('}')) then
...
begin
  ...
  write_string ('           Flag byte = $' || 
		...
		convert_decimal (local_offset_length, 0) ||
                ' byte(s)]');
  for local_counter := 1 to local_SDID do begin
    read_one_byte (local_one_byte);
    check_eof_flag;
    decrement_check_byte_count (1);
    write_string ('           ESDID = $' || 
...
begin
  writeln (tty, 'M68000 Object File Analyzer, Version 1.0');
...
$ENDIF
$IF M68
    write_string ('M68000 Object File Analysis for File: ' ||
                  filename (input_file));
    write_string ('');
$ENDIF
  end;
