 ...
 begin
  ...
  with subr_formal_type^ do begin
    ...
    if parms_required then begin
      parm_mode := [flexible_ok];
      parmlist.first := nil;
      parmlist.last := nil;
    end
    else
      parm_mode := [flexible_ok, forward_ok];
 ...
 begin
  ...
  with make_array_type^ do begin
    ...
    if flex_ok           (* process index type, guarenteed to be range, or type id *)
      then begin
        index_type := type_semantics (node, [flexible_ok]);
        if index_type <> nil then         (* if index type is flexible, mark array as such *)
...
begin
  ...
  if (fl <> nil) andif (fl^.defn <> nil) then begin                       (* must be <variant part>, if non-nil *)
    ...
    while var_case <> nil do begin
      ...
      elsen                                (* process list of labels in <range list> *)
        range := lab^.defn;
...
(* TYPE SEMANTICS semanticates a parse tree describing a type.  At this level we
   ...
   we check that forward and flexible types are used in permissable contexts. *)
...
public function type_semantics (* typetree: parse_node; flags: tsm_flags ): typ *) ;
 ...
 begin
  ...
  (* perform checks for flexible and forward types *)
