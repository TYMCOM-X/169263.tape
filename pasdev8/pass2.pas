  ...
  while t <> nil do begin
	...
	label_node:
	  begin
	    ...
	    if (t_label^.next^.opcode = gen_andif_op) or
...
$PAGE next_pass
(* NEXT_PASS saves the heap and, if the AUTO_RUN flag is set, initiates
...
begin
  ...
  if auto_run <> 0 then begin
    if opts_listing or (err_count <> 0) then
      ...
      next := tmprefix || 'OCG';
      del_file (tempname ('ERR'));
      del_file (tempname ('XRF'));
    end;
    run (next || prgm_dir (), 2);
...
begin
  ...
  if finish then begin
    elf_open;
    ch_open (true, true);
    abort := create (abt_routine, abort_stack_size);
    if switch (prog_options.dump_switches, 'CGRAPH') then
      dmpcgraph; (* dump the call graph *)
  ...
  if prog_options.statistics then begin
    seginfo (segstuff);
    rewrite (tty);
    writeln (tty, '[Pass 2: ', (runtime - start_time) / 1000:8:3, ' seconds, ',
