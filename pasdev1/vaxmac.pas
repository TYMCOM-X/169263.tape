...
public function cvhex (value: integer): cvhex_string;
    ...
    case kind of
      ...
      indirect_word: with pub_sym^ do begin
        ...
        if kind = vars then begin
          ...
          fio_write ( listfb , ' K.' );
          fio_write ( listfb , cv_int ( def (init_value.defp)^.defnumber         end
        else begin
...
public procedure mac_header;
  ...
  begin
    if list_file = '' then return; (* <---- no listing file *)
     ...
     fio_line (listfb, 'VAX Checkout Code Generator,  ' ||
  ...
  begin
    ...
    if not prog_options.overlay_opt then (* overlay handled indirect_word  in routine list in vaxmac *)
      while symbol <> nil do
	    ...
	    if not printed_first_public then begin
	      printed_first_public := true;
	      fio_tab (listfb, 9); fio_line (listfb, ';');
