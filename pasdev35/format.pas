...
$ENDIF
$IF vax
$INCLUDE dtime.vax[31024,320157]
$INCLUDE pasdir.typ[31024,320157]
$INCLUDE run.inc[31024,332214]
external function jobnum: string[8];
$ENDIF
$IFNONE (adp,tymshare,vax)
 One of the system switches must be enabled to compile this module;
  legal switches are: 'ADP', 'TYMSHARE', and 'VAX'.
$ENDIF
...
$IFANY (tymshare,adp)
      device = 'DSK:';
      f_name_delim1 = ':';             (* To strip device and ppn from filename for printing *)
      f_name_delim2 = '[';
      queue_program = 'sys:queue';      (* Name of print queue program on the 10 *)
      max_digits = 3;            (* To create filenames for the queue
                                           program.  Format of the filenames
                                           is substr(jobnum, max_digits + 1)
                                           || '###.###' *)
$ENDIF
$IF vax
      device = 'DRA1:';
      f_name_delim1 = ']';
      f_name_delim2 = ';';
      queue_program = 'dra1:[250245.format]queue';
      max_digits = 5;
$ENDIF
...
var cmd: integer;				(* command to dispatch on *)
...
type
  cmdnames = array[listcmd..noechocmd] of cmd_lookup_record;
...
const
   cmdnamelist: cmdnames :=
	...
	( 'EDIT',	4, editcmd ),
	( 'NEW',	3, newcmd ),
	( 'UPPERCASE',	5, uppercmd ),
	( 'LOWERCASE',	5, lowercmd ),
	( 'PASS',	4, passcmd ),
	( 'COLUMN',	3, columncmd ),
	( 'LENGTH',	3, lengthcmd ),
	( 'QUIT',	4, quitcmd ),
	( 'DECAP',	1, decapcmd ),
	( 'NODECAP',	3, nodecapcmd ),
	( 'XREF',	1, xrefcmd ),
	( 'NOXREF',	3, noxrefcmd ),
	( 'APPEND',	1, appendcmd ),
	( 'NOAPPEND',   3, noappendcmd ),
	( 'QUEUE',	3, queueopt ),
	( 'HEADER',	4, headercmd ),
	( 'NOHEADER',	3, noheadcmd ),
	( 'OPTION',	3, optioncmd ),
	( 'NOOPTION',	3, nooptcmd ),
	( 'DEFAULT',	3, defcmd ),
	( 'INCLUDE',	3, includecmd ),
	( 'NOINCLUDE',	3, noincludecmd ),
	( 'ECHO',	3, echocmd ),
	( 'NOECHO',	3, noechocmd )  );
...
$PAGE write_stars
procedure write_stars (filler, num_stars: integer);
...
procedure list_header;
 ...
 begin
  ...
  fname := substr (f_name, index (f_name, f_name_delim1) + 1);    (* strip dev and ppn *)
  fname := substr (fname, 1, index (fname, f_name_delim2) - 1);

  title_line1 := 'File ' || f_name;
  title_line2 := day_name [day_of_week (extr_date (file_desc.creation))] ||
		 ' ' || substr (dc_ext (file_desc.creation), 1, 15) ||
		 '   MDSI, Company Confidential   FORMAT Version 1.10  ';
  fill1 := (page_length - length e_line1)) div 2;
  fill2 := (page_length - length (title_line2)) div 2;

  for i := 1 to 3 do begin
    write (output,' ':fill2);
    writeln (output, title_line2);
  end;
  for i := 1 to 8 do writeln (output);

  write_stars (fill1, length (title_line1));
  write (output,' ':fill1);
  writeln (output, title_line1);
  write_stars (fill1, length (title_line1));
 ...
 begin
  ...
  if lindex <= length (line) then begin	(*  a '/' was found *)
    ...
    while lindex <= length (line) do begin      (* loop over options *)
      if not cmd_lookup(line, lindex, token_chars, cmdnamelist, cmd)
	then begin
...
begin
  ...
  writeln (tty, 'FORMAT, Version 1.10');
  title_heading := '                             ';
  ...
  if auto_run then begin
    if length (jobnum) > max_digits     (* if jobnum is too long to include in the fname
                                           delete the leftmost digits *)
      then reset (cmdfile, substr( jobnum, length(jobnum) - max_digits + 1) || 'FMT.TMP')
      else reset (cmdfile, jobnum || 'FMT.TMP');
...
150 (* next command *):
  ...
  exit if quit_flag;
    ...
    if uppercase (curoptions.fid_qualifier) = 'LPT:' then begin (* queue result to printer *)
      if length (jobnum) > max_digits  (*If jobnum is too long, to include in the fname 
                                         delete the leftmost digits. *)
        then curoptions.fid_qualifier := device || substr(jobnum,length(jobnum)-max_digits + 1) || 
                                         'FMT.LST'
        else curoptions.fid_qualifier := device || jobnum || 'FMT.LST';
...
100 (* EOF abort *) :
  ...
  if file_queued then begin			(* queue temp list file to printer *)
    if length (jobnum) > max_digits (*If jobnum is too long to include in the fname
                                      delete the leftmost digits *)
      then rewrite (output, substr (jobnum, length(jobnum)-max_digits + 1) || 'QUE.TMP')
      else rewrite (output, jobnum || 'QUE.TMP');
    writeln (output, 'lpt:=', queue_file, queue_options);
    close (output);
    run (queue_program, true);
