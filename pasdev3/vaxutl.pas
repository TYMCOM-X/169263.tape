...
(* AUTO_INCR_ADDR returns an auto-increment mode address
   for a given register (parameter REG).  The byte size field
   of the address descriptor returned is set to the value of
   parameter VTYPE.  *)

public function auto_incr_addr ( reg: registers; vtype: vax_type ): addr_desc;

begin
  auto_incr_addr := pop_reference;
  with auto_incr_addr do begin
    byte_size := vtype;
    register := reg;
  end;
end  (* proc auto_incr_addr *) ;
$PAGE inc_addr, off_addr
(* INC ADDR adds a specified offset to the offset field of its
   parameter ADDR DESC and returns this updated record.	
...
public function inc_addr (var addr: addr_desc; addition: unit_range): addr_desc;

begin
  addr := off_addr (addr, addition);
  inc_addr := addr;
end;
$PAGE rt_addr
...
public function add_constant ( addr: addr_desc; cons_val: int_type;
