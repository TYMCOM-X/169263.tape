...
procedure rel_int ( x: integer; rel: boolean );
...
public function radix50 ( code: integer; sym: str6 ): pdp10word;
...
procedure rel_50 ( code: integer; sym: str6 );
...
begin
  ...
  rel_xwd (#O400000, false, #O400000, false);
...
procedure offset_value ( value: code_address; offset: integer;
...
begin
  svalue := value + offset;
  if (svalue >= 0) andif
     ( (svalue < #O400000) = (value < #O400000) ) then begin
    ...
    if value <= #O400000
      ...
      else back_chain (get_offset (high_base, svalue - #O400000), lr, ic, rslt_value, rslt_rel);
end (* offset_value *);
$PAGE relocate
...
procedure relocate ( value: code_address; rel: reloc_value;
                     lr: left_right; ic: code_address;
                     var rslt_value: code_address; var rslt_rel: boolean );
    ...
    while s <> nil do begin
      with s^ do begin
        if (kind = consts) andif public_dcl andif (init_value.kind <> subr_cst) then begin
          if emit_symbols then
            rel_50 (def_global_sym, name^.text);
          if emit_sy and prog_options.overlay_opt then
            rel_int (true_highseg_fin, true)
          else begin
