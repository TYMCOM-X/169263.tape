...
type
  tuple_opcodes =
	...
	length_op,	lwc_op,		upc_op,		addr_op,
	...
	func_call_op,	func_signal_op,	(* These are the call expressions. *)


			(*  S T A T E M E N T S  *)
			(*  -------------------  *)

	call_op,	signal_op,	(* These are the call statements. *)

	start_stmt,	start_with,	end_with,	start_cond,
	end_cond,	eval_op,	assign_op,	dispose_op,
	nop,		abort_op,	case_abort_op,

	start_block,	end_block,	label_node,	jump_op,
	jump_t_op,	jump_f_op,	jump_in_op,	gen_jump_op,
	case_jump_op,	stop_op,	return_op,	goto_op,
...
const
  first_expr	   = cst_ref;		last_expr	= func_signal_op;
  ...
  first_sunary_op  = new_op;		last_sunary_op	= iostatus_op;
...
type
  ...
  oper_index = 1 .. maximum (oper_range);
$PAGE expr_type_desc
	...
	case kind: type_kind of		(* type-specific information *)
	  ...
	  sets:
	    (  set_cst_lwb: boolean;	(* => first set element known at compile time *)
	       ...
	       set_lwb: set_element_range;	(* first element -- const or lower bound *)
	       set_length: set_element_range  )	(* # of elements -- const or upper bound *)
...
$PAGE tuple_node - miscellaneous statements
  tuple_node = packed record
	...
	(  with_rec: expr  );			(* referenced record value *)
      ...
      start_cond, end_cond:		(* enter/leave the scope of a condition handler *)
	(  handler: blk  );			(* denotes handler for condition *)
...
$PAGE tuple_node - flow control statements
      (* The first tuple in a routine is a StartBlock tuple, and the last is
	 an EndBlock tuple.  Within the routine, each basic block begins with
	 a LabelNode tuple.  The StartBlock, EndBlock, and all the LabelNode's
	 are numbered and linked in a circular list.  The StartBlock is 0, and
	 has an upward pointer to the EndBlock and a downward pointer to the
	 first LabelNode.  The first LabelNode is 1, and has an upward thread
	 to the StartBlock and a downward thread to the second LabelNode.  The
	 EndBlock is n+1, where n is the number of basic blocks, and has an
	 upward pointer to the last LabelNode and a downward pointer to the
	 StartBlock. *)

      start_block, (* first node of the body of a routine *)
      end_block, (* last node of the body of a routine *)
      label_node: (* start of a basic block; may define a label *)
	(  label_sym: sym;			(* the label defined, nil if just start of block *)
	   block_order_no: index_range;    (* DFST ordering, 1 is first label *)
	   downward_thread: tuple;		(* successor label, or EndBlock tuple *)
	   upward_thread: tuple;		(* predecessor label, or StartBlock tuple *)
      ...
      (* Each basic block ends with a StopOp, a ReturnOp, a GotoOp, or one or
	 more jump tuples of various sorts.  These block terminating tuples
	 may be accessed by starting at label^.downward_thread^.prev and
	 working backwards.

	 The LabelNode also has InwardJumps, a pointer to a chain of all the
	 jump tuples whose destination is this LabelNode.  The jumps within
	 the chain are linked together by their NextInwardJump pointers.  *)
	...
	(  cond: expr;				(* condition or selector, nil -> unconditional *)
	   low_cntrl, high_cntrl: int_type;	(* range of values of computation causing jump *)
...
$PAGE tuple_node - input/output statements
	...
	(  rw_mode: rd_wr_modes;		(* the mode of the transfer *)
...
$PAGE tuple_node - data reference expressions
      ...
      call_op, signal_op: (* treated like expr as they have parm lists *)
...
$PAGE tuple_node - computational expressions
	     ...
	     func_signal_op:
		  ...
		  nargs: parm_range;		(* number of arguments specified *)
		  arglist: packed array [parm_index] of expr  );	(* list of arguments *)

	     nary_op..last_nary_op,		(* all simple operators *)
	     first_chk_op..last_chk_op:		(* + all range check statements *)
	       (  nopers: oper_range;		(* number of operands *)
		  operand: packed array [oper_index] of expr  );	(* each operand *)

	     size_op:				(* returns size of type, with specified flexible value *)
	       (  type_ref: typ;		(* type record whose size is to be returned *)
		  type_parm: expr  )		(* upper bound for flex array;
						   (upperbound - lowerbound + 1) for generic;
						   max length for flex string *)
