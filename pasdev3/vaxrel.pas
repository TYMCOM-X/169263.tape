...
type
  relf_byte = 0.."ff;  (* unit of output to the rel file *)
  packed_relf_bytes = packed array [1..4] of relf_byte;
  binary_file = file of packed_relf_bytes;
  
external function pathname_of_binary (var binary_file): file_id;
external procedure scratch ( var binary_file );
...
const
  ...
  static_flags: psect_flags := ("01, "89);
  code_flags:   psect_flags := ("00, "e9);
  ...
  for ioper := 1 to cr^.noperands do
	...
	other:
	  ...
	  if immediate then begin
	    ...
	    n := offset;
	    if reloc.kind = local_sc then (* addressing backwards from fp *)
	      n := n - reloc.relsym^.item_addr
	    else if reloc.kind = parameter_sc then (* addressing forwards *)
	      n := n + reloc.relsym^.item_addr;
...
type
  elem_array = array [1..1] of  packed record
    span,
    long: word;
    sdi_ptr: code
  end;
...
begin
  ...
  while cr <> nil do begin
	...
	ic := ic + op_length (cr) + 1
      ...
      stringword:		 ic := ic + 2 + cr^.stringlen;
      pstringword:		 ic := ic + cr^.stringlen;
...
public procedure rel_init;
  ...
  const
    lang_name: lang_string = 'MDSI PAXCAL Version ';
  ...
  begin
	...
	return
      ...
      rel_file := pathname_of_binary (relf); (* save actual file name *)
    end;
...
*)
    ...
    rel_byte (0); rel_byte (2); (* max rec size = "02 00 *)
    with root_block^.children^.id^ do
      rel_name (substr (text, 1, len) );
...
procedure sta_gbl (global_sym: std_var_string);
  begin
    stoim_count := "100;  (* later store-immediates will begin new command *)
    if last_byte > (max_relf_rec - length (global_sym) - 2) then
...
procedure sta_psect_base (psect: byte; offset_in_psect: unit_range);
  begin
    stoim_count := "100;  (* later store-immediates will begin new command *)
    if last_byte > (max_relf_rec - 6) then
      rel_record (obj$c_tir);  (* start new record if insufficient room in this one *)
...
procedure sta_int (int_value: unit_range);
  ...
  begin
    stoim_count := "100;
    if last_byte > (max_relf_rec - 5) then
      rel_record (obj$c_tir);
    int_bytes.int := int_value;
...
$PAGE sto_picr, set_rel_base, add_top_two
...
*)
    if indirect then stoim ("ff); (* lw displ. deferred off pc *)
    stoim_count := "100;  (* later store-immediates will begin new command *)
    if last_byte > (max_relf_rec - 1) then
      rel_record (obj$c_tir);  (* start new record if insufficient room in this one *)
    if indirect then
...
Begin
	  if (n = 0) and not indirect then begin (* no displacement required? *)
	    ...
	    stoim ("a0 + "10 * ord(indirect) + register);
	    stoim_b_int (n);
	    ic := ic + 2
	  end
	  else if (n >= -32768) and (n <= 32767) then begin (* need a word? *)
	    stoim ("c0 + "10 * ord(indirect) + register);
	    stoim_w_int (n);
	    ic := ic + 3
	  end
	  else begin (* need fullword *)
	    stoim ("e0 + "10 * ord(indirect) + register);
...
(*$x1
    ...
    if switch ( cur_block^.dump_switches, 'OPSTATS' ) then begin
      if index <> noreg then
        write ( ops_file, ' ' || cv_str2 ( "40 + index ) );
...
*)
    ...
    case addr_mode of
      ...
      auto_inc: begin
        if indirect then
          stoim ("90 + register)  (* autoincrement deferred *)
        else
	  stoim ("80 + register);  (* autoincrement *)
	ic := ic + 1
      end;
...
*)
	...
	if immediate then begin
	  ...
	  if reloc.relsym^.init_value.kind = no_value then
	    with reloc.relsym^.name^ do
	      sta_gbl (substr(text, 1, len))
	  else
	    with reloc.relsym^.init_value.valp^ do
	      sta_gbl (substr(str_val, 1, str_len));
	  ...
	  if ext_refs = nil then begin
	    ...
	    ext_refs^.next := nil;
	    ext_refs^.ext_name := reloc.relsym
	  end
	  else begin
	    next_ext := ext_refs;
	    while (next_ext^.ext_name <> reloc.relsym) and
	            (next_ext^.next <> nil) do
	      next_ext := next_ext^.next;  (* scan until sym found or list ends *)
	    if next_ext^.ext_name <> reloc.relsym then begin (* wasn't already in list *)
	      new (next_ext^.next, extern_def);
	      next_ext := next_ext^.next;
	      next_ext^.next := nil;
	      next_ext^.ext_name := reloc.relsym
	    end
	  end
	end
  
	else if reloc.kind = static_sc then begin
	  sta_psect_base (psect_id [static_psect], reloc.relsym^.item_addr + offset);
	  sto_picr (indirect);
	  ic := ic + max_disp_bytes + 1
	end
	  ...
	  If RELOC.RELDEF^.DEFTYPE = TEMP_SIZE_DEF
	    ...
	    if byte_size = vax_byte then begin
	      stoim ("af);  (* byte displacement mode off pc *)
	      stoim_b_int (reloc.reldef^.addr + offset - 2 - ic);
	      ic := ic + 2
	    end
	    else if byte_size = vax_word then begin
	      stoim ("cf);  (* word displacement mode off pc *)
	      stoim_disp (reloc.reldef^.addr + offset - 3 - ic, 2 * bits_per_byte);
	      ic := ic + 3
	    end
	    else begin
	      stoim ( "ef );	(* longword displacement mode off pc *)
...
type
  vax_real_template = packed record
    vax_fill: 0.."f;
    vax_low_fraction: 0.."ffff;
    vax_sign: 0..1;
    vax_exponent: 0.."ff;
    vax_high_fraction: 0.."7f;
    vax_w2_fill: 0.."f;
    vax_w2_low_fraction: 0.."ffff;
    vax_w2_high_fraction: 0..7;
    vax_w2_mid_fraction: 0.."1fff
  end;
  ...
  ten_real_template = packed record
    ...
    pdp10_high_fraction: 0.."7f;
    pdp10_low_fraction: 0.."ffff;
    pdp10_fill: 0..7;
    pdp10_w2_sign: 0..1;
    pdp10_w2_high_fraction: 0.."1fff;
    pdp10_w2_low_fraction: 0.."ffff;
    pdp10_w2_unused_fraction: 0.."3f
  end;
...
begin
  with cr^ do begin
	...
	with ten_real, template do begin
	  if (pdp10_high_fraction <> "7f) or 
	     (pdp10_low_fraction <> "ffff) then begin
	    int_equiv1 := int_equiv1 + 4;
	  end
	  else if pdp10_exponent <> "ff then begin
    ...
    (* Double precision case.  *)
      ...
      if (ten_real.real_equiv <> 0.0) andif
	 (ten_real.template.pdp10_w2_unused_fraction >= "20) then begin
	with ten_real, template do begin
	  if (pdp10_w2_high_fraction <> "1fff) or
	     (pdp10_w2_low_fraction <> "ffff) then begin
	    int_equiv2 := int_equiv2 + "20;
	  end
	  else if (pdp10_fill <> 7) orif
		  (pdp10_low_fraction <> "ffff) orif
		  (pdp10_high_fraction <> "7f) orif
		  (pdp10_exponent <> "ff) then begin (* all this => not max real *)
	    pdp10_w2_high_fraction := 0;
	    pdp10_w2_low_fraction := 0;
	    if (pdp10_fill <> 7) or
	       (pdp10_low_fraction <> "ffff) or
	       (pdp10_high_fraction <> "7f) then begin
...
begin
  ...
  if p4_error_level > warning then begin	(* close and delete rel file *)
  ...
  for rt := rt_first to rt_last_used do
    if rt_refs [rt] then begin
      if last_byte > (max_relf_rec - (PUBLICS_LENGTH+5)) then
        rel_record (obj$c_gsd);
      rel_byte (gsd_sym); (* symbol specification entry *)
      rel_byte (0);  (* data type = unspecified *)
      rel_byte ("08); rel_byte (0);  (* flags = strong; ref; within; relative *)
  ...
  while next_ext <> nil do begin
    ...
    rel_byte (0);  (* data type = unspecified *)
    rel_byte ("08); rel_byte (0);  (* flags = strong; ref; within; relative *)
    if next_ext^.ext_name^.init_value.kind = no_value then
      with next_ext^.ext_name^.name^ do
	rel_name (substr(text, 1, len))
    else
      with next_ext^.ext_name^.init_value.valp^ do
	rel_name (substr(str_val, 1, str_len));
    temp_def := next_ext;
    next_ext := next_ext^.next;
    dispose (temp_def)
  end (* while *);
  ...
  while symbol <> nil do
	...
	if kind = vars then begin
	  if last_byte > (max_relf_rec - (PUBLICS_LENGTH+10)) then
	    rel_record (obj$c_gsd);
	  rel_byte (gsd_sym); (* symbol specification entry *)
	  rel_byte (0); (* data type = unspecified *)
	  rel_byte ("0a); rel_byte (0); (* flags = strong; def; within; relative *)
	  rel_byte (psect_id[static_psect]);
	  rel_longword (item_addr);
	  rel_name (substr(name^.text, 1, name^.len))
	end
  
	else if kind = consts then begin
	  
	  if init_value.kind = subr_cst then begin	(* global defns for public routines *)
	    if last_byte > (max_relf_rec - (PUBLICS_LENGTH+12)) then
	      rel_record (obj$c_gsd);
	    rel_byte (gsd_epm);  (* entry point and mask def. entry *)
	    rel_byte ("16);  (* data type = seq. of instructions *)
	    rel_byte ("0a); rel_byte (0); (* flags = strong; def; within; relative *)
	    ...
	    rel_byte ("0a); rel_byte (0); (* flags = strong; def; within; relative *)
	    rel_byte (psect_id[code_psect]);
	    temp_def := defval (init_value.defp);
  ...
  if startdef <> nil then begin	(* global defn for PAX$PROGRAM. *)
    if last_byte > (max_relf_rec - (PUBLICS_LENGTH+12)) then
      rel_record (obj$c_gsd);
    rel_byte (gsd_epm);  (* entry point and mask def. entry *)
    rel_byte ("16);  (* data type = seq. of instructions *)
