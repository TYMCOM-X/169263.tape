...
type
  ...
  heap_array = array [1..2] of packed array [0..7] of boolean;
  set_byte_array = ^heap_array;
...
function len_bytes (lwb_bit, len_bits: set_element_range): set_element_range;
  begin
    len_bytes := ngm (len_bits + lwb_bit mod bits_per_byte, bits_per_byte) div bits_per_byte
  end;
  
function byte_num (bit_num: set_element_range): set_element_range;
  begin
    byte_num := bit_num div bits_per_byte
  end;
  
function upb (lwb_arg, len_arg: set_element_range): int_type;
  begin
    upb := lwb_arg + len_arg - 1
  end;
  
function len (upb_arg, lwb_arg: set_element_range): set_element_range;
  begin
    assert (upb_arg >= lwb_arg);
    len := upb_arg - lwb_arg + 1
  end;
...
public function setb_size (exp: expr): set_len_range;
...
begin
  ...
  with int_valued_exp^ do
    case opcode of
      ...
      others: begin
        min_val := minimum (int_type);
        max_val := 2**desc.int_prec - 1
      end
    end (* case *);
...
public procedure shape_set (    exp: expr;
                            var shaped_lwb, shaped_len: set_element_range);
  
var
  temp_lwb, temp_len, upper_bound, temp_upper_bound: set_element_range;
  i: oper_range;
  fixed: boolean;
  
procedure shape_gen_set_operand (    int_valued_exp: expr;
                                 var op_lwb, op_len: set_element_range;
                                 var fixed: boolean);
  var
  ...
  begin
    ...
    assert (min_val <= maximum (set_element_range));
    if (max_val < 0) orif (max_val < min_val) then (* null set *) begin
      ...
      op_len := len (min (maximum (set_element_range), max_val), op_lwb);
    end
  end;
...
begin (* shape_set *)
  with exp^ do begin
	...
	desc.set_cst_len := (operand[1]^.desc.set_cst_len andif 
      ...
      union_op: begin
        shaped_lwb := maximum (set_element_range);
        upper_bound := 0;
...
public function set_temporary (ctxt_lwbbit: set_element_range; len_addr: addr_desc): set_desc;
...
public procedure gen_c5 (opc: opc_range; slen, sloc, tlen, tloc: addr_desc);
  ...
  var
    ...
    source_len, target_len: set_element_range;
  ...
  begin
	...
	case source_len of
      ...
      else if slen_const andif ((opc = movc5) and (source_len in [1,2,4,8])) then
	case source_len of
...
begin
  ...
  if aconstp (to_desc.arg[3], target_length) then begin
    if to_desc.arg[2].offset < from_desc.arg[2].offset then begin
      ...
      target_addr := offset_addr (duplicate_addr (to_desc.arg[1]), int_value (cleared_length));
...
public procedure prep_bitreference (element: addr_desc; var bit_position: addr_desc;
				    min_val, max_val: int_ttarget_desc: set_desc;
				    var defn: def);
  
var
  ctxt_base, elem_val, temp: set_element_range;
...
var
  bit_position, temp_addr: addr_desc;
  elem_val: set_element_range;
  fixed: boolean;
  defn: def;
  min_val, max_val: int_type;
...
begin
  ...
  if not aconstp (bit_position, elem_val) orif
    ...
    (* try to be clever *)
    temp_addr := offset_addr (duplicate_addr (target_desc.arg[1]), int_value (byte_num (elem_val)));
    if bit_value = 0 then
...
public procedure gen_set (source_desc: set_desc; target_desc: set_desc);
...
var
  ...
  low, high, ctxt_base, const_base, const_bytelength, target_len: set_element_range;
...
procedure clip_bits (set_addr: addr_desc;
		     source_lwb, source_length, target_lwb, target_length: set_element_range);
  
type
  mask_table = array [1..7] of int_type;
const
  lmask: mask_table := ("01, "03, "07, "0f, "1f, "3f, "7f);
  hmask: mask_table := ("80, "c0, "e0, "f0, "f8, "fc, "fe);
...
public function set_fetch (set_exp: expr;
                           ctxt_lwbbit, ctxt_lenbits: set_element_range;
			   var desired_loc: addr_desc; force_fixed: boolean): set_desc;
...
function do_set_fetch (exp: expr;
		       ctxt_lwbbit, ctxt_lenbits: set_element_range;
		       var desired_loc: addr_desc): set_desc;
  ...
  procedure reverse (var first_op, second_op: set_desc);
    var
      temp: set_desc;
    begin
      temp := first_op;
  ...
  function in_context (arg_set: set_desc;  ctxt_lwbbit: set_element_range): boolean;
    begin
  ...
  function index_addr (var addr: addr_desc; x_reg: registers): addr_desc;
    begin
      ...
      index_addr := addr; (* note: not duplicated *)
...
begin
  with exp^ do begin
	...
	else if desired_loc = no_preference then
	...
	if desired_loc <> no_preference then
	...
	if adr_equal (first_op.arg[1], do_set_difference.arg[1]) then begin
	  ...
	  if emit_loop then begin
	    gen2 (typ_opc (bicl2, vtype), index_addr (second_op.arg[1], r0), index_addr (target, r0));
	  ...
	  if emit_loop then begin
	    gen3 (typ_opc (bicl3, vtype), index_addr (second_op.arg[1], r0), index_addr (first_op.arg[1], r0),
			    index_addr (do_set_difference.arg[1], r0));
	    gen2 (sobgeq, r0_addr, branch_addr (defn))
	  end
	  else
...
var
  ...
  first_loc, second_loc, destination_loc, loop_length, temp_addr: addr_desc;
  loop_lwb, low_empty, loop_upb, high_empty: set_element_range;
  vtype: vax_type;
  defn: def;
...
begin
  with exp^ do begin
    ...
    if adr_equal (second_op.arg[1], do_intersection.arg[1]) then (* second_op equals desired location? *)
      reverse (first_op, second_op); (* so we can take advantage of it *)
	...
	destination_loc := offset_addr (destination_loc, int_value (low_empty))
	 ...
	 (not reuse_first_op orif  (* don't clear space already null *)
	    (byte_num (upb (operand[1]^.desc.set_lwb, operand[1]^.desc.set_length))
		    > loop_upb)) then begin
        temp_addr := offset_addr (duplicate_addr (destination_loc), loop_length);
	gen_c5 (movc5, int_value (0), stack_top_reference,
    ...
    if first_loc = destination_loc then
      (* Go with 2-operand form, implicitly targeting back into first_op. *)
      if emit_loop then begin
	gen2 (typ_opc (mcoml, vtype), index_addr (second_loc, r0), r1_addr);
	gen2 (typ_opc (bicl2, vtype), r1_addr, index_addr (first_loc, r0));
	gen2 (sobgeq, r0_addr, branch_addr (defn))
      end
      else begin
	...
	gen2 (typ_opc (bicl2, vtype), r1_addr, first_loc)
      ...
      if emit_loop then begin
	gen2 (typ_opc (mcoml, vtype), index_addr (second_loc, r0), r1_addr);
	gen3 (typ_opc (bicl3, vtype), r1_addr, index_addr (first_loc, r0), index_addr (destination_loc, r0));
	gen2 (sobgeq, r0_addr, branch_addr (defn))
      end
      else begin
	gen2 (typ_opc (mcoml, vtype), second_loc, r1_addr);
...
var
  ...
  set_byte: set_byte_array;
  j, real_lwb, real_upb,
  alloc_len, const_bytelength, const_base, const_lwb, const_upb, first_const_elem: set_element_range;
  bit: 0..7;
...
procedure perform_or (var second_op: set_desc; last_op: boolean);
  ...
  begin
    ...
    if singleton (do_union) andif not singleton (second_op) then
      reverse (second_op, do_union); (* cater to special casing of singleton operand *)
	...
	destination := do_union
      ...
      else if (not last_op) orif (desired_loc = no_preference) then
	...
	reverse (second_op, do_union);

      (* Choose the destination. *)

      if last_op andif (desired_loc <> no_preference) andif
	...
	if emit_loop then begin
	  gen2 (typ_opc (bisl2, vtype), index_addr (second_op.arg[1], r0), index_addr (target, r0));
	  gen2 (sobgeq, r0_addr, branch_addr (defn))
	end
	else
	...
	(* Utilize 3-operand form. *)
	if emit_loop then begin
	  gen3 (typ_opc (bisl3, vtype), index_addr (second_op.arg[1], r0), index_addr (do_union.arg[1], r0),
							      index_addr (destination.arg[1], r0));
	  gen2 (sobgeq, r0_addr, branch_addr (defn))
	end
	else
...
begin
  with exp^ do begin
    ...
    num_nonconst := 0;
    real_upb := 0;
    real_lwb := maximum (set_element_range);
	...
	first_const_elem := min (const_lwb - const_base, first_const_elem)
	  ...
	  perform_or (current_op, i = nopers (* last operand? *))
      end
    end (* for *);
...
var
  ...
  desc_upb, const_lwb, const_upb: set_element_range;
...
begin
  ...
  with exp^, do_set_fetch do begin
	...
	in_temp := (opcode = func_call_op) andif in_context (do_set_fetch, ctxt_lwbbit)
      ...
      gen_set_op: begin
        if nopers = 0 (* null set *) then begin
          nargs := 0; (* "z" format *)
	  desc.set_lwb := 0;
	  desc.set_length := 0
	end
        else (* nopers = 1 or 2 *) begin
          nargs := 2; (* "o" format *)
          arg[1] := fetch_fullword (operand[1]);
          lwb_exp := operand[1];
          if nopers = 1 then begin (* singleton set *)
...
var
  shaped_lwb, shaped_len: set_element_range;
...
function find_runtime_upb (set_exp: expr; var ltz_check_required: boolean): addr_desc;
  ...
  begin
	...
	case opcode of
	  ...
	  gen_set_op: begin
	    operand [nopers]^.usage_count := operand [nopers]^.usage_count + 1; (* keep fetch happy *)
	    find_runtime_upb := fetch_fullword (operand [nopers]);
	    genset_op_bounds (operand[nopers], min_val, max_val, fixed);
...
public function set_parameter (actual_expr: expr; parm_kind: sym_kind;
			       ctxt_lwbbit, ctxt_upbbit: set_element_range): addr_desc;
  
var
  ctxt_lenbits, ctxt_len_bytes: set_element_range;
