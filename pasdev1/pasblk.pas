...
$PAGE semantics
(* SEMANTICS is the master semantics routine.  It creates the top-level
   program block, processes the parse tree (if any), and calls BLOCK to
   read the declarations and (optional) body of the main block. *)

public procedure semantics;

var have_public: boolean;
$PAGE open_block
(* OPEN BLOCK establishes a block as the current block. *)

procedure open_block ( this_block: blk );
 begin
  cur_block := this_block;
  if cur_block^.kind = module_blk then
    have_public := false;
 end;
$PAGE close_block
...
procedure close_block;
 ...
 begin
  ...
  if (cur_block^.kind = module_blk) and (not have_public)
    then error (err_no_entry);                  (* must have one symbol accessible to outside *)
...
procedure declare_consts;
 ...
 begin
  ...
  have_public := have_public or public_flag;
  del_tuples;
...
procedure declare_types;
 ...
 begin
  ...
  while group <> nil do begin
    ...
    typenode := type_semantics (typedesc, [flexible_ok, flex_rec_ok]);      (* semanticate the type *)
     ...
     if (name^.scopechain <> nil) andif (name^.scopechain^.block = cur_block)
       then begin                                       (* same name declared in current block *)
         with name^.scopechain^ (* the symbol *) do begin
           if (kind = consts) andif (inite.kind = subr_cst)        (* is name of actual subr *)
...
$PAGE semantics - main routine
 ...
 begin
  ...
  (* Process the compilation ptree.  May either see a <program id> or a <module id>.
     The BNF specifies that a <module id> node will be returned if no ptree
     is actually present.  (There is no module identifier.)  This conforms
     to the archaic syntax. *)

  get_ptree;
  opts := nil;
  module_name := nil;
  if ptree^.sym in [program_id, module_id, envmod_id] then begin
    case ptree^.sym of
