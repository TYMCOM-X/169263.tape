...
public function fetchstring ( exp: expr; length_context: str_len_context ): str_desc; forward;
$PAGE free_string
...
public function str_temp ( DESC : ADDR_DESC;  len: char_range ): str_desc;

begin
  init_str_desc ( str_temp );
  with str_temp do begin
    If ( LEN <= 128 ) Or DESC.IMMEDIATE
      Then BASE_ADDR := GET_TEMP ( INT_VALUE ( LEN ) , VAX_WORD )
    Else BASE_ADDR := GET_TEMP ( DESC , VAX_WORD );
...
begin
  ...
  source_addr := skip_len_word ( rhs_desc );
  dest_addr := skip_len_words ( lhs_desc );
...
public procedure move_string ( lhs_desc: str_desc; rhs_desc: str_desc; padded: boolean;
    free_proc: free_procedure; remainder: boolean;  var remainder_addr: addr_desc );
...
(* CONCAT_DESC generates code for a CAT_OP expression tuple.  Parameter
   EXP is the CAexpression tuple.  Parameter CASE_CODE indicates
...
type
  str_desc_array = array [1..1] of str_desc;
...
begin
  with exp^ do begin
    ...
    CONCAT_DESC := STR_TEMP ( TOTAL_LEN_ADDR , TEMP_LENGTH );
    Free ( TOTAL_LEN_ADDR );
    ...
    For I := 1 to NOPERS Do Begin
      MOVE_STRINGS ( CONCAT_DESC, OP_DESCS^[i], False, CONCAT_FREE, I<>NOPERS, NEW_BASE_ADDR );
  ...
  case exp^.opcode of
	...
	with get_str_desc do begin
	  base_is_bound := ( exp^.desc.kind <> chars ) andif
	    ...
	    then base_addr := gen_cst ( char_value, vax_byte );	(* an immediate character value. If so, *)
...
function do_translation ( desc: str_desc ): str_desc;
  ...
  with desc do begin
    ...
    if trans_code <> no_trans then begin
      ...
      is_varying := ( type_desc.kind = strings ) andif
		    ( type_desc.str_kind = varying );
      if is_varying
        then dest_desc := str_temp ( DESC.LEN_ADDR , max_len + ( str_lw_width div bits_per_byte ) )
        else dest_desc := str_temp ( DESC.LEN_ADDR , max_len );
...
begin
  with index_op_expr^ do begin
    ...
    if ( nopers = 3 ) andif 	(* explicit default *)
...
public function copy_string ( source_desc: str_desc ): str_desc;
...
begin
  ...
  COPY_STRING := STR_TEMP ( SOURCE_DESC.LEN_ADDR , SOURCE_DESC.MAX_LEN );
...
begin
  with lhs_desc do begin
    ...
    else length_valid := false;		(* no special cases apply *)
...
procedure var_parameter ( actual_expr: expr; formal_type: typ ;
			var params:param_ptr; var param_ary_idx: parm_index );
...
(* VALUE_PARAMETER generates code for string parameters passed by value.
   ...
   the argument word(s) on the stack.  Parameter ACTUAL_EXPR is the
...
begin
  ...
  if not no_copy then begin
    ...
    if formal_kind = varying
      then temp_length := temp_length + (str_lw_width div bits_per_byte);
  
    TEMP_DESC := STR_TEMP ( BOUND_ADDR , TEMP_LENGTH );
