...
begin
  prevfptr := #o200 + curmod^.modid;	(* points into directory block *)
...
begin
  ...
  while dirsiz > 0 do begin		(* count up number of pages *)
    ...
    if coerce.rh <> 0 then begin	(* a real file page, non-zero *)
      ...
      exesize := exesize + (coerce.lh div #o1000) + 1	(* in high 9 bits *)
      end;
    curdir := curdir + 2;
    dirsiz := dirsiz - 2
    end;
  ...
  readrn (dbfile, #o201, dir);		(* update core directory block *)
...
begin
  ...
  exesize := 1; curdir := 1;
  coerce.int := page[0];
  if coerce.lh <> #o1776 then io_error (false, 'File not in DB format');
  page_size := coerce.rh;
  ...
  while dirsize > 0 do begin
    ...
    if coerce.rh <> 0 then begin	(* corresponding to file page *)
      ...
      exesize := exesize + (coerce.lh div #o1000) + 1
      end;
    dirsize := dirsize - 2;
    curdir := curdir + 2
    end;
  ...
  readrn (dbfile, #o201, dir);		(* keep core copy correct *)
