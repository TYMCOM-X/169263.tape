...
var
  ...
  current_ESDID : one_byte (* ESDID count for XREF's *);
...
(* CONVERT_ONE_BYTE - converts a one-byte value to either hexadecimal or
		      decimal characters, based on the value of input_mode, 
		      and then returns them. *)

function convert_one_byte (input_one_byte : one_byte;
			   input_mode : convert_mode) : string_10;

begin
  if input_mode = hex then
    putstring (convert_one_byte, input_one_byte:2:h)
  else
    putstring (convert_one_byte, input_one_byte:0);
end (* convert_one_byte *);
$PAGE convert_two_bytes
(* CONVERT_TWO_BYTES - converts a two-byte value to either hexadecimal or
		       decimal characters, based on the value of input_mode, 
		       and then returns the string. *)

function convert_two_bytes (input_two_bytes : two_bytes;
			    input_mode : convert_mode) : string_10;
...
begin
  if input_mode = hex then
    ...
    putstring (convert_two_bytes, input_two_bytes:0);
end (* convert_two_bytes *);
$PAGE convert_three_bytes
(* CONVERT_THREE_BYTES - converts a three-byte value to either hexadecimal or
			 decimal characters, based on the value of input_mode, 
			 and then returns the string. *)

function convert_three_bytes (input_three_bytes : three_bytes;
			      input_mode : convert_mode) : string_10;
...
begin
  if input_mode = hex then
    ...
    putstring (convert_three_bytes, input_three_bytes:0);
end (* convert_three_bytes *);
$PAGE convert_four_bytes
(* CONVERT_FOUR_BYTES - converts a four-byte value to either hexadecimal or
			decimal characters, based on the value of input_mode, 
			and then returns the string. *)

function convert_four_bytes (input_four_bytes : four_bytes;
			     input_mode : convert_mode) : string_10;
...
begin
  if input_mode = hex then
    ...
    putstring (convert_four_bytes, input_four_bytes:0);
...
begin
  fio_line (fb, 'M68000 Object File Analysis for file: ' || 
	    filename (input_file) || 
	    '         Page ' || 
	    convert_two_bytes (fb.pageno, decimal));
  fio_skip (fb);
end (* write_header *);
$PAGE warning
  ...
  repeat
    if local_char_number mod 256 = 0 then begin
      ...
      local_code_string := '    SN=$' || 
			   convert_one_byte (local_segment_number, hex); 
      local_char_string := '      ' ||
			   convert_one_byte (local_segment_number, decimal);
      local_segment_number := local_segment_number + 1;
    end;
    if local_char_number mod 16 = 0 then begin
      write_string (local_code_string || local_char_string);
      local_code_string := convert_one_byte (local_char_number mod 256, hex) || '    ';
      local_char_string := '     ';
    end;
    if local_char_number mod 16 = 8 then
      local_code_string := local_code_string || ' ';
    read_one_byte (local_one_byte);
    local_code_string := local_code_string || convert_one_byte (local_one_byte, hex) || ' ';
    local_char := '.';
...
begin
  ...
  write_string ('   Module version number = ' || 
		convert_one_byte (local_one_byte, decimal));
  read_one_byte (local_one_byte);
  check_eof;
  ...
  write_string ('   Source file user number = ' || 
		convert_two_bytes (local_two_bytes, decimal));
  ...
  case local_one_byte div 16 of
    ...
    2 : begin
	  ...
	  write_string ('   Section = $' || 
			convert_one_byte (local_one_byte mod 16, hex) ||
			'  (' || 
			convert_one_byte (local_one_byte mod 16, decimal) || 
                        ')');
	  read_four_bytes (local_four_bytes);
	  write_string ('   Size = $' || 
			convert_four_bytes (local_four_bytes, hex) || 
			'  (' || 
			convert_four_bytes (local_four_bytes, decimal) || 
                        ')');
	end;
    ...
    7 : begin
	  ...
	  write_string ('   XREF = ''' || local_string || '''');
          write_string ('   ESDID = $' ||
			convert_one_byte (current_ESDID, hex) ||
			'  (' || 
			convert_one_byte (current_ESDID, decimal) || 
                        ')');
...
procedure process_absolute_code;

var
  local_two_bytes : two_bytes;

begin
  write_string ('   Absolute code:');
  read_two_bytes (local_two_bytes);
  decrement_check_byte_count (2);
  write_string ('      Code = $' || 
		convert_two_bytes (local_two_bytes, hex));
...
procedure process_relocation_data;
...
begin
  write_string ('   Relocation data:');
  read_one_byte (local_one_byte);
  check_eof;
  decrement_check_byte_count (1);
  write_string ('      Flag byte = $' || 
		convert_one_byte (local_one_byte, hex));
  local_num_ESDID := get_bits (local_one_byte, 5, 3);
  write_string ('         Number of ESDIDs = ' || 
		convert_one_byte (local_num_ESDID, decimal));
  if get_bits (local_one_byte, 3, 1) = 0 then
    write_string ('         Size of relocated data = 1 word')
  else
    write_string ('         Size of relocated data = 2 words');
  local_offset_length := get_bits (local_one_byte, 0, 3);
  write_string ('         Offset field length = ' || 
		convert_one_byte (local_offset_length, decimal));
  ...
  case local_offset_length of
    ...
    3 : write_string ('      Offset = $' || 
		      convert_three_bytes (local_three_bytes, hex));
...
begin
  ...
  for local_counter := 1 to 4 do begin
    ...
    write_string ('   Map byte = $' || 
		  convert_one_byte (local_map[local_counter], hex) ||
                  '  (' ||
  ...
  decrement_check_byte_count (1);
  write_string ('   ESDID = $' || 
		convert_one_byte (local_ESDID, hex) ||
		'  (' || 
		convert_one_byte (local_ESDID, decimal) || 
		')');
  local_counter := 0;
  while (local_counter < 32) and
	 (byte_count > 0) do begin
    write_string ('');
    write_string ('   Entry number ' || 
		  convert_one_byte (local_counter + 1, decimal));
    if get_bits (local_map[ (local_counter div 8) + 1], 
		 7 - (local_counter mod 8), 1) = 0 then
      process_absolute_code
    else
      process_relocation_data;
  ...
  while not eof (input_file) do begin
    ...
    if byte_count = 0 then
      ...
      if skip_count > 0 then begin
        write_string ('');
        write_string (convert_four_bytes (skip_count, decimal) || 
