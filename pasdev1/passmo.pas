...
procedure sem_check_options ( var sub_opt: parse_node; subr_block: blk; positive: boolean );
...
procedure sem_special_options ( var sub_opt: parse_node; subr_block: blk; positive: boolean );
...
public function set_sem_options
  ...
  var
    sub_opt: parse_node; (* for chain of dump identifiers, or whatever *)
    positive: boolean;
    opt_ix: integer;
...
begin
  ...
  (* process option *)
  sub_opt := opt_node^.defn; (* first suboption, if there is one *)
  ...
  case option_ix of
    ...
    opt_storage:
      begin
        ...
        if (sub_opt = nil) orif (sub_opt^.sym <> intconst) then begin
          err_node (err_number_expected, opt_node);
          return;
        end;
        prog_options.storage := sub_opt^.value.ival;
	sub_opt := sub_opt^.next
      end;

    opt_ext_unique_length:
      begin
	if not (subr_block^.kind in [program_blk, module_blk]) then begin
	  err_node (err_extlen_module, opt_node);
	  return
	end;
	if not positive then
	  prog_options.external_unique_length := 0
	else begin
	  if (sub_opt = nil) orif (sub_opt^.sym <> intconst) then begin
	    err_node (err_number_expected, opt_node);
	    return
	  end;
	  if (sub_opt^.value.ival < 1) or (sub_opt^.value.ival > 255) then begin
	    err_node (err_ext_unique_length, sub_opt);
	    return
	  end;
	  prog_options.external_unique_length := sub_opt^.value.ival;
	  sub_opt := sub_opt^.next
	end
      end;
    ...
    opt_alloc:
      begin
        ...
        if sub_opt^.value.ival > 99 then begin
          err_node (err_alloc_mode, opt_node);
          return;
        end;
        prog_options.alloc_mode := sub_opt^.value.ival;
	sub_opt := sub_opt^.next
      end;
    ...
    opt_check:
      sem_check_options (sub_opt, subr_block, positive);
    ...
    opt_special:
      sem_special_options (sub_opt, subr_block, positive);

    opt_dump:
      begin
        while sub_opt <> nil do begin
          subr_block^.dump_switches :=
            enable_switch (subr_block^.dump_switches, sub_opt^.name^.text, positive);
          sub_opt := sub_opt^.next;
        end;
      end
  end (* case *) ;

  (* Verify that no unexpected suboptions are present. *)

  if sub_opt <> nil then begin
    err_node (err_too_many_opt_parms, sub_opt);
    return
  end;
