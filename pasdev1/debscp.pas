...
public function r50$asc (r50name: r50word): id_string;
  ...
  function convert_ch (r50ch: r50chtype): char;
    begin
      case r50ch of
        0:          convert_ch := '?';
        1..#O12:     convert_ch := chr (r50ch+#O57);
        #O13..#O44:   convert_ch := chr (r50ch+#O66);
...
begin               (* r50$asc *)
  ...
  repeat
    i := i - 1;
    text[i] := convert_ch (temp mod #O50);
    temp := temp div #O50
...
begin
  ...
  for i := 1 to min (linknamesize, length (ascname)) do begin   (* only look at first 6 chars *)
    cur$ch := ascname[i];
  exit if cur$ch = ' ';
    asctor50 := asctor50 * #O50;
    case cur$ch of
      'a'..'z':
        r50ch := ord (cur$ch) - #O126;
      'A'..'Z':
        r50ch := ord (cur$ch) - #O66;
      '0'..'9':
        r50ch := ord (cur$ch) - #O57;
      '.':
        r50ch := r50period;
      '$':
        r50ch := r50dollar;
...
begin
  with linkhead_ptr (jbsym)^ do
    if linkstsize = 0 then
      ...
      first$module := ptr (ord(firstsymbol) + (#O1000000 - linkstsize) - entrysize)
end (* first$module *);
  ...
  if curmodule <> nil then
    with curmodule^ do
      if stsize = 0 then
        ...
        next$module := ptr (ord(curmodule) - (#O1000000 - stsize));
...
begin
  ...
  with scope_stack do begin
    ...
    for dl := display_levels - 1 downto 1 do
      displays[dl] := open_parent (ays[dl + 1])
  end;
...
type
  ...
  xfer_vector_entry = packed record             (* resident xfer vector entry for public routine *)
    op_code: 0..#O777;                           (* first 3 fields represent *)
    filler: 0..#O777;                            (* PUSHJ to overlay manager *)
...
const
  pushj = #O260;                                 (* opcode for PUSHJ instr *)
