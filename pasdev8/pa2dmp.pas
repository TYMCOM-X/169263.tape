...
const tuple_names: tuple_narray :=
	...
	'STMT',		'WITH',		'ENDWITH',	'COND',
	'ENDCOND',	'EVAL',		'ASSIGN',	'DISPOSE',
	'NOP',		'ABORT',	'CASEABT',

	'BLOCK',	'END',		'LABEL',	'JUMP',
	'JUMP',		'JUMP',		'JUMP',		'*JUMP',
	'CASE',		'STOP',		'RETURN',	'GOTO',
...
begin
  with t^ do begin
	...
	begin
	  ...
	  if (opcode <> jump_op) and (opcode <> gen_jump_op) then begin
	    fio_write (dumpfb, ' IF ');
...
(*  DMPFGRAPH will dump the flow graph of the current module.  *)


public procedure dmpfgraph;

type
    block_pred = function ( index_range; index_range ): boolean;

var
    n_blocks: index_range;
...
public procedure dmpdominators;
...
begin
  ...
  (*  This is the same old non-recursive tree walk--other examples are in
      the quick block computation in PASCGR.  *)

  level := 0;
  b := t_chain^.downward_thread;
  loop
    loop
      for i := 1 to level do
	fio_write (dumpfb, '| ');
      fio_line (dumpfb, cv_int (b^.block_order_no));
    exit if b^.dom_son = nil;
      b := b^.dom_son;
      level := level + 1;
    end;
    while (b^.dom_brother = nil) and (b^.idom <> nil) do begin
      b := b^.idom;
      level := level - 1;
    end;
  exit if b^.dom_brother = nil;
    b := b^.dom_brother;
  end;
end (* dmpdominators *);
$PAGE dmpwenv
