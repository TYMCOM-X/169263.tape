...
$SYSTEM pasbnf.nam
$SYSTEM ptmprm.inc
external function cvhex (integer): string;
$PAGE start_dump
...
begin
  ...
  with s^ do
    ...
    else sym_text := 'C.' || cv_int ( id_number );
end (* sym_text *);
$PAGE type_name
...
function cv_address (addr: integer): string;
...
begin
  cv_address := cv_int (addr);
  cv_address := substr (blanks, 1, 6 - length (cv_address)) || cv_address;
  s := cvhex (addr);
  cv_address := cv_address || substr (blanks, 1, 8 - length (s)) || s;
end;
  ...
  begin
    if not title_printed then begin
      ...
      fio_skip (dumpfb);
      title_printed := true;
      fio_line (dumpfb, '   DEC     HEX');
    end;
    fio_line (dumpfb, cv_address (addr) || code [class] || ' ' || name);
...
begin
  ...
  loop
    loop
      title_printed := false;
      printed_classes := [ ];
      if b^.return_sym <> nil then begin
	if passed_by_address (b^.return_sym)
	  then class := parameter_sc
	  else class := local_sc;
	prt_address ('<RETURN SYM>', b^.return_sym^.item_addr, class);
      end;
      if b^.kind = subr_blk then begin
	symbols := b^.parm_list.first;
	while symbols <> nil do begin
	  prt_address (sym_text (symbols), symbols^.item_addr, parameter_sc);
