...
begin
  ...
  with tsym^ do begin
    ...
    type_desc := temp_type;
    public_dcl := false;
    if cur_block^.kind = program_blk
      then dcl_class := static_sc
      else dcl_class := local_sc;
...
$PAGE new_call
(* NEW CALL compiles an invocation of the 'procedure.  The argument list is:
...
(* CHECK COMPATIBILITY is called with a pair of expressions.  If they are
   (or include) flexible arrays, then it may be necessary to emit a runtime
   check to make sure that their bounds match. *)
...
begin
  ...
  left_flex := left;
  right_flex := right;
  if left^.desc.base^.kind = records then begin
    if left^.desc.base^.variant_tag <> nil then begin
      err_node (err_flex_var_assignment, node);
      return;
    end;
    last_field := left^.desc.base^.field_list; (* find the trailing, flexible field *)
  ...
  assert (left_flex^.desc.kind <> records);

  if (left_flex^.desc.base^.kind <> arrays) or
     not (chk_com_opt in cur_block^.semantic_options) then return;

  chk := op2 (compat_chk, nil, arr_bound (upb_op, left_flex),
                               arr_bound (upb_op, right_flex) );
...
begin
  if mode = reading then
    ...
    if (io_item^.desc.kind <> unknown_type) andif
       not (io_item^.opcode in [first_data_ref..last_data_ref] - [cst_ref]) then begin
  ...
  else if (io_item^.opcode = ident_ref) andif
	  (io_item^.id_sym^.kind = consts) andif
	  not io_item^.id_sym^.explicit_type then
    err_node (err_bio_const_notype, arg)
  else begin
    assert (io_item^.desc.base <> nil);
