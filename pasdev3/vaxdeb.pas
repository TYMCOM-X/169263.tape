...
$PAGE stmt_block
  ...
  public procedure stmt_block (stmt: source_id; stmt_type: symbols);
...
begin
  ...
  gen_string (code_area, substr (name^.text, 1, name^.len));
  ...
  begin
	...
	while page <> nil do with page^ do begin
	  if left_page <> nil then begin	(* stmts in page *)
	    ...
	    if len > 0
	      then gen_string (code_area, substr (sub_title, 1, len))
	...
	gen_string (code_area, substr (f_name, start_ix, len));	(* generate filename *)
