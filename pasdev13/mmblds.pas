...
Begin
  ...
  If LINK_ITEM = CODE_REC		(* Set start addr of block *)
    Then REL_XWD ( #o0 , False , THE_CURRENT_LOC , True )
End;
$PAGE REL_WORD
...
type
    rep_array = packed array [' ' .. '_'] of 0..#o50;

const
    rep_50: rep_array =
     (    0,   0,   0,   0,  #o46,#o47,   0,   0,     0,   0,   0,   0,     0,   0,#o45,   0,
        #o1, #o2, #o3, #o4,   #o5, #o6, #o7,#o10,  #o11,#o12,   0,   0,     0,   0,   0,   0,
          0,#o13,#o14,#o15,  #o16,#o17,#o20,#o21,  #o22,#o23,#o24,#o25,  #o26,#o27,#o30,#o31,
       #o32,#o33,#o34,#o35,  #o36,#o37,#o40,#o41,  #o42,#o43,#o44,   0,     0,   0,   0,#o47  );
...
begin
  ...
  for i := 1 to 6 do
    if sym [i] <> ' ' then
      symval := symval * #o50 + rep_50 [sym [i]];
  radix50.code50 := code;
...
(* Add a symbol to the current record being output. Remember only six
   ...
   actual LINK-10 identifier. #o1 indicates a public definition symbol.
   #o14 indicates an external symbol linkage. Call the general wrapper REL_WORD
...
Begin
  ...
  REL_XWD ( #o0 , False , SYM^.ADDR , Not SYM^.FIRST )
End;
$PAGE DROP_SYMBOLS
...
Begin
  ...
  While WALK <> Nil Do
	...
	LOCAL  : ;	(* Do nothing *)
	EXT_DEF: REG_SYMBOL ( WALK , #o14 );
	DEFINED ,
	PUBLIC_DEFINITION : REG_SYMBOL ( WALK , #o1 )
      End;
      HOLD := WALK;
      WALK := WALK^.NEXT;
      Dispose ( HOLD )
...
begin
  ...
  if setupcode then begin		(* we're cool *)
    hsloc := #o400000;			(* Where the highseg starts *)
    lsloc := #o0;			(* Where the losegment starts *)
...
begin
  ...
  REC [ 2 ].RH := #o0;
  REC_IDX := 3;
  REL_XWD ( #o0 , False , HSLOC , True );
  REL_XWD ( #o0 , False , LSLOC , True );
  REC [ 1 ].RH := #o2;
...
Begin
  ...
  DROP_LINE ( '    jsp 1,' || s );
  REL_XWD ( #o265040 , False , ADR , RELOC )
End;
...
Begin
  ...
  REL_XWD ( #o260740 , False , ADR , ADR<>0 )
End;
...
Begin
  SET_ADR ( S , SYM , ADR , RELOC );
  DROP_LINE ( '     000000,,' || s );
  REL_XWD ( #o0 , False , ADR , RELOC )
End;
$PAGE jrst
publocedure jrst (id: mdlid);
...
begin
  ...
  drop_line ('	jrst		' || id);
  REL_XWD ( #o254000 , False , ADR , False )
end (* procedure jrst *);
...
Begin
  ...
  drop_line ('        jrst    1,      ' || id );
  SYM := FINDSYM ( ID );
  REL_XWD ( #o254040 , False , ADR , RELOC )
End;
$PAGE PORTAL
...
begin
  if curr_high
    Then P := ( HSLOC + #o10 ) Mod #o1000	(* Reserved stuff *)
  Else P := ( LSLOC + #o140 ) Mod #o1000;	(* The loseg reserved stuff *)
  P := #o1000 - P;
  If P < #o1000
    Then ALLOC ( P )
