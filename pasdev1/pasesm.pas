...
function count_elems ( list: parse_node ): integer;
...
function new_aggregate ( trgt_type: typ; size: integer; var nopers: integer ): expr;
...
function build_array ( level: integer; var def: parse_node;
                      list_size: integer; trgt_type: typ ): expr;

var nopers: integer;
    array_size: integer; (* The number of elements in the array. *)
    i: integer;
    def1: parse_node;
...
var nopers: integer;
    ...
    list_size: integer; (* The number of elements in the value list. *)
    array_size: integer; (* The number of elements in the array. *)
    array_levels: integer; (* The number of dimensions listed as one. *)
...
begin
  ...
  (*  If the array is flexible, then any length list is ok.  *)
  ...
  if (list_size >= array_size) orif (array_levels = 1) then
    array_semantics := build_array ( array_levels, def, list_size, type )
  else begin
  ...
  if array_levels > 1 then
    err_node (err_all_parens, ptn);
end (* array_semantics *);
$PAGE scl_limits
...
begin
  with src^ do begin
    ...
    (*  In the worst case, we have no useful limit information.  *)
      ...
      lo_lim := type_fullword^.minval;
      hi_lim := type_fullword^.maxval;
    end;
  end;
end (* scl_limits *);
$PAGE chk_scalar
...
(*  CheckArgBounds takes the type node for a formal parameter and the expression
    ...
    is not, then a runtime bounds check must be performed.  *)
...
begin
  ...
  if parm_type^.flexible or not arg^.desc.base^.flexible then return;

  if parm_type^.kind = strings then
    ...
    check := op2 (compat_chk, nil,
                    arr_bound (upb_op, arg),
                    cst_expr (cst_scalar (parm_type^.index_type^.maxval),
			      parm_type^.index_type) );
