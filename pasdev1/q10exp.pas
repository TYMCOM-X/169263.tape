...
static var
  with_table: array [2..#O14] of packed record
...
begin
  if (reg <> noreg) andif (reg < #O15) then
    with regdesc[reg] do begin
...
begin
  for reg := 2 to #O14 - ord (p > bits_per_unit) do
...
begin
  free ( fetch (with_tpl) );
  if (with_base < #O15) andif (regdesc [with_base].uses_remaining = 0) then begin
...
begin
  if with_base > #O14 then
    return;  (* no withs require restoration *)
  maxwith_regs := max (maxwith_regs, #O15-with_base);
...
public procedure left_shift (reg: registers; p: bit_range; n: integer);
...
public procedure right_shift (reg: registers; p: bit_range; n: integer);
...
public function iconstp (exp: expr; var val: integer): boolean;
...
public function aconstp (maddr: addr_desc; var val: integer): boolean;
...
public procedure get_bounds (exp: expr; var minval, maxval: integer);
...
public function int_value ( val: integer ): addr_desc;
 
begin
  int_value := absolute_reference;      (* init most fields to default *)
  if (val >= 0) and (val <= #O777777)
    then begin
...
(* PARAM_ADDR is passed a symbol node of kind VARS or VALUES and
   ...
   Note that:
        1. if the symbol is a formal parameter which is a flexible
           array, then the address is that of the start of the array,
           not the address of the bounds words.
...
begin
  with exp^ do begin
	...
	if id_sym^.init_value.kind = subr_cst then
	  (* Note - only "address (procedure or function)" can cause us to be here - in
	     any other context subroutines are referenced by cst_ref's not ident_ref's *)
	  with ident_addr.reloc do begin
	    kind := def_sc;
	    reldef := get_def (subr_def, id_sym^.init_value.blkp^.number)
	  end
	else
...
(* GET_LBOUND is passed an expression tuple whose associated type
   is string or array.  It returns the constant lower bound of the
   string or array. *)
...
(* LOWER_BOUND is passed both the expression tuple for an array or
   ...
   lower bound of the array or string, which will simply be an immediate
   reference or a reference to a constant in memory.  *)
...
$PAGE fetch_temporary
(* FETCH TEMPORARY generates code for an ALC_TEMP_OP tuple.  It either
   GET_TEMP or allocates a dynamic temporary on the top of the stack,
   depending on whether its base type is flexible or not. *)

function fetch_temporary (exp: expr): addr_desc;

var
  width : addr_desc;
  a, b, c, d: unit_range;
  grotesque: boolean;
  reg, loc_reg: registers;
  width_val: integer;

begin
  with exp^ do begin
    if not desc.base^.flexible then begin
      assert (operand[1] = nil);
      fetch_temporary := get_temp ((desc.base^.base_size + 35) div 36);
    end
    else begin
      assert (operand[1] <> nil);
      size_of (desc.base, false, a, b, c, d, grotesque);
      assert (not grotesque);
      if iconstp (operand[1], width_val) then
        fetch_temporary := get_temp ((a * width_val + b) div c)
      else begin
        if c = 1 then
          reg := load (operand[1], 36)
        else
          reg := load (operand[1], 72);
        if a <> 1 then
          gen_ri (imuli, reg, a);
         if b > 0 then
          gen_ri (addi, reg, b)
        ...
        gen (adjsp, sb, reg, 0, none);
        decr_reg_usages (reg);
        fetch_temporary := temp_reference;
        fetch_temporary.index := loc_reg;
        reset_needed := true;
      end (* if non-constant length *);
    end (* if flexible *);
  end (* with exp^ *);
end;
$PAGE case_op
...
begin
  with array_ref_exp^ do begin
    ...
    (* Calcuate the elements address.  Unless the index is known at compile
       time, an ADJBP instruction will be emitted.  *)


    if not index_is_constant then begin
