...
procedure prompt(line: string);
 begin
  if length(line)=0 then begin
    writ$str (' ?? ');
    read$nl (input_line);
    input_line := input_line || cr;
  end
  else begin (*set up passed line as new line*)
    input_line:= line || cr; (*which read$ch returns on end of line*)
  end;
  at$eoln := true;
...
public procedure debug$;
  ...
  (* CHECK MAXBRKPT is called whenever breakpoints are set or cleared
     and sets the maximum used breakpoint number to limit the required
     runtime search. *)

  procedure check_maxbrkpt;

  var
    brkidx: brknumtype;

  begin
    brk$max := -1;	(* in case no breakpoints set *)
    for brkidx := minimum (brknumtype) to maximum (brknumtype) do begin
      if brk$table[brkidx] <> nil then
	brk$max := brkidx
    end;
  end;
$PAGE check_status
...
type word_list =	(* includes both commands and other reserved words *)
      (	open_wrd, clear_wrd, display_wrd, step_wrd, sstep_wrd, stop_wrd,
...
const words: wn_type :=
	...
	( 'MODULES   ', 1 )  );
$PAGE keyword
(* KEYWORD parses a reserved word symbol.  First it checks that the current token
 ...
 begin
  ...
  case wrd of
	...
	if check (intconst) then begin		(* clear specified breakpoint *)
	  ...
	  check_end;
	  if query ('Clear all') then begin	(* check before doing it *)
	    clr$all$brkpt;
	    brk$max := -1;
	  end;
	end
