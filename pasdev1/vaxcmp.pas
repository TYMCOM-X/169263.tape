...
procedure compare_constants ( cmp: cmp_codes; value1: integer;
    value2: integer; loc: def );
...
procedure rangeset (set_op: expr; (* Expr tuple range_set is value of.   Only used
  ...
  begin
    ...
    if aconstp (range_set.arg[2], const_upb) then (* constant length *)
...
type
  set_op_table = array [1..4] of opc_range;st
...
begin
  with test^ do begin
    ...
    if negate then cmp := inverse [cmp];
    if compare_with_zero (operand[1], cmp, tcmp) then begin
      bool_single_compare := bool_compare_zero (tcmp, operand[2]);
      free (fetch (operand[1], no_preference))
    end
    else if compare_with_zero (operand[2], reverse [cmp], tcmp) then begin
      bool_single_compare := bool_compare_zero (tcmp, operand[1]);
      free (fetch (operand[2], no_preference))
    end
