...
begin
  ...
  type_string := dcl_string (132, varying, false);
  alloc_type (type_string);
  predef ('EXCEPTION_MESSAGE', exnamextern, consts, prtyp0 (type_string));
...
begin
  ...
  temp_sym := compiler_symbol (consts, 'NIL', type_ptr, cst_nil);

  temp_sym := compiler_symbol (consts, 'NILF', dcl_file (anyfile(*file mode*), nil(*component type*)),
			      cst_nil);
  temp_sym := compiler_symbol (consts, 'STORAGE_UNIT_SIZE', type_int, cst_scalar (byte_size))
end;
$PAGE init_std_pf
