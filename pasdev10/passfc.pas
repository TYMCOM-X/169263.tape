...
const arg_limits: call_desc = (
    ...
    (* extent *)        (1, 1, true),
    (* address *)       (1, 1, false),
    (* field_offset *)	(2, 2, false),
    (* field_width *)	(2, 3, false),
    (* xfer *)          (2, 2, false) );
...
begin
  ...
  with arg1^ do begin
    if (opcode = cst_ref) or
       ((opcode = ident_ref) andif (id_sym^.kind = consts)
			     andif not id_sym^.explicit_type) then begin
      err_node (err_size_const_arg, arg_list);
      bad_call;
      return
    end;
    if not ( ( (opcode = ident_ref) andif
               (id_sym^.kind in [types, vars, values, for_inds, consts]) )
	    orif
...
$PAGE make_offset_width_op
(*  MakeOffsetWidthOP creates a field_offset or field_width operator.  *)

procedure make_offset_width_op;
...
begin
  backup_tuple := if_cursor;
  arg1 := ref_semantics (arg_list, ref_ctxt); (* See what we are to find the offset or width of. *)
  ...
  if arg1^.desc.kind <> records then begin
    err_node (err_not_record_type, arg_list);
    bad_call;
    return
  end;

  a2 := arg_list^.next;
  field_sym := arg1^.desc.base^.field_list;
  with a2^ do
    while (field_sym <> nil) andif (field_sym^.name <> name) do
      field_sym := field_sym^.next;
  if field_sym = nil then begin
    err_node (err_invalid_field, a2);
    bad_call;
    return
  end;
  ...
  if code = fnfieldoffset then
    std_fcall := cst_expr (cst_scalar (field_sym^.fld_offset), type_int)
  else if not field_sym^.type_desc^.flexible then begin
    if a2^.next <> nil then begin
      err_node (err_arg_extra, a2^.next);
      bad_call;
      return
    end;
    std_fcall := cst_expr (cst_scalar (field_fld_width), type_int)
  end
  else begin
    if a2^.next = nil then begin
      err_node (err_flx_upb_required, arg_list);
      bad_call;
      return
    end;
    if field_sym^.type_desc^.kind = arrays then
      subscr_type := field_sym^.type_desc^.index_type
    else
      subscr_type := type_non_neg;
    arg3 := copy_semantics (a2^.next, subscr_type, true);
    std_fcall := op2 (imul_op, type_non_neg,
		      cst_expr (cst_scalar (byte_size), type_non_neg),
		      rtsize (field_sym^.type_desc, arg3, true))
  end;
end (* make_offset_width_op *);
  ...
  case code of
    ...
    fnabs:
      if chk_arg_type (arg1,a1,num_kinds,'numeric') then begin
        if arg1^.desc.kind = ints then begin
          make_unary_op (iabs_op,type_int);
	  std_fcall^.desc.int_prec := arg1^.desc.int_prec + ord (arg1^.desc.signed);
	  std_fcall^.desc.signed := false;
	end
	else
          make_unary_op (rabs_op,type_real);
      end
      else
        bad_call;
    ...
    fnaddress:  begin
      ...
      if arg1^.desc.kind = unknown_type then
        bad_call
      else if not (arg1^.opcode in [first_data_ref..last_data_ref] - [cst_ref]) then begin
