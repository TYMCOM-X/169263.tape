...
function int_const (value: integer): expr;
 var sclval: val;
 begin
  if (value < 0) or (value > #O377777) then begin        (* full word constant *)
...
function scalar_const (scalar_type: typ; value: integer): expr;
 begin
...
function parm_descriptor (id: sym; offset: integer): expr;
 begin
...
(* LOWER BOUND constructs a tree computing the lower bound of a string or
   array reference ("ref").  This returns a constant. *)
...
(* UPPER BOUND constructs a tree computing the upper bound of a string or
   ...
   known at compile time; otherwise (flexible arrays or strings),
...
function upper_bound (ref: expr): expr;
 ...
 begin
  ...
  else if ref^.opcode = agg_val then
    upper_bound := int_const (upperbound (ref^.operand) + reftype^.index_type^.minval - 1)
...
procedure ident_access (ref: expr; node: expr);
 ...
 begin
  ...
  with node^ do begin
	...
	if id^.init_value.kind = subr_cst then begin
	  (* Only "address (procedure or function)" can cause us to take this route
	     in handling a subroutine symbol. *)
	  item := subr_item;
	  item.sym_name := id^.init_value.blkp^.subr_sym
	end
	else
...
procedure unpacked_array_access (ref: expr; node: expr);
 ...
 var
  ...
  multiplier: integer;                 (* size of elements of the array in units *)
 ...
 begin
  with ref^ do begin
    ...
    (* Accumulate the lower bound directly into the constant offset. *)

    node^.item.offset := node^.item.offset - (idxtype^.minval * multiplier);
...
procedure add_cst_bit_index (byteaddr: expr; idx: integer);

 var
   nelems_per_unit: align_range;                (* number of packed elements per unit *)
   units: integer;                     (* pos/neg unit offset *)
   bits: integer;                              (* pos bit offset from start of word *)
   nelems: integer;                            (* offset in terms of packed elements *)
...
function expand_exp
  (op: tuple_opcodes; desc: expr_type_desc; x: expr; n: integer): expr;
...
function is_iconstp (node: expr; var c: integer): boolean;
 begin
  is_iconstp := false;
  c := 0;
  with node^ do begin
    ...
    strings:
      begin
        ...
        with parm_type^ do begin
          ...
          else begin (* pass address from descriptor *)
	    with temp^.operand[1]^ do
	      if item.pack or
		 ( (desc.base = byte_ptr_desc.base) andif
		   (desc.kind = byte_ptr_desc.kind) andif
		   (desc.signed = byte_ptr_desc.signed) andif
		   (desc.int_prec = byte_ptr_desc.int_prec) ) then begin
	      saddr := str_temp (temp^.desc, temp^.operand[2]);
	      expand_str_move (saddr, temp, temp^.operand[2]);
	      outcall^.arglist [parmidx] := saddr^.operand[1]
	    end
	    else
...
function prepare_call (* incall: tuple): tuple *) ;
 ...
 begin
  (* Special descriptor parameters are passed for flexible parameters.
  ...
  with ref^ do begin
    case desc.kind of
      ...
      pointers, files:
        begin
          if opcode = cst_ref
            then node := scalar_const (type_ptr, #O377777)
            else node := simple_mem_access (ref);
        end;
      ...
      (* Aggregate references (arrays and records) also fall into two classes.
         ...
         expression (for a flexible aggregate). *)
        ...
        with desc.base^ do begin
          if (base_size <= 72) and not flexible then (* short aggregate, use mem_ref *)
            ...
            if not flexible then (* constant length *)
              ...
              if kind = arrays then (* the parm to SIZE is the upper bound *)
          ...
          if (rhs^.opcode = agg_val) and (not overlaps) then begin
            ...
            if (rhs^.opcode = desc_ref) and (lhs^.opcode = desc_ref) then begin
              ...
              (*  There is no need to compute the length for an array or record
                   move more than once.  If one side is flexible, and
                   the other side isn't, we want to use the length which can be
                   computed more completely at compile time.  *)

              if lhs^.desc.base^.flexible and not rhs^.desc.base^.flexible
...
procedure splitprod (e: expr; var epart: expr; var cmultiplier: integer);
 var c: integer;
 begin
...
procedure splitsum (e: expr; var epart: expr; var caddend, cmultiplier: integer);

 var
   t: expr;
   c: integer;
   enclosing_multiplier: integer;
...
function mulc (e: expr; c: integer; desc: expr_type_desc): expr;
 begin
  if (e = nil) or (c = 0)
...
function sub (e1: expr; cm1: integer; e2: expr; cm2: integer; desc: expr_type_desc): expr;
 begin
...
function add (e1: expr; cm1: integer; e2: expr; cm2: integer; desc: expr_type_desc): expr;
 begin
  if (e1 = nil) or (cm1 = 0)
...
function addc (e: expr; cm: integer; cst: integer; desc: expr_type_desc): expr;
 var t: expr;
 begin
  if (e = nil) or (cm = 0)
      ...
      case opcode of
        ...
        imin_op, imax_op:
          begin
            ...
            if opcode = imin_op
              then tm := maximum (integer)     (* get boundary value *)
              else tm := minimum (integer);
