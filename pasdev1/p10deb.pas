...
static var
    ...
    deb_file: file of integer; (* .DEB file for debugger *)
...
const
    loc_prog_block := #O400001;  (* follows module word *)
...
$PAGE reserve
  ...
  function reserve (size_required: integer): deb_file_index;
  begin
...
$PAGE place_entry
  ...
  procedure place_entry (size: integer; offset: deb_file_index);
...
$PAGE sym_size, value_size, typ_size, blk_size, nam_size
  ...
  function nam_size (n: nam): integer;

  begin
    nam_size := 2 + (length (n^.tex4) div 5;
  end;

  function sym_size (s: sym): integer;
  begin
    with s^ do
      case kind of
  ...
  function typ_size (t: typ): integer;
  begin
    case t^.kind of
      bools:
  ...
  function blk_size (b: blk): integer;
  begin
    case b^.kind of
      root_blk:
...
$PAGE rebush_nam_tree
  ...
  var index, limit: integer;
    ...
    function count_nodes (name: nam): integer;
    ...
    function rebush (left, right: integer): nam;
    ...
    if b <> nil then with b^ do begin
      if visited <> relocating then begin
        ...
        if relocating then begin
          ...
          if kind in [program_blk, module_blk, subr_blk] then
...
begin
  ...
  if b <> nil then with b^ do begin
    case kind of
      root_blk, program_blk, module_blk, subr_blk: begin
