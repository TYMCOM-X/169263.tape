  ...
  begin
    with desc do begin 
      ...
      if (dkind = int_dt) andif                 (* integer value *)
         ...
         (ext$scalar >= #O400000) then           (* value is negative *)
        ext$scalar := ext$scalar - #O1000000;
...
(* COMP$TYPES checks to see if two descriptors are value compatible, i.e. the types of the
   ...
   depending on the value of the RIGHT object aarticular time.  *)
...
public function get$element$addr (    desc:  descriptor;
                                      index: machine_word): addr_record;

  var
    lower_bound:          machine_word;
    elem_size,
    ...
    with addr, display_rec do begin
      ...
      case id_class of
        ...
        constant_sc:                            (* constants *)
          if id_offset >= #O400000 then          (* constants may be allocated in low seg, *)
            wordoffset := prog_blk^.hiseg_base + id_offset - #O400000    (* if overlay compilation *)
          else
