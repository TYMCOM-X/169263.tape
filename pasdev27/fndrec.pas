...
external procedure use_pattern ( pattern );
external procedure free_pattern ( pattern );
$PAGE declarations
...
begin
  ...
  repeat
	...
	if add_state^.right <> nil then
      ...
      eql:
	free_pattern (new_ch); (* Pattern already has a state. *)
