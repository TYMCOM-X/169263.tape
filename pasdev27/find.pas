...
begin
  ...
  while def <> limit do begin
    next_def := def^.next;
    free_pattern (def^.value.pattern);
    dispose (def);
    def := next_def;
  end;
end (* pop_definitions *);
...
begin
  dest := source;
  use_pattern (dest.find_pattern);
  use_pattern (dest.alpha_pattern);
end;
...
begin
  with rec do begin
    ...
    free_pattern (find_pattern);
    free_pattern (alpha_pattern);
  end;
end (* release_options *);
...
type token_type =
	...
	acct,	pattern_nt,	pat_term_nt,	pat_fact_nt,
...
begin
  while stk_ind <> 0 do begin
    if state[stk_ind] <> 27 then (* State 27 is a number state. *)
      free_pattern (value[stk_ind].pattern);
    stk_ind := stk_ind - 1;
  end;
  if pure_pattern <> nil then
    free_pattern (pure_pattern);
  cmd_error (code, msg);
end;
