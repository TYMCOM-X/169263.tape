...
begin
  ...
  ind1 := 1;
  while (arg_char(n,ind1) = space) or (arg_char(n,ind1) = eol) or (arg_char(n,ind1) = tab) do
    ind1 := ind1 + 1;
  ind1 := ind1 - 1;
  ind2 := arg_length(n);
  while (arg_char(n,ind2) = space) or (arg_char(n,ind2) = eol) or (arg_char(n,ind2) = tab) do
    ind2 := ind2 - 1;
  len := ind2 - ind1;
...
begin
  ...
  with make_definition^ do begin
    str_length := def_length;
    if str_length = 0 then return;
    blk_len := 0;
    str_ind := 1;
...
begin
  ...
  with defn^ do begin
	...
	with def_text^ do begin			(* Copy the definition to the input stack. *)
	  low := 1;
	  while low <= str_length do begin
	    size := str_length - low + 1;
	...
	begin
	  ...
	  with def^ do begin
		...
		new (text:str_length);
		text^.str_length := str_length;
		text^.str_text [1:str_length] := substr(str_text,1,str_length);
	      end
	    else
	      text := nil;
