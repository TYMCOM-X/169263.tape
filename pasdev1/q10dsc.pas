      ...
      if not cst_addr (base_desc.text_addr) then begin
        if index_const then begin
          ...
          if index_value <= 6 then
            gen_ri (hrli, reg, #O440700 - (index_value - 1) * #O70000)
          else begin
            taddr := absolute_reference;
            taddr.reloc := gen_blt (#O440700 - ((index_value - 1) mod 5) * #O70000, none,
...
const
  one_arg: val := (scalar_cst, -#O1000000);      (* [-1,,0] *)
...
begin
  ...
  if (actual_desc.base_addr.mode = byte) and
     not ((formal_kind = nonvarying) and formal_is_flex) then
    no_copy := false (* only formal that is nonvarying flex can take byte pointer *)
  else
 ...
 begin
  with node^ do begin
   ...
   if desc.base^.packable
	...
	if file_str_desc.type_desc.kind = chars then
	  file_str_desc.text_addr := increment_addr (file_str_desc.text_addr, 0, 22, 7)
	else
