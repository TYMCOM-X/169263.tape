...
(*   +--------------------------------------------------------------+
     ...
     STARTED:  12/03/79  (copied from p10csp)
  ...
  while (lookup <> nil) andif not found do
    with lookup^ do begin
      if not deleted then begin
        ...
        found := (cur_code^.fwd = key) and (cur_code^.kind = key_kind)
      end;
...
begin
  case const_rec^.kind of
    ...
    fullword,
    realword:    byte_length := 4;
    quadword,
    doubleword:  byte_length := 8;
    stringword:  byte_length := 2 + const_rec^.stringlen;
    pstringword: byte_length := const_rec^.stringlen;
    others:      assert (false)
  end
end;
$PAGE pool_cons
...
begin
  ...
  entry := lookup (lwd_code^.fwd, lwd_code^.kind);
  if entry <> nil then 				(* if record was already in table *)
    ...
    index := abs ( lwd_code^.fwd mod (lwd_tab_max + 1) );
    new ( entry );
...
begin
  ...
  while cur_code <> nil do begin		
    def_mark := cur_code;
    While CUR_CODE^.KIND = DEFMARK Do	(* Ignore 1st defmark, rest null recs *)
      CUR_CODE := CUR_CODE^.NEXT;
    end_of_const := cur_code;
    ...
    if (cur_var_size <> nil) andif (cur_length = cur_var_size^.num_bytes) then begin
	  ...
	  while fixed_link <> nil do begin
		...
		len_word := len_word^.next;
	      if substr (cur_fixed_code^.strvalue, 1, cur_fixed_code^.stringlen) =
	         substr (len_word^.next^.strvalue, 1, len_word^.next^.stringlen) then begin
		if len_word^.next^.kind = stringword then begin 
		  ...
		  temp_code^.word_value := len_word^.next^.stringlen;
  ...
  while cur_link <> nil do begin		(* process 1 link record per iteration *)
	...
	if cur_link_word^.kind <> code_record^.kind then
	  ...
	  case code_record^.kind of
	    ...
	    doubleword,
	    setbyte:
	      equal := cur_link_word^.fwd = code_record^.fwd;
  
	    pstringword,
	    stringword:
	      equal := substr (cur_link_word^.strvalue, 1, cur_link_word^.stringlen)  =
		       substr (code_record^.strvalue, 1, code_record^.stringlen);
...
procedure align (initial_ic: unit_range);
  ...
  procedure chain_into_cst_area (var list_head, list_tail: code; comment_str: string);
	...
	ctext_len := length (comment_str);
	ctext [1: length (comment_str)] := comment_str;
	next := list_head
      end;
  ...
  begin
    ...
    cons_comment^.ctext_len := length (area_description);
    cons_comment^.ctext [1:length (area_description)] := area_description;
	...
	ic := ic + cur_code^.stringlen;	(* ... and if so, assure its alignment   *)
...
begin
  ...
  while cur_link <> nil do begin		(* process 1 'others' constant per iteration *)
	...
	if ((temp_code^.kind in [fullword, realword, doubleword]) and ((ic mod 4) = 0))
	  ...
	  entry := lookup (temp_code^.fwd, temp_code^.kind);
