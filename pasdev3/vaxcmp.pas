...
$PAGE succ_or_fail_branch, make_rangeset
...
(* MAKE RANGESET allocates a set temporary and builds the range set in it.  *)
  
procedure make_rangeset (set_op: expr; (* Expr tuple range_set is value of.   Only used
					  here to get at type info in the expr_type_desc.  *)
			 var range_set: set_desc; (* "O" format set descriptor. *)
			 ctxt_base: set_len_range); (* Contextual lower bound to be used. *)
  
  var
    temp_set: set_desc;
    temp: addr_desc;
    max_len, const_upb: set_len_range;
  defn: def;
...
procedure set_equality (test: expr; negate: boolean; failure_loc: def);

var
  cmp: cmp_codes;
  temp_expr: expr;
  first_set, second_set, temp_set: set_desc;
  const_bytelen, ctxt_base, const_upb: set_len_range;
  temp: addr_desc;
...
begin
  ...
  first_set  := set_fetch (test^.operand[1], 0, maximum (set_len_range), no_preference, false);
  second_set := set_fetch (test^.operand[2], 0, maximum (set_len_range), no_preference, false);
  ...
  case second_set.nargs of
	...
	if adr_equal (first_set.arg[1], first_set.arg[2]) and
	  ...
	  succ_or_fail (bneq, cmp = eqc, failure_loc, success_loc);
	...
	make_rangeset (test^.operand[2], second_set, ctxt_base);
...
procedure set_inclusion (test: expr; negate: boolean; success_loc: def);
...
var
  ...
  ctxt_base, const_upb, first_len, second_len, loop_len: set_len_range;
  fixed: boolean;
  ...
  case first_set.nargs of
	...
	case second_set.nargs of
	  ...
	  3: begin (* [x..y] <= <set> *)
	    ...
	    make_rangeset (test^.operand[1], first_set, ctxt_base);
...
procedure test_and_branch (test: expr; negate: boolean; branch_loc: def);
...
begin
  case test^.opcode of
    ...
    in_op: begin
      ...
      if (max_val - set_lwb) >= maximum (set_len_range) then
	set_len := maximum (set_len_range)
      else
...
procedure test_single_and_jump (test: expr; jump_if_false: boolean; tloc, floc: def);
 var tcmp: cmp_codes;
 begin
  with test^ do begin
