...
const
  ...
  option=       'AA';                   (* com/char protocol value *)
...
$IFNONE(ADP,TYMSHARE) (*$ Enable one of the switches: ADP or TYMSHARE *)
    ascii[ chr(i) ] := 32;
...
begin
  if eoln ( infil ) then readln ( infil );
  if not eof ( infil ) then begin
    ...
    while not eoln ( infil ) do begin
      read ( infil, rec[ recsiz + 1 ] );
      recsiz := recsiz + 1;
    end;
    if response [ 2 ] = 'B' then begin
      q := false;
      while not q and (recsiz > 0) do
        if rec[ recsiz ] = ' '
          then recsiz := recsiz - 1
          else q := true;
    end;
