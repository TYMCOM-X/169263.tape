...
function element ( s: val; i: integer ): boolean;
begin
  with s.valp^ do
...
procedure scal_const ( s: integer );
begin
...
procedure intgr_const ( i: integer );
begin
...
$PAGE set_combine - in fold_binop
  ...
  var min_range, max_range, ind: integer;
  begin
  ...
  case opcode of
    ...
    igt_op:
      if not leftop^.desc.signed andif (rightval.ival < 0) then
        bool_const (true);
    ige_op:
  f not leftop^.desc.signed andif (rightval.ival <= 0) then
        bool_const (false);
    ieq_op:
      if not leftop^.desc.signed andif (rightval.ival < 0) then
        bool_const (false);
...
var
    tag_val: integer;                          (* Value of a variant tag. *)
...
$ENDIF
          if constp(base_array) and constp(index_val) then
            fold := cst_expr (
                  base_array^.cst_val.valp^.elem_vals
					      [index_val^.cst_val.ival -
						base_array^.desc.base^.index_type^.minval + 1],
