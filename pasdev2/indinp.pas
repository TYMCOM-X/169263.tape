...
type
  ...
  SYM_X_TYPE = array [1..MAX_SYM] of TOKENTYPE;
  SYM_TAB_TYPE = array[SYM_SUBRANGE] of TOKENTYPE;
  SYM_SUB_SET = set of SYM_SUBRANGE;
  SYM_TOKEN = array[TOKENTYPE] of SYMBOLS;
  SCAN_TEXT = string[LINLEN];

const
  EOL_CH: char := chr(1);		(* SOH as endofline character *)
  MAX_RES_WD := 63;			(* number of reserved words + 10 *)

  SYM_LIST: SYM_L_TYPE := (
    ':=', '**', '(*', '*)', '!!', '<>', '<=', '>=', '..'   );

  SYM_LIST_SYM: SYM_X_TYPE := (
    ASSI_OP, EXPO_OP, OPEN_COM_OP, CLOSE_COM_OP, CONCAT_OP, NE_OP,
...
public const				(* so mainline can see it *)
  ...
  SYM_TO_TOKEN: SYM_TOKEN := (		(* table of symbols to tokens *)
    LOGCONSY, LOGCONSY, LOGCONSY, LOGCONSY, NOTSY, ARRAYSY, BEGINSY,
    CASESY, CONDSY, CONSTSY, COMPILSY, COMPILSY, COMPILSY,
    ARITHWORDSY, ARITHWORDSY, DOSY, TOSY, TOSY, ELSESY, ENDSY, EXITSY,
    EXTERNALSY, CLASSY, CLASSY, FILSETSY, FILSETSY, FORSY, PSEUDOSY,
    PSEUDOSY, FUNCTIONSY, GOTOSY, IFSY, INSY, LABELSY, ITERSY, ITERSY,
    OFSY, ONSY, OPTIONSY, OTHERSY, PACKEDSY, PRECSY, PROCSY, RECORDSY,
    TERMINSY, TERMINSY, STRINGSY, THENSY, TYPESY, UNTILSY, VARSY,
    W_DOSY, W_DOSY, ASSIGNSY, ARITHSY, ARITHSY, ARITHSY, ARITHSY,
    ARITHSY, ARITHSY, ARITHSY, ARITHSY, ARITHSY, ARITHSY, ARITHSY,
    ELLIPSISY, COLONSY, EQUALSY, LPARENSY, RPARENSY, COMMASY, LBRACKSY,
    RBRACKSY, SEMISY, DEREFSY, PERIODSY, ETC, ETC, ETC, ETC
  );
...
public var
  NEXT_PAGE: boolean;			(* true if NEXT_SYM is '$PAGE' *)
...
const
  ...
  (* The token types corresponding to each entry in the lex table above *)
    ...
    ETC_OP, OF_OP, TO_OP, IN_OP, IF_OP, OR_OP, DO_OP, ON_OP,
    ETC_OP, VAR_OP, END_OP, AND_OP, FOR_OP, SET_OP, DIV_OP, MOD_OP,
	    NOT_OP,
    ETC_OP, TYPE_OP, THEN_OP, ELSE_OP, EXIT_OP, WITH_OP, LOOP_OP,
	    ORIF_OP, CASE_OP, GOTO_OP, FILE_OP, STOP_OP, PREC_OP,
    ETC_OP, CONST_OP, BEGIN_OP, WHILE_OP, UNTIL_OP, LABEL_OP,
	    ANDIF_OP, ARRAY_OP,
    ETC_OP, STATIC_OP, REPEAT_OP, MODULE_OP, EXTERN_OP, PACKED_OP,
	    RECORD_OP, PUBLIC_OP, DOWNTO_OP, OTHERS_OP, STRING_OP,
	    RETURN_OP,
    ETC_OP, FORWARD_OP, PROGRAM_OP, OPTION_OP,
    ETC_OP, FUNCTION_OP, EXTERNAL_OP,
    ETC_OP, PROCED_OP, COND_OP,
    ETC_OP, DATAMOD_OP,
    ETC_OP
    );
...
var
  ...
  begin
  WORD_LOOKUP := ETC_OP;		(* set now in case IDENT bad length *)
  LENG := length(IDENT);
...
public function SYM_LOOKUP (SYMB: SCAN_TEXT): TOKENTYPE;
...
(* SCAN_NEXT is the token scanner for INDENT. It recognizes all PASCAL
   ...
   on NEXT_SYM, NEXT_TOK_LEN, and NEXT_EOL_CT; in other words, the
  ...
  if NEST_LEVEL = 0 then		(* if 0, perform scan *)
	...
	if (NEXT_TEXT <> '.') or
	  ...
	  if CH = '.' then
	    begin
	    NEXT_SYM := ELLIPSISY;
	    NEXT_TOKEN := ELLIPS_OP;
	    NEXT_TEXT := '..';
	    GET_CH
	    end
	  else begin
	    NEXT_SYM := PERIODSY;
	    NEXT_TOKEN := PERIOD_OP
	    end
	  end;
	...
	if (NEXT_TEXT[1] >= '!') and
	  ...
	  if NEXT_TEXT[1] in VALIDFIRSTS then
	    begin
	    NEXT_TOKEN := SYM_LOOKUP (NEXT_TEXT || CH);
	    if NEXT_TOKEN <> NO_GOOD then
	      begin
	    ...
	    else NEXT_TOKEN := SYM_TABLE[NEXT_TEXT[1]]
	    end
	  else NEXT_TOKEN := SYM_TABLE[NEXT_TEXT[1]]
	  end
	else NEXT_TOKEN := NO_GOOD;

	if NEXT_TOKEN <> OPEN_COM_OP then
	  NEXT_SYM := SYM_TO_TOKEN[NEXT_TOKEN]
	else begin
	  if NEXT_TEXT = '%' then
	    NEXT_TEXT := '(*';
	  ...
	  ((NEST_LEVEL > 0 ) or		(* and either isn't closed yet... *)
	   (CH = EOL_CH)) then		(* or end of line reached while proc *)
	    NEXT_SYM := SA_COM	(* then it's a stand-alone com. line *)
	  else NEXT_SYM := OPEN_COM	(* else it's an embedded *)
	  end
      end (* others *)
    end (* case and then *)
    ...
    GET_COM;
    if THIS_SYM <> SA_COM then
      begin
      if NEST_LEVEL = 0 then
	NEXT_SYM := CLOSE_COM
      else NEXT_SYM := CONT_COM
      end
    else NEXT_SYM := SA_COM
    end;
...
(* CHECK_BEFORE enforces the user directive BEFORE(...) by insuring that
   the text of THIS_SYM is followed by at least one space when THIS_SYM
  ...
  begin
  if EOL_CT = 0 then			(* don't bother if THIS_SYM first *)
    begin
    if NEXT_TOKEN in TOK_BEFORES then	(* then it requires *)
...
(* options dump(ifm,final,opt,shape)  *) ;
...
(* INDINP is the coroutine living in the environment READER, performing
   ...
   completely processing NEXT_SYM when resuming SPLITR with THIS_SYM.
   Therefore, an extra input buffer is required -- input cannot be
   read directly into LINE. SCAN_NEXT reads NEXT_SYM into NEXT_TEXT,
   and INDINP places this text into line before resuming to SPLITR. *)

  begin					(* initialize *)
  THIS_SYM := ETC;  NEXT_SYM := ETC;
  CURSTART := 1;  CUREND := 1;
  LINE := ' ';
  ...
  GET_CH;				(* read in first char to start off *)
    ...
    loop
      if eof(INFILE) then
	begin				(* got eof on last NEXT_SYM scan *)
	EOL_CT := 1;			(* only one end of line to follow *)
	resume(SPLITR);			(* it's now THIS_SYM -- send it *)
	if THIS_SYM in [SA_COM, DIRECTIVE] then
	  begin				(* must clear out buffered comment *)
	  THIS_SYM := OPEN_COM;	(* will slip through WRITER *)
	...
	detach				(* and we're all done *)
      ...
      SCAN_NEXT;			(* process NEXT_SYM *)
      CHECK_BEFORE;			(* make sure THIS_SYM has trailing
					   space if NEXT_SYM needs leading *)

      (* at this point, THIS_SYM and NEXT_SYM are ready. *)
	...
	CUREND := 0
      ...
      (* now place NEXT_SYM as THIS_SYM. *)
      ...
      (* check 'after' spacing on the new THIS_SYM now. *)
