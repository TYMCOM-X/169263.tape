...
(* QEDCL.PAS - modified 10/07/81 by djm to replace the old MASK/UNMASK/
               PUSHESCAPE/ESCPOP/FIRESCAPE attention handling with the new
               Exception Handling constructs available in Pascal.  *)
(*           - modified 10/07/81 by djm to add SET CASE code borrowed from
               P. Lee. *)
(*           - modified 10/12/81 by djm to change chr(11b) to tab and 
               chr(10) to lf. *)
(*           - modified 04/29/82 by djm to change all calls to the old
               lookup procedures to calls to the new lookup procedure. *)
...
(*     	     - Modified 7/28/82 by WNH to allow ANC to use QEDLIB
	       routines with either the terminal or the LIFE
	       interface.  (References to tty and ttyoutput made to
...
begin
  ...
  begin
	...
	begin
      ...
      if not ( ((nld = 0) and (cmd in [bound, writecmd, save])) or
	       ((range.lbound = (qfirst_val(buffer) - 1)) and (cmd = append))		)
  ...
  begin
    if buffer.changes
      then discard_changes := query (prompt [changes])
  ...
  function do_delete (range: ldrange): boolean;
   ...
   begin
    with range do begin
      cnt := hbound - lbound + 1;
      do_delete := (cnt < qdata.maxdel) orif query (numtochar (cnt, 0) || prompt[lines]);
  ...
  procedure print_times (num: qlineno);
   begin
    if num = 1 then
     begin
     ttwrite (numtochar (num, 0)); ttwrite (prompt [qptime]);
     end
    else
     begin
     ttwrite (numtochar (num, 0)); ttwrite (prompt [qptimes]);
     end;
    ttwrite ('.'); ttwtln
   end;
...
1:						(* each pass over following code parses and executes one
  ...
  if not command (line, lindex, cmd) then begin
    if (lindex > length (line)) orif (line[lindex] = ';') then begin
      if nld = 0 then begin			(* no ld - print next linecount lines *)
	if findflag then
	...
	termlstlns (buffer, fno, lno, true, false, qdata.tabprint, err);
	chkerr;
  ...
  case cmd of
	...
	if nld > 0 then begin
	  if have_file then
	    ...
	    termlstlns (buffer, cmdrange.lbound, cmdrange.hbound, false, true, true, err);
	  chkerr;
	...
	(* list entire file, numbered by section *)
	  ...
	  loop					(* one pass certain, since no addr error => nonzero line cnt *)
		...
		termlstlns (buffer, 1, lno-fno+1, false, true, true, err);
              chkerr;
	...
	chkerr;
	confirm := query (prompt [qpconfirm]);
	idx := cmdrange.lbound;
	...
	spreddispose (pred);
        if confirm 
          then ttwrite (prompt [f_and_c])
          else ttwrite (prompt [found]);
        ttwrite (numtochar (cnt, 0));
        if cnt = 1 then ttwrite (prompt [qptime])
        else ttwrite (prompt [qptimes]);
        ttwrite ('.');
        ttwtln;
        lindex := length (line) + 1;
      end;					(* find *)
	...
	if (total <= 2) orif query (numtochar(total, 0) || prompt [lines]) then
	begin
    ...
    eqcmd:
      begin
        ck_extra_txt;
        ttwrite (numtochar (cmdrange.lbound, 0)); ttwtln
      end;
	...
	if not qdata.openfileopen then
	  err := qnotopen
	else qlistlines (buffer, cmdrange.lbound, cmdrange.hbound, openfile, false, false, false, err);
        chkerr
      end;					(* outputcmd *)
    ...
    print:
      begin
        ck_extra_txt;
	termlstlns (buffer, cmdrange.lbound, cmdrange.hbound, true, false, qdata.tabprint, err);
        chkerr;
	buffer.curlineno := cmdrange.hbound
      end;					(* print *)
	...
	case setopt of
	  ...
	  mark_param:
	    begin
	      if not spredparse (line, lindex, pred, qdata.wildswitch, err) then seterr(qnoparamval);
              ck_extra_txt;
	...
	termlstlns (buffer, buffer.curlineno, buffer.curlineno, true, false, qdata.tabprint, err);
        chkerr
      end;					(* uparrow *)
  ...
  exit if err = qquit;
    ...
    exception
      storage_overflow: begin
			  ...
			  buffer.curfile := '';  (* don't let him ruin his file unless he wants to *)

                          ttwrite (prompt [hp_ovflw1]);
                          ttwtln;
                          ttwrite (prompt [hp_ovflw2]);
                          ttwtln;
                          ttwrite (prompt [hp_ovflw3]);
                          ttwtln;
