...
begin
  ...
  if not data_type^.flexible then begin
	...
	the upper bound, and the number of elements is (upb-lwb+1).
...
procedure map_array
      ( elem_type: typ;                 (* type of elements of array *)
        ...
        nelems: integer;               (* number of elements in array *)
...
var
  elems_per_ef: integer;                     (* number of elements per extraction field *)
...
begin
  with field^ do begin
	...
	is valid.  If EFA < EFW, extraction field boundaries may be disre-
...
(*  MAP PARM LIST determines the total space that must be allocated for the
    ...
    to the parameter.  Flexible parameters have additional space allocated for
    their descriptor words; the descriptor word may be found by taking the
    location of the parameter, and subtracting the size of the descriptor.  *)
...
begin
  ...
  with subr_type^ do begin
	...
	if parm_type^.flexible then begin
	  ...
	  with desc_rule^ do
	    ploffset := ngm (ploffset, alignment) + width_factor;
	end;
