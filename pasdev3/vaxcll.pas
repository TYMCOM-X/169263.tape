...
Type PARAM_REC = Record
	...
	DESC    : ADDR_DESC		(* The address descriptor *)
     ...
     PARAM_ARY = Array [ PARM_INDEX ] of PARAM_REC;
     param_ptr = ^param_ary;
...
$PAGE push_parameters - in pas_call
  ...
  begin
    with node^ do begin
      if selector < nargs then	(* find end of parameter chain *)
...
begin (* pas_call *)
  ...
  if node^.nargs > 0
    Then Begin
      ...
      New ( proc_params : NODE^.NARGS*2 );	(* Will always be <= 2*num of args *)
...
begin
  with node^ do begin
    ...
    gen2 (calls, int_value (nargs), fetch (subr,no_preference));
  end;
end;
$PAGE procedure_call
