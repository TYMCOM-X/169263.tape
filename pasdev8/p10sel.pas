...
function agg_reference (agg: expr): expr; forward;
...
const
  ...
  str_len_desc: expr_type_desc := (nil, ints, false, 18);
  str_1_desc: expr_type_desc := (nil, strings, nonvarying, false, 0);
  set_len_desc: expr_type_desc :=  (nil, ints, false, 18);
  set_desc: expr_type_desc := (nil, sets, true, true, minimum (set_element_range), max_set_length);
...
function i_context (node: expr): align_range;
 begin
  if node = nil
    then i_context := maximum (align_range)	(* there is no constraint *)
	...
	i_context := maximum (align_range);			(* context does not constrain the value *)
...
procedure apply_constraints;
 ...
 begin
  ...
  while node <> nil do begin
	...
	expii_op:
	  begin
	    desc.int_prec := min (mode (node), i_context (node^.result));
	    for i := 1 to nopers do set_i_context (operand [i], node);
	  end;
	...
	sub_range_chk, str_range_chk, val_range_chk, fld_chk, compat_chk:
	  begin
	    for i := 1 to nopers do set_i_context (operand[1], node);
	  end;
	...
	sclcvt_op:
	  begin
	    if desc.kind = pointers
	      ...
	      else desc.int_prec := i_context (node^.result);
	    set_i_context (operand[1], node);
	  end;
	...
	index_op, search_op, verify_op:
	  begin
	    if nopers = 2
	      then desc := str_len_desc	(* at least as good as any contextually derived limit *)
	      else begin				(* failure result specified *)
		desc.int_prec := min (mode (operand[3]), i_context (node^.result));	(* result is unlimited *)
...
procedure transform_operands (t: expr);
 ...
 begin
  with t^ do begin
	...
	for i := 1 to nopers do operand[i] := transform (operand[i]);

      size_op:
	type_parm := transform (type_parm);

      call_op, signal_op, func_call_op, func_signal_op:
	begin
	  subr := transform (subr);
	  for i := 1 to nargs do arglist[i] := transform (arglist[i]);
	end;

      start_with:
	with_rec := transform (with_rec);

      jump_t_op, jump_f_op, jump_in_op, case_jump_op:
	cond := transform (cond);
...
(* INIT NODE initializes a new expr node, with a specified type descriptor, and
   ...
   the right place.  In a few cases, oors are patched in out of order; 
...
procedure const_access (value: val; node: expr);
 ...
 begin
  with node^ do begin
	...
	next_val := cur_block^.first_value_node;
	cur_block^.first_value_node := vn;
	def_addr := nil;
      end;
      item.cstref := vn;
    end
...
function op1 (opc: tuple_opcodes; desc: expr_type_desc; o1: expr): expr;
 begin
...
function op2 (opc: tuple_opcodes; desc: expr_type_desc; o1, o2: expr): expr;
 begin
...
function op3 (opc: tuple_opcodes; desc: expr_type_desc; o1, o2, o3: expr): expr;
 begin
...
procedure parm_access (var node: expr; parm: sym);
 begin
  with node^ do begin
    item := parm_item;
...
$PAGE runtime_size
(* RUNTIME SIZE creates a tree which computes the size function.  "Type_ref" is
   the type whose size is to be computed.  If "type_ref" is a flexible type, then
   "type_parm" will be an expression giving the flexible array upper bound.  If
   "type_ref" is a generic type, then "type_parm" will be an expression giving
   the generic array *dimension*. *)

function runtime_size (type_ref: typ; type_parm: expr): expr;
 var
  cst_size: int_type;			(* constant part of size *)
  align: align_range;
  expr_size: expr;			(* tree computing variable part *)
  flex_type: typ;			(* flexible type of parameterized size *)
  fld: sym;
  units_per_elem: bit_range;		(* non packed element size in units *)
  elems_per_unit: bit_range;		(* packed: # of elements in one unit *)
  lwb: int_type;			(* lower bound of flex type *)

 begin

  (* Get the constant part of the size, rounding up to the nearest full unit
     size.  In the case of flexible types, this will include space for the
     descriptor and length words. *)

  cst_size := (type_ref^.size + bits_per_unit - 1) div bits_per_unit;

  (* If there is a type parameter, then we have an array or string type, or
     a record/variant whose last field has a flexible string or array type.
     The size of the array is calculated and placed in expr_size.  Note that
     the cst_size already accounts for any descriptor words. *)

  expr_size := nil;			(* implies no variable size until changed *)
  if type_parm <> nil then begin	(* defaults are always supplied in pass 1 if flexible *)
    flex_type := type_ref;		(* find array/string type *)
    if flex_type^.kind in [records, variants] then begin
      fld := flex_type^.field_list;	(* search for trailing field *)
      while (fld^.next <> nil) andif (fld^.next^.fld_variant = fld^.fld_variant)
	do fld := fld^.next;
      flex_type := fld^.type_desc;
    end;

    if flex_type^.kind = strings		(* get size parms for type *)
      then begin
	units_per_elem := 1;
	elems_per_unit := bits_per_unit div type_char^.size;
	lwb := 1;
      end
      else begin			(* arrays *)
	units_per_elem := max (flex_type^.element_size div bits_per_unit, 1);
	elems_per_unit := max (bits_per_unit div flex_type^.element_size, 1);
	if flex_type^.generic
	  then lwb := 1
	  else lwb := flex_type^.index_type^.minval;
      end;

    expr_size := transform (type_parm);
    if lwb <> 1			(* compute the number of elements *)
      then expr_size := op2 (isub_op, index_desc, expr_size, int_const (lwb - 1));

    (* If there is more than one element per unit, then the computation of
       size is:  size := ((parm - lwb + 1) + (n - 1)) div n, where n is the
       number of elements per unit. *)

    if elems_per_unit > 1 then begin
      expr_size := op2 (iadd_op, index_desc, expr_size, int_const (elems_per_unit - 1));
      expr_size := op2 (idiv_op, index_desc, expr_size, int_const (elems_per_unit));
    end

    (* Otherwise, the size computation is size := (parm - lwb + 1) * units_per_elem.
       Notice, that when either we have one packed element in a unit, or an
       unpacked element occupying only one unit, then size := (parm - lwb + 1). *)

    else if units_per_elem > 1
      then expr_size := op2 (imul_op, index_desc, expr_size, int_const (units_per_elem));
  end (* if type_parm <> nil *) ;

  if expr_size = nil then			(* form total result and return *)
    runtime_size := int_const (cst_size)
  else if cst_size = 0 then
    runtime_size := expr_size
  else
    runtime_size := op2 (iadd_op, index_desc, expr_size, int_const (cst_size));
...
procedure ident_access (ref: expr; node: expr);
 ...
 begin
  ...
  with node^ do begin
	...
	begin
	  if prog_options.overlay_opt and
 ...
 begin
  with ref^ do begin
    ...
    if arraytype^.generic then begin
      ...
      if multiplier <> 1
	then lwb := op2 (imul_op, index_desc, lwb, int_const (multiplier));
      base_idx := op2 (isub_op, pointer_desc, base_idx, lwb);
    end
    ...
    if multiplier <> 1
      then idx := op2 (imul_op, index_desc, idx, int_const (multiplier));
	...
	if desc.kind = pointers
	  then idx :=				(* index is just base addr *)
	    op2 (iadd_op, index_desc, idx, base_idx)	(* by convention move base ptr to right *)
	else if (opcode = iadd_op) andif (operand[2]^.desc.kind = pointers)
	  then idx :=	(* index is (sub + baseaddr) *)
	    op2 ( iadd_op, index_desc,
		     op2 (iadd_op, index_desc, operand[1], idx),
		     operand[2]					 )
	else idx := op2 (iadd_op, index_desc, idx, base_idx);	(* index is simple expr *)
      end (* with *) ;
    end;
 ...
 begin
  with ref^ do begin
    ...
    if arraytype^.generic then begin	(* lower bound is not constant *)
      ...
      node^.item.index := op1 (ineg_op, index_desc, lwb);	(* make it a packed indexed address *)
      bitindirect (node);
    end
...
function addr (ref: expr): expr;
 begin
  ...
  if (ref^.desc.kind in [arrays, records]) andif
    ...
    then access (agg_reference (ref), addr)
  ...
  with ref^ do begin
    if ( ( (desc.kind = arrays) andif desc.base^.flexible ) or
	 ( (desc.kind = strings) andif desc.str_flex ) ) andif
      ...
      if addr^.item.pack
	then addr^.item.offset := addr^.item.offset + flex_arr_desc_size
	else addr^.item.offset := addr^.item.offset + (flex_arr_desc_size div bits_per_unit);
...
function alloc_dynamic_temp (ssize: expr): expr;
 begin
  ...
  with alloc_dynamic_temp^ do begin
    item := var_item;
    item.index := op1 (alc_temp_op, pointer_desc, ssize);
  end;
...
begin
  ...
  (* Otherwise, must dynamically allocate a temporary of the appropriate size
    ...
    op := op2 (iadd_op, set_len_desc, slen, int_const (bits_per_unit - 1));	(* runtime size calc *)
    op := op2 (idiv_op, set_len_desc, op, int_const (bits_per_unit));
  ...
  set_temp := op3 (desc_ref, sdesc, saddr, slwb, slen);
end;
...
function padded_set_operand ( opdesc: expr_type_desc; op: expr ): expr;
 ...
 var
   src: expr;
   len, opreglen: set_element_range;
...
begin
  ...
  with se^ do begin
	...
	begin
	  if nopers = 0
	    then begin				(* no op0 must generate directly *)
	      if tdesc.set_length <= 36
		then begin			(* can be immediate zero *)
		  new (newse, immed_ref, immed_ref);
		  newse^.item := immed_item;	(* value (offset) is zero *)
		end
		else begin
	    ...
	    then newse := op1 (gen_set_op, tdesc, operand[1])
	  else newse := op2 (gen_set_op, tdesc, operand[1], operand[2]);
	end;
	...
	begin
	  if (desc.set_length div bits_per_unit) = (tdesc.set_length div bits_per_unit)
	    then begin
	  ...
	  new (newse, nary_op, nary_op: nopers);	(* rebuild the operation at the derived precision *)
	  newse^.opcode := opcode;
	  newse^.nopers := nopers;
    ...
    if (newse^.desc.set_lwb <> tdesc.set_lwb) or	(* shift required *)
       ...
       ((newse^.desc.set_length div bits_per_unit) <> (tdesc.set_length div bits_per_unit))
      then newse := op1 (setcvt_op, tdesc, newse);	(* or truncation required;  note that zero
...
function implicit_set_op (sdesc: expr_type_desc; se: expr): expr;
 var i: index_range;
 begin
  new (implicit_set_op, set_op, set_op: se^.nopers);
  with implicit_set_op^ do begin
    nopers := se^.nopers;
    for i := 1 to nopers do operand[i] := se^.operand[i];
  end;
...
begin
  ...
  with se^ do begin
	...
	assign (target, implicit_set_op (target^.desc, se));
      ...
      (* For t := f (op1, op2, ... opn), generate t := op1; t := f (t, op2), and
	 ...
	 be evaluated in the registers. *)
      ...
      union_op, diff_op, both_op:
	begin
	  if desc.set_length <= 72		(* then evaluate in regs and put in storage *)
	    ...
	    for i := 2 to nopers do begin
	      if (operand[i]^.opcode = desc_ref) andif (operand[i]^.desc.set_lwb = target^.desc.set_lwb)
		then assign (target, op2 (opcode, desc, target, operand[i]))   (* assign directly to set *)
	      else if (operand[i]^.opcode = mem_ref) andif (operand[i]^.desc.set_lwb = target^.desc.set_lwb)
		then assign (target, op2 (opcode, desc, target, set_reference (operand[i])))
	      else begin
		...
		assign (target, op2 (opcode, desc, target, temp));
	      end;
	    end;
	  end;
	end
  ...
  with se^ do begin
	...
	case opcode of
	  ...
	  union_op, both_op:
	    begin
	      if opcode = union_op
		then new (lwb, imin_op, imin_op: nopers)
		else new (lwb, imax_op, imax_op: nopers);
	      lwb^.nopers := nopers;
	      for i := 1 to nopers do
...
begin
  with se^ do begin
	...
	case opcode of
	  ...
	  desc_ref:
	    begin
	      upb := op2 (isub_op, full_int_desc, operand[3], operand[2]);    (* upb := len - lwb + 1 *)
	      upb := op2 (iadd_op, full_int_desc, upb, int_const (1));
	    end;
	  ...
	  union_op, both_op:
	    begin
	      if opcode = union_op
		then new (upb, imax_op, imax_op: nopers)
		else new (upb, imin_op, imin_op: nopers);
	      upb^.nopers := nopers;
	      for i := 1 to nopers do
...
begin
  with se^ do begin
    if opcode = desc_ref
      ...
      if desc.set_cst_length
	then slen := int_const (desc.set_length)
	else begin (* must compute length at runtime *)
	  slen := op2 (isub_op, set_len_desc, set_upperbound (se), slwb);
	  slen := op2 (iadd_op, set_len_desc, slen, int_const (1))
	end;
	...
	then set_reference := op3 (desc_ref, desc, addr (se), slwb, slen)
...
function long_set_operand (tdesc: expr_type_desc; lwb: expr; se: expr): expr;
 ...
 begin
  with se^ do begin
    ...
    len := op2 (isub_op, full_int_desc, set_upperbound (se), lwb);
    len := op2 (iadd_op, full_int_desc, len, int_const (1));
    len := op2 (imax_op, set_len_desc, len, int_const (0));
 ...
 begin
  with str^ do begin
	...
	str_length := result^.operand[2];	(* result -> desc, op2 is length *)
      ...
      cst_ref:
	str_length := int_const (cst_val.valp^.str_len);
	...
	begin
	  ...
	  for i := 2 to nopers do begin
	    ...
	    str_length := op2 (iadd_op, str_len_desc, str_length, slen);
	  end
	end;
...
function str_temp (desc: expr_type_desc; supb: expr): expr;
 ...
 begin
  ...
  (* If the temporary has a flexible length, then we must compute the unit size
    ...
    op := op2 (iadd_op, str_len_desc, supb, int_const (ssize));
    op := op2 (idiv_op, str_len_desc, op, int_const (chars_per_unit));
    saddr := alloc_dynamic_temp (op);
  end;
  ...
  str_temp := op2 (desc_ref, desc, saddr, slen);
 end;
$PAGE vs_move_length
...
function vs_move_length ( target, source, target_upb: expr ): expr;
 ...
 100 (* no optimization possible *):
  ...
  vs_move_length := op2 (imin_op, str_len_desc, str_length (source), target_upb);
 end;
...
procedure expand_str_move ( target: expr; src: expr; t_len: expr );
 ...
 begin
  ...
  (* If the source is a function call, then the length word of the string may
    ...
    tdesc := op2 (desc_ref, target^.desc, length_word_address (target^.operand[1]),
					  target^.operand[2]);
    assign (tdesc, src);
  end
  ...
  (* If the target is a varying length string, then we must move the source 
    ...
    if t_len = nil then begin
      ...
      mv_len := t_len;
    tdesc := op2 (desc_ref, target^.desc, target^.operand[1], mv_len);
    assign (tdesc, t_src);			(* move to desc_ref (addr (text), upb) *)
    init_node (len, str_len_desc);		(* assign no of chars moved to target length word *)
    assign (len, mv_len);
  end;
 end;
...
function str_char_temporary ( str: expr ): expr;
 ...
 begin
  ...
  assign (temp, str);
  str_char_temporary := op2 (desc_ref, char_desc, temp_addr, int_const (1));
 end;
$PAGE str_reference
 ...
 begin
  with str^ do begin	(* str assumed to be transformed operand *)
	...
	begin
	  ...
	  str_reference := op2 (desc_ref, char_desc, saddr, int_const (1));
	end;
	...
	if transform (str) <> str then (* call has already been evaluated *)
	  ...
	  if desc.kind = strings then begin
	    ...
	    if desc.str_kind = varying then begin	(* must pass address of start of length word *)
	      ...
	      temp := op2 (desc_ref, desc, saddr, slen);
	    end;
	    assign (temp, str);
	  end
 ...
 begin
  if str^.desc.kind in scalar_types then
    ...
    while (base^.opcode = upc_op) or (base^.opcode = lwc_op) do
      base := base^.operand[1];
    sv_str_operand := op1 (upc_op, str^.desc, sv_str_operand (base));
  end
  else
...
procedure str_address (str: expr; node: expr);
 ...
 begin
  ...
  if sdesc^.operand[1]^.desc <> byte_ptr_desc
 ...
 begin
  with ref^ do begin
    ...
    (* If the base string is not a packed reference, then construct a byte
      ...
      node^.item.size := type_char^.size;	(* build char byte pointer *)
      bitindirect (node);
    ...
    if node^.item.index <> nil
      then idx := op2 (iadd_op, str_len_desc, idx, node^.item.index);
    node^.item.index := idx;
    bitindirect (node);
...
function str_bptr ( sdesc: expr ): expr;
 ...
 begin
  ...
  if str_bptr^.item.pack or
     (str_bptr^.desc = byte_ptr_desc) then
...
procedure offset_byte_address (* saddr: expr *);
 ...
 begin
  ...
  if sbp^.opcode = addr_ref then (* can offset directly *)
    ...
    saved_cursor := cursor; (* insert computation out of order *)
    t_set (saddr^.prev);
    saddr^.item.index := op2 (isub_op, index_desc, saddr^.item.index, int_const (1));
    t_set (saved_cursor);
  end;
 end;
$PAGE expand_exp
...
$PAGE iconstp
(* ICONSTP is a predicate which determines if is operand ("node") represents
...
function iconstp (node: expr; var c: int_type): boolean;
 begin
  iconstp := false;
  c := 0;
  with node^ do begin
...
(* ASSIGN AGG VAL assigns an aggregate constructor to a destination aggregate,
   ...
   array accessing operators, and then re-expanded. *)

procedure assign_agg_val (lhs, rhs: expr);
 ...
 begin
  ...
  expand (start_code^.next, cursor^.next); (* expand the assignments into low_level code *)
 end;
$PAGE agg_temp
(* AGG TEMP returns a dynamic temporary of a size appropriate to hold a
   specified aggregate expression. *)

function agg_temp (op: expr): expr;
 ...
 begin
  saved_cursor := cursor;
  t_set (op^.prev);
  asize := runtime_size (op^.desc.base, nil); (* the size of the aggregate *)
  agg_temp := alloc_dynamic_temp (asize); (* allocate a temp of the required size *)
  if op^.desc.base^.size <= 72 then begin
    agg_temp^.opcode := mem_ref;
    agg_temp^.desc := op^.desc;
  end
  else
    agg_temp := op2 (desc_ref, op^.desc, agg_temp, asize);
  t_set (saved_cursor);
 end;
$PAGE agg_reference
...
function agg_reference (* agg: expr): expr *);
 ...
 begin
  if (agg^.result <> nil) andif (agg^.result^.opcode <> agg^.opcode) then
    ...
    agg_reference := agg_temp (agg);
    saved_cursor := cursor;
    t_set (agg);
    assign_agg_val (agg_reference, agg);
    t_set (saved_cursor);
	...
	agg^.result := op2 (desc_ref, desc, operand[1], operand[2])
      else begin
...
procedure expand_io_string (op: expr);
 ...
 var str_block, sb_addr, sb_len, str_desc, str_addr: expr;

 begin
  str_desc := transform (op^.operand[1]);
  ...
  init_node (sb_len, str_len_desc);

  if (op^.opcode = out_str_op) andif
     (op^.operand[1]^.desc.kind = strings) andif
     (op^.operand[1]^.desc.str_kind = varying) then begin

    (* If the operation is PUTSTRING, and the string destination is a varying
       string variable, then we can use the SV form of the i/o call, which
       takes a block containing the string variable address and maximum length. *)

    init_node (sb_addr, pointer_desc);
    new (str_addr, addr_ref, addr_ref); (* get address of length word *)
    init_node (str_addr, pointer_desc);
    str_addr^.item := str_desc^.operand[2]^.item;
    assign (sb_addr, str_addr);
    assign (sb_len, upper_bound (op^.operand[1]));
    assign (str_desc^.operand[2], int_const (0)); (* set the target string to '' *)
    op^.result := op1 (io_var_str_op, file_desc, str_block);
  end

  else begin

    (* Otherwise, we must use the SS form of the i/o call, which takes a block
       containing a byte pointer to the character before the string, and the
       string length. *)

    init_node (sb_addr, byte_ptr_desc);
    assign (sb_addr, str_bptr (str_reference (str_desc)));
    assign (sb_len, str_desc^.operand[2]); (* the string length *)
    op^.result := op1 (io_fix_str_op, file_desc, str_block);
  end;
 end;
 ...
 begin
  with op^ do begin
	...
	begin
	  operand[1] := transform (operand[1]);
	  operand[2] := transform (operand[2]);
	  if iconstp (operand[2], ival) andif (ival >= 0)
	...
	result := upper_bound (operand[1]);
      ...
      dim_op:
	result := op2 ( iadd_op, index_desc,
			op2 ( isub_op, index_desc,
	...
	begin
	 ...
	 compares against the same strings: if s1 < s2 else if s1 >= s2. *)
      ...
      sle_op, slt_op, sgt_op, sge_op, seq_op, sne_op:
	begin
	  operand[1] := op2 (str_comp_op, index_desc,
	...
	begin
	  ...
	  if ((desc.str_kind = nonvarying) and (desc.str_flex))
	    ...
	    then result := 			(* shorten reference to longer nonvarying string *)
		   op2 (desc_ref, desc, operand[1]^.operand[1], int_const (desc.str_length))
	...
	begin
	  ...
	  if operand[2]^.opcode = gen_set_op
	    then begin
	      if operand[2]^.nopers = 0			(* x in [] !!!! *)
		then result := scalar_const (type_bool, ord (false))
	      else if operand[2]^.nopers = 1		(* x in [a] !!!! *)
		then result := op2 (ieq_op, desc, operand[1], operand[2]^.operand[1])
	  ...
	  sdesc.set_length :=
	    max ( operand[1]^.desc.set_lwb + operand[1]^.desc.set_length - 1,
		  operand[2]^.desc.set_lwb + operand[2]^.desc.set_length - 1 )
		    - sdesc.set_lwb + 1;
	  sdesc.set_cst_len := operand[1]^.desc.set_cst_length and operand[2]^.desc.set_cst_len;
 ...
 begin
  case parm_type^.kind of
	...
	if p_b_address (parm_type)
	  ...
	  else outcall^.arglist[parmidx] := agg_reference (arg);
      end;
...
function prepare_call (* incall: tuple): tuple *) ;
 ...
 begin
  ...
  if parmidx <> incall^.nargs then begin
    new (outcall, call_op, call_op: parmidx);
    with outcall^ do begin
      opcode := incall^.opcode;
      nargs := parmidx;
    end;
  end
  else outcall := incall;		(* parmidx = nargs, use same node *)
  ...
  parmidx := 0;
  with incall^.subr^.desc.base^ do begin
    for i := 1 to incall^.nargs do begin
      parmidx := parmidx + 1;
   ...
   if outcall^.subr^.opcode = subr_var_op	(* use desc_ref to tag call to subr constant *)
     then outcall^.subr := op1 (desc_ref, outcall^.subr^.desc, outcall^.subr^.operand[1]);
...
function make_variable_descriptor (str: expr): expr;
 ...
 begin
  ...
  if str^.desc.str_kind = nonvarying then begin
    ...
    else if saddr^.item.pack and (saddr^.desc <> byte_ptr_desc) then begin
  ...
  make_variable_descriptor := op2 (desc_ref, str^.desc, saddr, slen);
 end;
...
function make_substr_descriptor (str: expr): expr;
 ...
 begin
  ...
  make_substr_descriptor := op2 (desc_ref, str^.desc, saddr, slen);
 end;
  ...
  with ref^ do begin
	...
	begin
	  if opcode = cst_ref
	    then begin				(* generate descriptor directly *)
	      ...
	      node := op2 (desc_ref, desc, saddr, slen);
	    end
	  else if opcode = substr_ref
	    then node := make_substr_descritor (ref)
	...
	begin
	  ...
	  if desc.set_length <= 72 then		(* short set, use mem_ref *)
		...
		setlen := cst_val.valp^.set_len;
	      end
	      else begin
		setlwb := desc.set_lwb;
		setlen := desc.set_length;
	      end;
	    node := op3 (desc_ref, desc,
	...
	with desc.base^ do begin
	  if (size <= 72) and not flexible then (* short aggregate, use mem_ref *)
	    node := simple_mem_access (ref)
		...
		size_parm := op2 ( iadd_op, index_desc,
				   op2 ( isub_op, index_desc,
					 upper_bound (ref),
					 lower_bound (ref) ),
				   int_const (1) )
	    ...
	    saddr := addr (ref);
	    slen := runtime_size (desc.base, size_parm);
	    node := op2 (desc_ref, desc, saddr, slen);
	...
	begin
	  if opcode = cst_ref then begin		(* subr_var (addr (subr), display (parent)) *)
	    ...
	    if id^.block^.level <= 1			(* level 1 blocks have no display *)
	      then node := op1 (subr_var_op, desc, node)
	      else node := op2 (subr_var_op, desc, node,
	    ...
	    access (ref, node);
	    init_node (node, pointer_desc);
	    node := op1 (subr_var_op, desc, node);
	  end
      ...
      others:
	node := ref			(* no transformation applied, yet *)
 ...
 begin
  ...
  with op^ do begin
	...
	begin
      ...
      arrays, records:
	begin
	  lhs := agg_reference (lhs);
	  if (rhs^.opcode = agg_val) and (not overlaps) then begin
	    assign_agg_val (lhs, rhs); (* expand assignment directly *)
	    must_dechain := true;
	  end
	  else if (rhs^.opcode = func_call_op) and (not overlaps) then
	    (* the direct assignment is ok *)
	  else begin
	    rhs := agg_reference (rhs); (* create temporary if needed *)
		...
		then len := rhs^.operand[2]
	      ...
	      lhs := op2 (desc_ref, lhs^.desc, lhs^.operand[1], len);
	      rhs := op2 (desc_ref, rhs^.desc, rhs^.operand[1], len);
	    end
	...
	begin
	  ...
	  if lhs^.desc.str_kind = varying then begin
	    ...
	    dest := op2 (desc_ref, dest^.desc, dest^.operand[1], temp);	(* target uses upper bound as length *)
 ...
 begin
  ...
  while op <> last_op do begin
	...
	expand_operator (op);
      ...
      size_op:
	op^.result := runtime_size (op^.type_ref, op^.type_parm);
	...
	with op^ do begin
	  ...
	  case rw_mode of
		...
		if rw_item^.desc.kind in [arrays, records]
		  then rw_item := agg_reference (rw_item)
	...
	begin
      ...
      (* An end string i/o operator is only relevant on a write to a fixed-
	 length string, where it pads the string out with blanks.  Therefore,
	 if the file argument is not a fixed_length string, remove the end
	 string i/o operator. *)

      end_io_op:
	begin
	  temp := transform (op^.file_arg);
	  if (op^.file_arg^.opcode = out_str_op) and
	     (temp^.opcode = io_fix_str_op) then
	    op^.file_arg := temp
	  else begin
	    op := op^.prev;
	    dechain (op^.next);
      ...
      first_chk_op..last_chk_op:
	for i := 1 to op^.nopers do op^.operand[i] := transform (op^.operand[i]);
	...
	op^.target_frame :=
	  display (cur_block^.apparent_level - op^.target_label^.block^.apparent_level);
	...
	begin
	  ...
	  if temp_used then begin		(* must emit a reset stack *)
	    ...
	    until not ( ((first_jump_op <= op^.opcode) and (op^.opcode <= last_jump_op)) or
...
procedure splitprod (e: expr; var epart: expr; var cmultiplier: int_type);
 ...
 begin
  ...
  with e^ do begin
    ...
    else if (opcode = imul_op) andif iconstp (operand[2], c) then begin
      cmultiplier :=  c;
      epart := operand[1];
    end;
  end (* with *) ;
 ...
 begin
  ...
  with epart^ do begin
	...
	caddend := item.cstref^.scalar_val;
      ...
      if iconstp (operand[2], caddend)
	then epart := operand[1];
    end
    else if opcode = isub_op then begin
      if iconstp (operand[2], c) then begin
	epart := operand[1];
	caddend := - c;
      end
      else if iconstp (operand[1], caddend) then begin
	epart := operand[2];
	cmultiplier := -1;
      end
    end
...
function mulc (e: expr; c: int_type): expr;
 begin
  if (e = nil) or (c = 0)
    ...
    then mulc := op1 (ineg_op, op^.desc, e)
  else mulc := op2 (imul_op, op^.desc, e, int_const (c))
 end;
$PAGE sub, add - in simplify
(* SUB generates a subtract of two expr trees with individual constant multipliers. *)

function sub (e1: expr; cm1: int_type; e2: expr; cm2: int_type): expr;
 begin
  if (e1 = e2) and (cm1 = cm2)
    then sub := int_const (0)
    else sub := op2 (isub_op, op^.desc, mulc (e1, cm1), mulc (e2, cm2));
 end;
...
function add (e1: expr; cm1: int_type; e2: expr; cm2: int_type): expr;
 begin
  if (e1 = nil) or (cm1 = 0)
    then add := mulc (e2, cm2)
  else if (e2 = nil) or (cm2 = 0)
    then add := mulc (e1, cm1)
  else if (cm1 < 0) and (cm2 < 0)
    then add := op1 (ineg_op, op^.desc,
			 op2 (iadd_op, op^.desc, mulc (e1, -cm1), mulc (e2, -cm2)) )
  else if (cm1 < 0)
    then add := sub (e2, cm2, e1, -cm1)
  else if (cm2 < 0)
    then add := sub (e1, cm1, e2, -cm2)
  else add := op2 (iadd_op, op^.desc, mulc (e1, cm1), mulc (e2, cm2));
 end;
...
function addc (e: expr; cm: int_type; cst: int_type): expr;
 var t: expr;
 begin
  if (e = nil) or (cm = 0)
    ...
    if cst < 0
      then addc := add (e, cm, int_const (-cst), -1)
      else addc := add (e, cm, int_const (cst), 1);
  end;
 end;
$PAGE simplify - body
 ...
 begin
  ...
  while op <> nil do begin
	...
	iadd_op, isub_op:
	  begin
		...
		then begin
		  ...
		  t := add (e1, cm1 div tm, e2, sign (cm2));
		end
	      else if ((cm2 mod cm1) = 0) and (e1 <> nil)
		then begin
		  tm := abs (cm1);
		  t := add (e2, cm2 div tm, e1, sign (cm1));
		end
	      else begin
		t := add (e1, cm1, e2, cm2);
		tm := 1;
	...
	imul_op:
	  begin
	    ...
	    if ((e1 = nil) or (cm1 = 0)) and ((e2 = nil) or (cm2 = 0))
	      ...
	      then result := addc (e1, cm1 * ca2, ca1 * ca2)
	    else if ((e1 = nil) or (cm1 = 0))
	      then result := addc (e2, cm2 * ca1, ca2 * ca1)
	    else if (ca1 = 0) and (ca2 = 0)
	      then begin
		t := op2 (imul_op, desc, e1, e2);
		result := mulc (t, cm1 * cm2);
	      end
	    else if (ca1 = 0)
	      then begin
		t := addc (e2, cm2 * cm1, ca2 * cm1);
		splitprod (t, r, rm);
		r := op2 (imul_op, desc, r, e1);
		result := mulc (r, rm);
	      end
	    else if (ca2 = 0)
	      then begin
		t := addc (e1, cm1 * cm2, ca1 * cm2);
		splitprod (t, r, rm);
		r := op2 (imul_op, desc, r, e2);
		result := mulc (r, rm);
	      end;
	  end (* imul case *) ;
	...
	idiv_op:
	  begin
	    if iconstp (operand[1], cm1) andif iconstp (operand[2], cm2)
		...
		result := op2 (idiv_op, desc, mulc (e1, cm1 div cm2), e2)
	      else if (cm1 <> 0) andif ((cm2 mod cm1) = 0) then
		result := op2 (idiv_op, desc, e1, mulc (e2, cm2 div cm1));
	    end;
	  end;
	...
	ineg_op:
	  begin
	    ...
	    result := addc (e1, -cm1, -ca1);
	  end;

	iabs_op:
	  begin
	    splitprod (operand[1], e1, cm1);
	    t := op1 (iabs_op, desc, e1);
	    result := mulc (t, abs (cm1));
	  end;
...
$PAGE prologue
(* PROLOGUE generates uses of special values in an imaginary block following
...
public procedure shape;
 begin
  prologue;				(* generate references to parms, cur display *)
