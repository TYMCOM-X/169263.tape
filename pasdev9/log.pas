$options main, sp, noc, storage = 10000
$PAGE includes
$system pasdcl.inc
$system pashak.inc
$system rlb:dtime.typ
$system rlb:dtime.inc
$system paslog.typ
$system rlb:cmdutl.typ
$system rlb:query.inc
$PAGE declarations
const
    page_len := 45;
    user_header :=
...
type
    ...
    log_file_type= file of log_file_stats;
...
var
    ...
    last_date_reported: date_int:= minimum (date_int);
    last_date: dtime_int;
...
function width8 (i: integer): integer;
begin
  width8 := 0;
  repeat
    width8 := width8 + 1;
...
function num_string (val: integer; radix: integer): string_6;
var
  i, ix: integer;
...
const radix: radix_base := ("10, 10, 10b);
procedure ival (
	var f: text;
	must_print: boolean;
	value: integer;
...
begin
  with current_record do begin
	...
	runs_err_free := runs_err_free + 1;
      ...
      net_hiseg := net_hiseg + hiseg_size;
      ...
      n_max_hiseg := max (n_max_hiseg,hiseg_size);
...
procedure drop_total (skip: integer; header: string; compiles: integer; rtime: real);
var i: integer;
    r: real;
begin
...
begin
  ...
  while u <> nil do begin
	...
	drop_total (0,'   Day total: ',day_compiles,day_runtime);
	d := d^.next_day;
      end;
    end;
...
begin
  with current_record do begin
    find_user (users_ppn);
    with current_user^ do begin
      if (current_day = nil) orif (extr_date (current_day^.comp_first^.ctime) <> extr_date (date_and_time)) then begin
	new (d);
	d^.next_day := nil;
	d^.comp_tree := nil;
	d^.comp_first := nil;
...
begin
  with current_record, convert do begin
    if lines_out >= page_len then begin
      ...
      last_proj := '      ';
      last_prog := '      ';
      last_date_reported := minimum (last_date_reported);
    end;
    check_len;
...
begin
  ...
  if summarize then begin
    ...
    log_header := '*** Scan of PASCAL.LOG' || src_file_name;
  ...
  if summarize then begin
    if first_given then begin
      ...
      str[2:2] := lowercase (str[2:2]);
      both_line (out2);
      both_str (out2,'*** Scan from ' || str || ' to ');
...
begin
  ...
  repeat
    ...
    exit if summarize andif (secs_diff (date_and_time, last_date) > 0);
      if hiseg_size >= 400000b
	then hiseg_size := hiseg_size - 400000b
        else hiseg_size := max (0, hiseg_size);
	...
	global_err_free := global_err_free + 1;
      ...
      total_hiseg := total_hiseg + hiseg_size;
      ...
      max_hiseg := max (max_hiseg, hiseg_size);
