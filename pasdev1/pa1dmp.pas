...
type
    cptr = ^ cnode;
    cnode = record  num: integer; next: cptr end;
  ...
  with symbol^ do begin
	...
	begin
	  ...
	  if explicit_type then
	    fio_write (dumpfb, ', EXPLICIT TYPE')
	  else
	    fio_write (dumpfb, ', IMPLICIT TYPE');
	  if standard then
...
const opt_names: on_type :=
	...
	'CHECK POI', 'CHECK STK', 'CHECK STR', 'CHECK SUB', 'CHECK VAL', 'MAP',
	'SYMBOLS', 'CALLS', 'ASSEMBLY', 'XREF', 'SPEC COE', 'SPEC PTR',
	'SPEC WOR', 'TRACE', 'QBLOCKS', 'OPTIMIZE' );
