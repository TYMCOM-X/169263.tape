...
const
  version_string: string := 'Version 2.3';
...
var
  ...
  saved_extstatus:      machine_word;  (* for holding value of extstatus upon entry *)
...
$PAGE get_module, get_file - in debug$
...
(* GET_FILE parses a file reference with the syntax:  [ <file id>- | <file number>- ].
   If a file identifier is found SOURCE_ID.FILE_NAME is set to the text of the ID. IF an
   integer constant is found then SOURCE_ID.FILE_NUM is set to the number found.  Neither
   is modified, nor is the cursor changed if a syntactically correct file reference is not found.  *)

procedure get_file (var source_id: source_id_record);
...
begin
  ...
  with db_static^ do begin
    ...
    ovl_modno_ptr := ptr (#O400010);             (* restore original high seg *)
...
begin 
  ...
  with rt_static^ do begin
    ...
    ovl_modno_ptr := ptr (#O400010);             (* set overlay mod num of *)
    ...
    with db_static^, with_table do
      if (active_withs > 0) and cur_stmt_debug then begin
