...
var
  diridx, dirsize, storg, stend : integer;
  coerce: coerce_rec;
...
begin					(* procedure nsovfy *)
  ...
  if coerce.lh <> #o1776 then io_error (false, 'file is not in .EXE format');
  ...
  while dirsize > 0 do begin
	...
	if hspage = 0 then begin	(* the first one seen *)
	  ...
	  hsaddr := coerce.rh * #o1000	(* process addr *)
	  end
	else coerce.int := page [diridx + 1];	(* still gotta get next *)
	hscount := hscount + (coerce.lh div #o1000) + 1
	end
  ...
  if curmod^.marea <> Nil
    then begin
	 ...
	 available sizes. *)
      ...
      writeln ( tty , ' Actual code at         ' , hsaddr:6:O, ', ' , 
		    (hscount * #o1000):6:O, '(' , (hscount * #o1000):0 ,
		    ' decimal) words' );
	...
	vfyerr ('overlay area origin mismatch');
      if (hscount * #o1000) > curmod^.marea^.asize then
  ...
  writeln ( tty );
  writeln ( tty , ' Actual module index is ' , page [ #o10 ]:0 );
  if curmod^.modid <> page [#o10]
    then begin
  ...
  coerce.int := page [#o11];		(* static verification word *)
  ...
  if seen_static then begin		(* only if overlay has any *)
    ...
    writeln ( tty , ' Actual static at       ' , storg:6:O, ', ' ,
		(coerce.lh - storg):6:O, '(' , (coerce.lh - storg):0 , ' decimal) words');
...
$PAGE packdb the rest of it
  ...
  while curmod <> nil do begin
    if curmod^.modid > 0 then begin	(* nonsharable, potential entries *)
      ...
      readrn (olddb, #o201, page);	(* whack in the directory block *)
