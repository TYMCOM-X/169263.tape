...
$SYSTEM MMSSYM.TYP
$SYSTEM CMDUTL.TYP[,320156]
$SYSTEM LOOKUP.TYP[,320156]
$SYSTEM RUN.INC[,320156]
$SYSTEM MMTCOR.INC[,320210]
$SYSTEM INFPAC.INC[,320220]
...
begin
  if tty = nilf then begin		(* never opened *)
    open (tty);
    writeln (tty, 'ODMS Version 3.22');
    writeln (tty, 'Compiled : ', compdate, '  ', comptime );
    if curmdl <> nil then
  ...
  if curcmd = compile_cmd then do_compile	(* catch it now *)
	...
	if cur_item = resident_cmd then begin
	  ...
	  if not find (modname, curmdl) then
	    comerr ('++586unknown module name [' rrid || ']')
	...
	case cur_item of		(* whatever it is, get its param *)
	  ...
	  symbol_cmd: begin		(* so much for the simple ones *)
	    ...
	    if not find (modname, curmdl) then
	      comerr ('++630unknown module name [' || currid || ']')
	    else symmod := lastmod
	    end (* symbol_cmd case *);
	  ...
	  using_cmd: begin		(* munch rest of line *)
		...
		using_str := using_str || chr(#o15) || chr(#o12);  (* HACK! *)
