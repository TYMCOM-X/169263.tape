...
(*   +--------------------------------------------------------------+
	...
	ck_uniqueness
		    scans the symbol  nodes for public  and  external
		    symbols,  verifying that their  names  are unique
		    within  the  length  specified  by  PROG_OPTIONS.
     ...
     CHANGES:
        ...
        7/27/78  RNF    added   makearray  and  makerecord  routines;
                        ...
                        fields (see CIN-#2).
        11/14/78 RNF    The    makearray,   makerecord,   makestring,
	...
	6/16/82 DJW     Added CK_UNIQUENESS to support the EXTLENGTH
			option.
...
(* CST_INT converts a decimal or octal integer from source string form to
   an integer value, and then calls CST_SCALAR to add it to the scalar
   constant table.  *)

public function cst_int ( int_str: line_string; radix: integer; lindex: line_index ): val;
...
public function dcl_subrange ( base: typ; min,max: integer ): typ;
...
public function dcl_int ( min,max: integer ): typ;
...
public function dcl_char ( min,max: integer ): typ;
...
$PAGE dcl_array
(*  DCL_ARRAY takes a flexible array type and an integer array size, and
    returns an array type node for an array with the specified size which
    is compatible with the specified type.  *)

public function dcl_array ( flex_array: typ; array_size: integer ): typ;
...
function int_string (int: integer): line_string;
 var
   i: integer;
...
$PAGE ck_uniqueness
(* CK UNIQUENESS scans the symbol nodes for publics and externals, and
   emits warning messages for any that are not sufficiently unique (according
   to the EXTLENGTH option).  *)

public procedure ck_uniqueness;

  function match (s1, s2: packed array [1..*] of char): boolean;
    var
      ix: 1..maximum (prog_options.external_unique_length);
    begin
      match := min (upperbound (s1), prog_options.external_unique_length) =
	       min (upperbound (s2), prog_options.external_unique_length);
      if match then
	for ix := 1 to min (upperbound (s1), prog_options.external_unique_length) do
	  exit if s1 [ix] <> s2 [ix] do
	    match := false
    end;
  ...
  type
    sort_list_entry =
      record
	symbol: sym;
	next: ^sort_list_entry
      end;

  var
    sort_list, sort_cursor, last_cursor, temp_next: ^sort_list_entry;
    symbol: sym;
    len_label, kind_label: line_string;

  begin

    (* First, scan the symbol table and insert the public and external symbols
       into a list sorted by name. *)

    symbol := root_block^.children^.id_list.first;
    new (sort_list); (* dummy first entry on list *)
    sort_list^.next := nil;

    while symbol <> nil do begin
      with symbol^ do begin
	assert (kind in [consts, vars, conditions]);
	if public_dcl or (dcl_class = external_sc) then begin
	  sort_cursor := sort_list; (* the dummy entry *)
	  while (sort_cursor^.next <> nil) andif
		(sort_cursor^.next^.symbol^.name^.text < name^.text) do
	    sort_cursor := sort_cursor^.next;
	  temp_next := sort_cursor^.next;
	  new (sort_cursor^.next);
	  sort_cursor^.next^ := (symbol, temp_next)
	end
      end;
      symbol := symbol^.next
    end;

    (* Now run down the sorted list and detect sets of unsufficiently unique symbols. *)

    len_label := int_string (prog_options.external_unique_length);
    last_cursor := sort_list;
    repeat
      sort_cursor := last_cursor^.next;
      last_cursor := sort_cursor;
      while (last_cursor^.next <> nil) andif 
	    match (last_cursor^.next^.symbol^.name^.text, sort_cursor^.symbol^.name^.text) do
        last_cursor := last_cursor^.next;
      if last_cursor <> sort_cursor then
	repeat
	  with sort_cursor^.symbol^ do begin
	    if public_dcl then
	      kind_label := ' (PUBLIC '
	    else
	      kind_label := ' (EXTERNAL ';
	    if kind = vars then
	      kind_label := kind_label || 'VAR'
	    else if kind = conditions then
	      kind_label := kind_label || 'EXCEPTION'
	    else if type_desc^.kind = procs then
	      kind_label := kind_label || 'PROCEDURE'
	    else if type_desc^.kind = funcs then
	      kind_label := kind_label || 'FUNCTION'
	    else
	      kind_label := kind_label || 'CONST';
	    err_text (err_nonunique_ext,
		      name^.text || kind_label ||
		       ') isn''t unique within ' || len_label)
	  end;
	  sort_cursor := sort_cursor^.next
	until sort_cursor = last_cursor^.next
    until last_cursor^.next = nil;

    (* Finally, dispose of the sorted list. *)

    sort_cursor := sort_list;
    repeat
      temp_next := sort_cursor^.next;
      dispose (sort_cursor);
      sort_cursor := temp_next
    until sort_cursor = nil
  end.
  X@I