...
procedure init_output;
  ...
  begin
    ...
    listfb.new_page := fio_proc (fio_eject);
    listfb.page_header := fio_proc (fio_nop);
...
public function cvhex (value: int_type): cvhex_string;
  ...
  begin
    if value = 0
      ...
      while v <> 0 do begin
	first := first - 1;
	numbuf [first] := substr ('0123456789ABCDEF', (v mod 16) + 1, 1);
	v := v div 16;
      end;
...
procedure put_defname (intdef: def);
  ...
  const
    def_code: code_array := ('S', 'K', 'B', 'D', 'T', 'C', 'X', 'L');
	...
	parameter_sc, static_sc:
	  begin
	    if relsym <> nil then begin
	      if kind = static_sc then
		fio_write (listfb, 'STATIC.+');
	      fio_write (listfb, cv_int (relsym^.item_addr));
	    end;
	  end;
	...
	external_sc:
	  begin
	    if relsym^.init_value.kind = no_value then
	      with relsym^.name^ do
		fio_write (listfb, substr (text, 1, len))
	    else
	      with relsym^.init_value.valp^ do
		fio_write (listfb, substr (str_val, 1, str_len));
	  end;
 ...
 begin
  with cr^ do begin
	  ...
	  fio_tab (listfb, 25);
		...
		fio_write (listfb, '[' || reg_mnem (index) || ']');
              if (reloc.kind in [local_sc, parameter_sc, static_sc, external_sc])
                andif (reloc.relsym <> nil) then
                  with reloc.relsym^.name^ do
		    com_string := com_string || '  ' || substr (text, 1, len);
            end (* with *);
	...
	if kind = stringword then begin
	  ...
	  fio_write (listfb, cv_int (stringlen));
	  fio_skip (listfb);
	  ic := ic + 2
	end;
	put_ic (17);
	...
	fio_write (listfb, '"');
	fio_write (listfb, substr (strvalue,1,stringlen));
        fio_line (listfb, '"');
	ic := ic + stringlen
      end;
	...
	if psect = static_psect
	  then fio_write (listfb, 'PAX$STATIC')
	  else fio_write (listfb, 'PAX$CODE......B');
	...
	fio_tab (listfb, 9);
	fio_write (listfb, ';'); fio_tab (listfb, 17); fio_line (listfb, substr (ctext, 1, ctext_len)); 
	fio_tab (listfb, 9);  fio_line (listfb, ';');
      end;
	...
	ic := ic + 8
      ...
      maskword: begin
        put_ic (17);
	if block^.kind = program_blk then begin
	  fio_write (listfb, '.ENTRY');
	  fio_tab (listfb, 25);
	  fio_write (listfb, 'PAX$PROGRAM.,')
	end
	else if (block^.kind = subr_blk) andif block^.subr_sym^.public_dcl then begin
	  fio_write (listfb, '.ENTRY');
	  fio_tab (listfb, 25);
	  with block^.subr_sym^.name^ do
	    fio_write (listfb, substr (text, 1, len));
	    fio_write (listfb, ',')
	end
	else begin
  ...
  begin
    ...
    while symbol <> nil do
	  ...
	  if kind = vars then begin
	    fio_tab (listfb, 9);
	    fio_write (listfb, substr (name^.text, 1, name^.len));
	    fio_write (listfb, ' == STATIC.+');
	    fio_line (listfb, cv_int (item_addr))
	  end
