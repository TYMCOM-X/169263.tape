...
procedure put_field (off: unit_range; reloc: reloc_value);
 begin
  with reloc do begin
    case kind of
  ...
  if off < #O400000
    ...
    else fio_write (listfb, cvoct (off - #O1000000));
 end;
$PAGE put_symname
...
public procedure mac_end (startdef: def; code_size, const_size, static_size: unit_range)
 ...
 begin
  ...
  fio_tab (listfb, 9); fio_line (listfb, ';');
  fio_tab (listfb, 9); fio_write (listfb, ';');
  fio_tab (listfb, 17); fio_write (listfb, 'Public symbol definitions:');
  fio_tab (listfb, 9); fio_line (listfb, ';'); fio_skip (listfb);
  if prog_options.overlay_opt then begin
    fio_tab (listfb, 17); fio_write (listfb, 'RELOC'); fio_tab (listfb, 25);
    fio_line (listfb, cvoct (code_size + #O400000));
    fio_skip (listfb)
  end;
  ...
  while vl <> nil do
	...
	if (kind = vars) andif public_dcl andif
	  ...
	  fio_tab (listfb, 9); fio_write (listfb, name^.text);
	  ...
	  fio_tab (listfb, 17);
	  if prog_options.overlay_opt then begin
	    fio_write (listfb, 'EXP');
	    fio_tab (listfb, 25)
	  end;
	  if init_value.kind = no_value then
	    fio_write (listfb, cvoct (item_addr + size_init))
	  else
	    fio_write (listfb, cvoct (item_addr));
	  fio_line (listfb, '''')
	end;
      vl := last
    end;
  ...
  while symbol <> nil do
	...
	if kind = conditions then begin
	  fio_tab (listfb, 9); fio_write (listfb, name^.text);
	  fio_write (listfb, '=:'); fio_tab (listfb, 17);
	  fio_write ( listfb , cvoct ( item_addr + size_init + size_uninit ) );
	  fio_line (listfb, '''')
	end

	else if (kind = consts) andif (init_value.kind <> subr_cst) then begin
	  fio_tab (listfb, 9); fio_write (listfb, name^.text);
	  if prog_options.overlay_opt then
	    fio_write (listfb, '::')
	  else
