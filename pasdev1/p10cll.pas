...
function get_regs_used (subrdef: reloc_value): set_of_registers;
 ...
 begin
  if subrdef.kind = external_sc
    then get_regs_used := [0..#O15]      (* externals can clobber any reg *)
 ...
 begin
  with node^ do begin
    ...
    (* Indirect call thru a subroutine value.  The display for the called subroutine
      ...
      kill_regs ([0..#O15]);             (* assume all killed in lieu of better info *)
...
procedure for_call (node: expr);
 ...
 begin
  with node^ do begin
    ...
    kill_regs ([0..#O15]);                       (* must assume external proc kills all registers *)
