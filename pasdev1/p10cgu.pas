...
public function gen_cint (int_cons: integer): reloc_value;  forward;
...
public procedure gen_init;
 ...
 begin
  ...
  loc_code := #O400000;                  (* standard high segment base *)
  loc_static := 0;
 ...
 begin
  ...
  with od^ do begin
    ...
    if defined andif relocatable then begin
      ...
      fixup_required := (svalue < 0) or ( (svalue < #O400000) <> (intdef^.addr < #O400000) );
    end
    else
      fixup_required := false;
...
public procedure gen_source (srcid: source_id; srcindx: integer);
 var cr: code;
 begin
  new (cr, source);
  cr^.stmtid := srcid;
...
(* HALFWORD returns the low order 18 bits of a signed integer value as a positive
   value in the range 0..#O777777. *)

public function halfword (i: integer): unit_range;
 begin
  if i >= 0
    then halfword := i mod #O1000000
    else halfword := #O1000000 + (i mod #O1000000);
 end;
$PAGE gen, genind
...
public procedure gen
  ( opc: opc_range; acc, index: registers; offsnteger; rel: reloc_value );
 var cr: code;
 begin
  new (cr, instruction);
  with cr^ do begin
...
function rel_match (r1, r2: reloc_value): boolean;
...
(* GEN BLT generates a constant word whose left and right halfwords are
   ...
   in the blt area, and a reloc_value denoting the address of the generated
   constant is returned. *)


public function gen_blt (source_offset: code_address;
                         source_reloc: reloc_value;
                         dest_offset: code_address;
                         dest_reloc: reloc_value)
                            : reloc_value;
...
(* GEN BPTR generates a constant byte pointer referencing a slice of a particular
   ...
   byte pointer is placed in the byte pointer area, and a reloc_value denoting
   the address of the byte pointer is returned. *)

public function gen_bptr (mem: addr_desc): reloc_value;
 ...
 begin
  ...
  if (reloc.kind = temp_sc) and not quick then begin
    ...
    if reloc.kind in [parameter_sc, local_sc] then begin
      if bptr.bpoffset <= #O377777
        then bptr.bpoffset := halfword (bptr.bpoffset + reloc.relsym^.item_addr)
        else bptr.bpoffset := halfword (bptr.bpoffset + reloc.relsym^.item_addr - #O1000000);
      reloc.kind := absolute_sc;
    end;
  ...
  while btmp_area.first <> nil do begin
    ...
    with bptr_word^ do begin (* compute the byte pointer offset *)
      ...
      if bptr.bpoffset <= #O377777
        ...
        else bptr.bpoffset := halfword (bptr.bpoffset + bpreloc.relval^.loc.offset - #O1000000);
...
public function gen_cnode (value: val_ptr): reloc_value  options special(coercions);
...
public function gen_cval (value: val): reloc_value  options special(coercions);
  ...
  with value do begin
    ...
    (* Generate it now. *)
      ...
      if kind in has_ptr then
        valp^.def_addr := val_ptr(cons_def) (* coerce *)
    end
  end (* with *)
 end;
$PAGE gen_cword
...
public function gen_cword (cons_word: pdp10word; 
  cons_code: code_types): reloc_value;
...
public function gen_cint (* int_cons: integer): reloc_value *);
...
public function gen_cst ( value: integer ): addr_desc;
