...
(*  ArrBound takes a bound operator (LWB, UPB, or DIM), an array expression,
    ...
    If the index type is flexible and the operator is UPB or DIM, then the
    bound must be computed at runtime.  In this case, a bound computation
    expression is returned, with the array expression as its operand.  (Note
    that a flexible index type can only come from a bound function call
    with a dimension of 1.)  *)


public function arr_bound ( op: tuple_opcodes; arg: expr ): expr;
...
begin
  index := arg^.desc.base^.index_type;
  if op = dim_op
    then rslt_type := type_int
    else rslt_type := index^.base_type;
  if index^.flexible andif
     (arg^.opcode = field_ref) andif
     (arg^.base_rec^.opcode = agg_val) then begin
    with arg^.base_rec^, operand [upperbound (operand)]^ do begin
      if not desc.base^.flexible then
	index := desc.base^.index_type;
    end
  end;
  with index^ do begin
    if flexible and (op <> lwb_op) then
