...
$SYSTEM paslog.inc
$SYSTEM pasdat.inc
$SYSTEM corout.inc[31024,320156]
$SYSTEM versio.inc[31024,320156]
$SYSTEM prgdir.inc[31024,320156]
$SYSTEM dtime.inc[31024,320156]
$SYSTEM infpac.inc[31024,320156]
$SYSTEM run.inc[31024,320156]
$SYSTEM tmpnam.inc
$SYSTEM pasopn.inc
$SYSTEM pasrdr.inc
$SYSTEM paspt.typ
...
const
	...
	'POINTERS', 'STACK', 'STRINGS', 'SUBSCRIPTS', 'VALUES', 'COERCIONS',
...
begin
  ...
  with prog_options do begin
    ...
    if external_unique_length = 0 then
      out ('NOEXTLENGTH')
    else
      out ('EXTLENGTH (' || cv_int (external_unique_length) || ')');
...
begin
  ...
  exception
    ...
    stack_overflow: begin
      rewrite (tty);
      writeln (tty, '?The listing pass of the compiler has exceeded its available');
    ...
    io_error: begin
      rewrite (tty);
      writeln (tty, '?The listing pass of the compiler has encountered an I/O problem.');
      case exiostatus of
	io_rewr: begin
