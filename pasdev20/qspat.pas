...
begin (* Parsing the string pattern *)
  ...
  begin
    ...
    if (ch in legalopen) orif (ch = '{') orif  
       (ch = '|') orif (ch = chr(176b)) then begin
...
public function spatmatch (line: qstring; spat: spattern;
  ...
  begin
	...
	equal := substr(line,idx,len) = pattern.sstring
      else
	while (i < len) and (equal) do
	  if (pattern.sstring[i+1] = '?') orif
	    (pattern.sstring[i+1] = line[idx+i]) then
	      i := i + 1
	    else
	      equal := false
    end
...
$PAGE wildmatch
  ...
  begin  (* body of wildmatch *)
	...
	temppos := column (templine, spat.lastrec^, backward) - 1;
	pos := temppos + 1;
	...
	token:
	  if len = 0 then begin
	    ...
	    if (not wildcard) or (sidestars = neither) then
	      repea(* regular token searching--no critical wildcards *)
		if not wildcard then
