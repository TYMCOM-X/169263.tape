...
$INCLUDE qstr.typ[31024,320156]
$INCLUDE editre.inc[31024,320156]
...
type						(* processing modes *)
  ...
  cmdfile_type = (user, nonuser);		(* commands from the user or a file? *)
...
var
  ...
  curcmdfile: cmdfile_type;			(* source of command inputs *)
  ...
  saveline: string[255];			(* temp for the above *)
  saveidx: 1..256;				(* index into the above *)
  lineno: 0..99999;				(* line number in main file *)
  inc_lineno: 0..99999;				(* line number of include file, like above *)
  ...
  inc_file: text;				(* the include or header file *)
  cmdfile: text;				(* non-user command file *)
...
procedure list_header;
 ...
 begin
  ...
  title_line := 'File ' || f_name || '   ' ||
		...
		'   MDSI, Company Confidential   FORMAT Version 1.8  ';
  ...
  begin
	...
	then begin
	  ...
	  page_lineno := page_lineno + 1
       ...
       if curfile = main then begin	(* we only go one level on include files *)
...
var
    ...
    cline: cmdline;			(* the command line *)
...
function checkpunct ( ch: char ): boolean;
 begin
  skipblanks;
  checkpunct := false;
  if (lindex <= length (cline)) andif (cline[lindex] = ch)
    then begin
      lindex := lindex + 1;
      checkpunct := true
    end
...
function cmdfile_line: cmdline;
  begin
    cmdfile_line := '';
    loop
      while not eoln(cmdfile) do begin
	cmdfile_line := cmdfile_line || cmdfile^;
	get(cmdfile)
	end;
      readln(cmdfile);
    exit if cmdfile_line <> '' do
	   if curoptions.echo_cmds then begin
	     writeln(tty, '[', substr(cmdfile_line,1,length(cmdfile_line)), ']');
	     break(tty)
	     end;
    exit if eof(cmdfile)
    end;
...
$PAGE process_command_line mainline
 ...
 begin
  ...
  (* first, let's see if we have an @<cmd file> given. *)
  cline := line;
  startidx := verify (cline,[' '],1);

  if (cline[startidx] = '@') andif (curcmdfile = user) then begin
    reset (cmdfile, '.CCL ' || substr(cline,startidx+1));
    if eof(cmdfile) then begin
      close(cmdfile);
      reset (cmdfile, '.CMD ' || substr(cline,startidx+1));
      if eof(cmdfile) then begin
	close(cmdfile);
	writeln(tty,'Command file null or nonexistent.');
	goto (* next command *)150
	end
      end;

    (* we have a good command file *)
    curcmdfile := nonuser;
    cline := cmdfile_line
    end
  else if curcmdfile = nonuser then cline := cmdfile_line;

  lindex := index (cline, '/', length (cline) + 1); (* get filename *)
  f_name := substr (cline, 1, lindex - 1);

  idx := index (f_name, '=');		(* see if there is output info *)
  if idx = 0 then
    fid_specified := false
  else begin
    f_name := substr (cline, idx + 1, lindex-idx-1);
    curoptions.fid_qualifier := substr (cline, 1, idx - 1);
    fid_specified := true;
  end;

  if lindex <= length (cline) then begin	(*  a '/' was found *)
    lindex := lindex + 1;			(* position after the slash *)
    while lindex <= length (cline) do begin	(* loop over options *)
      if not lookup_cmdnames (cline, lindex, cmdnamelist, maximum (commands), cmd)
	then begin
	  ...
	  defcmd:     begin
			...
			idx := index (substr (cline, lindex), end_quote);
			if idx = 0 then begin
			  writeln (tty, 'File name defaults not terminated.');
			  goto 150;
			end;
			curoptions.input_defaults := substr (cline, lindex, idx-1);
			lindex := lindex + idx;
		      end;
	  ...
	  queueopt:   begin
			...
			if end_quote = ' ' then begin
			  ...
			  queue_options := substr (cline, lindex, idx-1);
			  lindex := lindex + idx;
			end;
		      end;
...
begin
  ...
  writeln (tty, 'FORMAT, Version 1.8');
  title_heading := '                             ';
  ...
  exit if quit_flag;
    ...
    if uppercase (curoptions.fid_qualifier) = 'LPT:' then begin (* queue result to printer *)
      curoptions.fid_qualifier := 'DSK:###FMT.LST';
	...
	if ch = cr then begin	(* assume directive not in middle *)
	  curmode := intext;
	  nextch;
	  if curfile = main then		(* we only go one level deep on includes *)
    ...
    inheader, ininclude:
      begin
	if curmode = inheader 
	  then reset (inc_file, '.HDR ' || inc_filename, [ASCII])
	  else reset (inc_file, '.INC ' || inc_filename, [ASCII]);
	if eof(inc_file) then begin
	  ...
	  saveline := inputline; saveidx := inputidx;   (* save the MAIN input *)
	  read (inc_file, inputline); inputidx := 1;     (* get the INCLUDE input *)
	  curfile := include;
	  curmode := intext;
	  inc_lineno := 0;
      ...
      if (curfile = include) andif eof(inc_file) then begin
	curfile := main;
	inc_lineno := 0;
	close(inc_file);
	inputline := saveline;
	inputidx := saveidx;
	readch				(* get a CH from the main file *)
	end
    end (* WHILE loop *);
100 (* EOF abort *) :
